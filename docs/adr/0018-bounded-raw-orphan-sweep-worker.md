...
- Run raw orphan reconciliation as one independently scheduled worker per service process. The worker obtains maintenance admission before opening or advancing the raw directory iterator. A denied admission performs no raw-directory I/O and persists a five-minute retry.
