...
- source_type: storybook_canvas
  ...
  sensitive_exclusion: N/A
  submission_gate: pending-owner-approval
  story_id_or_title: Account Pool/Components/Upstream Account Group Settings Dialog/Unlimited Draft
  state: draft-group-shared-settings
  evidence_note: 验证尚未落库的新分组草稿会通过同一套共享设置弹窗承载分组级草稿状态，当前值胶囊回显为无限且继续保持自动路由空态。
  image:
  ![Draft group settings unlimited](./assets/group-settings-unlimited-draft.png)
