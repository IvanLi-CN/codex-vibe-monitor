...
- `次数` 图已保留成功 / 失败 / 进行中分钟级柱状结构，并叠加分钟级对齐的低权重 `首字总耗时` 曲线；0 次调用分钟不会继承邻近分钟、10 分钟窗口或不一致源数据中的耗时样本。
- `/api/stats/timeseries` 响应出口、前端 API 归一化与实时增量合并已统一清理 `0 次调用 + latency` 的不一致点。
