...
- Derive all route-selection projections from a file-backed snapshot in one file read and one JSON parse. Keep only compact routing values after parsing; reopening the same snapshot for sticky key, prompt-cache key, model, encrypted-content, image, and compaction extraction multiplies CPU and filesystem work under concurrent large requests.
