...
- 高频 runtime snapshot 不应默认等同于主事实写。`running` / first-byte / response-ready 这类 UI 新鲜度事件可以先走内存态 + SSE，并把轻量占位放进 batch writer；terminal success/failure 才是必须同步覆盖的主事实。
...
- `running` snapshot 如果只是为 UI/SSE 提供进度，应避免每次同步写主表。可广播 `id=0` 的内存记录，batch 写入 `status='running'` 占位，并让 terminal 主事实在请求结束时同步覆盖；HTTP reconcile 在 batch flush 前可能短暂看不到 running 行，这是 bounded freshness 的一部分。
