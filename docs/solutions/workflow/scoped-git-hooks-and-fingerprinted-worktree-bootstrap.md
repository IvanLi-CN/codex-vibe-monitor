...
- Keep only staged-path formatters in `pre-commit`: Biome for web files, Rustfmt with the repository edition for Rust, and dprint for Markdown. Keep `stage_fixed` for fully staged files. The formatter wrapper inspects Lefthook's current unstaged-patch manifest and rejects a matching target before it runs, preventing context-drift during restoration.
