...
- Store one status and digest per dependency surface in each worktree's Git metadata. Digest root Bun, web Bun, docs Bun, and Cargo from their manifests, lockfiles, and toolchain input. A reusable Bun state needs every manifest-direct package directory, and Cargo needs every registry archive represented by `Cargo.lock`. Restore only a missing or changed surface; `--force` is the intentional escape hatch.
