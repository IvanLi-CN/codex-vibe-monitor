...
- Delete a legacy `prepare-commit-msg` wrapper only after generating the current Lefthook template in an isolated Git repository and comparing bytes. Update any managed hook only after the same exact-template comparison; preserve symlinks, configured hooks, and any non-identical local hook.

## Guardrails

- A formatter wrapper must filter deleted, nonexistent, and symlink paths before invoking a formatter and must receive only Lefthook staged files.
