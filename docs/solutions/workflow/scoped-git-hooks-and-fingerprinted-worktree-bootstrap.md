...
- CI smoke may copy the pinned Lefthook binary to a temporary repository-external path, but production hook installation must still require a genuinely external executable on `PATH`.
