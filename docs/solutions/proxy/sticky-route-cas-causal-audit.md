...
- Persist only structured causal evidence in operation events: reason code, routing source, status, and public attempt IDs. Keep raw upstream messages in the existing protected attempt detail surface.
...
- Verify the event API and UI show safe reason text plus Records links for cause and trigger attempts, while historical rows remain explicitly unannotated.
