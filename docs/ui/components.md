...
- `button.tsx`：`default / secondary / outline / ghost / destructive` 五种按钮语义，以及 `default / sm / lg / icon` 四种尺寸。
- `input.tsx`：标准输入框，包含边框、placeholder、focus ring、disabled 透明度约束。
- `card.tsx`：`Card / CardHeader / CardTitle / CardDescription / CardContent / CardFooter`。
- `badge.tsx`：`default / accent / secondary / success / info / warning / error` 七类状态徽标。
- `alert.tsx`：`default / info / success / warning / error` 五类提示面板。
- `switch.tsx`：布尔开关，`checked` 态使用 `primary`，`unchecked` 态停留在 `base-300`。
- `dialog.tsx`、`popover.tsx`、`tooltip.tsx`、`info-tooltip.tsx`：浮层与提示类组件。
- `filterable-combobox.tsx`：输入+筛选+列表框的组合输入模式。
- `form-field-feedback.tsx`、`floating-field-error.tsx`：表单标签与错误/说明信息。
- `spinner.tsx`：`sm / md / lg` 三档加载旋转器。
...
- 新增基础组件前，先确认能否通过现有 primitive 组合完成；只有当模式被多处复用且现有组合已经开始重复时，才上升为 `ui/*` 组件。
...
- 部分页面仍有直接拼 utility class 的表单区块，没有完全回收到 `ui/*` 组件层；新增页面应优先避免继续扩大这种分叉。
