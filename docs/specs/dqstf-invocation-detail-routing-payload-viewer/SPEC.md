...
- 以统一时间线暴露完整工作流：路由决定、等待、每次尝试，以及失败场景下最终返回给调用方的系统裁定。
...
- 时间线顺序固定为：辅助块（如路由决定、等待）在前，随后是每次尝试；如果最终调用失败，则在最后一次尝试之后补充一个系统裁定块。
...
- 成功、失败、池化、直连与“无尝试表行”的直通调用都必须走同一套工作流详情接口；缺失 attempt 行时允许合成一条 synthetic attempt 作为当前真相视图。

### 数据来源与持久化契约

- `codex_invocations` 是调用级主记录，继续承载完整 request/response raw body 路径，以及 share route / hero 所需的调用级上下文。
- `pool_upstream_request_attempts` 是每次尝试的独立事实表，尝试级结构化详情允许通过 `request_summary_json` / `response_summary_json` 保存，必要时可回退为运行时重建。
...
- 时间线可以顺序展示辅助块、尝试块和失败场景下的系统裁定块；同一时刻仅展开一个块，展开区保持紧凑。
- 点击尝试块后，默认展示概览，并可通过次级操作进入请求 / 响应；点击辅助块或失败裁定块后，默认展示概览，并可通过次级操作进入 JSON / 响应体（按块能力显示）。
...
绑定场景：Storybook `Invocations/InvocationWorkflowDetailPanel/FailedPoolWorkflow`。

- Story id: `invocations-invocationworkflowdetailpanel--failed-pool-workflow`
- 视觉证据覆盖五种状态：
  - 概览态：Rich Structured Snapshot + 路由块展开，证明高价值信息优先与时间线排序。
  - Dark theme 概览态：FailedPoolWorkflow 在 `themeMode=dark` 下保持深色实验台语义，hero 与 timeline surface 不再回退为浅色渐变面板，首屏文字与状态标签保持可读对比。
  - Light theme 概览态：FailedPoolWorkflow 在亮色主题下延续同一套 surface 层级，摘要区、时间线与内层信息块不再出现突兀的重边框，状态标签与正文保持稳定对比。
  - 尝试子详情目录：Attempt 1 默认展开后直接展示 `概览 / 时间详情 / 解析请求 / 请求头 / 请求体 / 解析响应 / 响应头 / 响应体` 八个页面入口，其中 7 个为显式尝试子详情页。
  - 请求头视图：Attempt 1 展开后切换到 `请求头`，证明请求头、路由与会话信号、请求体存档状态可以紧凑共存。
  - 请求体视图：Attempt 1 展开后切换到 `请求体`，证明调用级原始请求体、请求体存档状态与结构化 JSON 查看器可直接排障。
  - 响应头视图：Attempt 1 展开后切换到 `响应头`，证明响应头、传输与下游收口、响应体存档状态可直接查看。
  - 响应体视图：Attempt 1 展开后切换到 `响应体`，证明最终响应体的结构化查看和调用级归档来源说明同时可见。
