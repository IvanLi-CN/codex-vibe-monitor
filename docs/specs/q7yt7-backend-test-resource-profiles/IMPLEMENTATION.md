...
- 当前候选将 Cargo registry/git 与 target cache 分离。target cache key 同时绑定 lockfile、manifest 和 Rust source fingerprint，clippy 与 nextest 使用不同 target namespace；新 namespace 的首轮只读恢复旧 lockfile-only cache 作为 dependency seed，后续写入回到 source-key，避免旧 key 让互不兼容的产物长期命中。`profile.test` 关闭 debug information；PR Docker smoke 使用专用 `ci-smoke` profile，生产 release profile 不变。
