...
- 当前候选将 Cargo registry/git 与 target cache 分离。nextest target key 同时绑定 lockfile、manifest 和 Rust source fingerprint；迁移时先用同一三路径集合只读恢复旧 lockfile-only cache，再以分离的 source-key 写回，避免路径版本不匹配让 ancestor target 无法解包。test-only profile 固定为 `debug=0`；`codegen-units=256` 会改变 profile fingerprint 并使已恢复依赖全部失效，因此被撤回。clippy 只读取 legacy cache，不再保存独立 target，避免与 nextest 争用。PR Docker smoke 并行构建当前 debug binary 与 web bundle，只封装到私有 `ci-smoke-runtime` target；其 Ubuntu 24.04 runtime 与 host debug binary 的 glibc 对齐，生产默认 Bookworm `runtime` target 和 release profile 不变。
