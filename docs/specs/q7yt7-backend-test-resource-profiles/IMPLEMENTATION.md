...
- 当前候选将 Cargo registry/git 与 target cache 分离。nextest target key 同时绑定 lockfile、manifest 和 Rust source fingerprint；迁移时 Lint 可用同一三路径集合只读恢复旧 lockfile-only cache，但 archive producer 只恢复分离的 registry 与 source-key target，避免重复解包 legacy `target` 阻塞 backend critical path。test-only profile 固定为 `debug=0`；`codegen-units=256` 会改变 profile fingerprint 并使已恢复依赖全部失效，因此被撤回。clippy 只读取 legacy cache，不再保存独立 target，避免与 nextest 争用。PR Docker smoke 并行构建当前 debug binary、web bundle 与与生产相同版本的 Xray archive，只封装到私有 `ci-smoke-runtime` target；其 Ubuntu 24.04 runtime 与 host debug binary 的 glibc 对齐，生产默认 Bookworm `runtime` target 和 release profile 不变。
