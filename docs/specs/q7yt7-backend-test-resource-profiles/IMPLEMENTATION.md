...
- Portable backend contract: `BACKEND_TEST_WORKSPACE` accepts any normalized absolute writable directory; empty Cargo inputs become run-local `cargo-home`/`target`, explicit inputs are validated as disjoint external directories, existing and dangling symlink parents are resolved or rejected before writes, and `CARGO_NET_OFFLINE` is passed through unchanged.
- Production-copy validator: `scripts/validate-summary-production-fixture.sh` resolves repository paths from its own location, consumes only `SUMMARY_PRODUCTION_COPY` plus standard Cargo inputs, rejects source-snapshot and staged-copy overlaps, and creates/cleans a disposable Runtime Workspace. The backend-test image includes Python for the exactness oracle; `scripts/run-summary-production-copy-testbox.sh` is an optional direct-Docker adapter that stages under `/srv/codex/agents/<CODEX_THREAD_ID>/`, syncs only the helper-owned worktree child, mounts the selected paths, and does not require a shared-testbox runner.
...
- Catalog note: 三路 backend profile 合同保持不变；本轮收敛可移植 Rust 编译缓存输入、Archive file fixture、PR smoke build 与非 backend required-job 资源边界
