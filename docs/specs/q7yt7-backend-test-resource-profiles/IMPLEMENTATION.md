...
- 当前候选将 Cargo registry/git 与 target cache 分离。target cache key 同时绑定 lockfile、manifest 和 Rust source fingerprint，clippy 与 nextest 使用不同 target namespace；迁移时先用同一三路径集合只读恢复旧 lockfile-only cache，再以分离的 source-key 写回，避免路径版本不匹配让 ancestor target 根本无法解包，也避免旧 key 让互不兼容的产物长期命中。PR Docker smoke 并行构建当前 debug binary 与 web bundle，只封装到私有 `ci-smoke-runtime` target；生产默认 `runtime` target 和 release profile 不变。
...
- `CI PR` 和 `CI Main` 将 Rust `Lint & Format Check` 与 `Repository Tooling Checks` 分离，并把原有前端合并 job 拆为 `Front-end Tests`、`Storybook Accessibility Tests`、`Docs & Web Demo Build`；三个 backend jobs 保持不变。Records Overlay 与 Web Demo E2E 仍运行完整两套 spec，但在独立 server、report 和 result 目录中并行执行。release snapshot 等待完整的拆分拓扑；quality-gates 声明、contract fixtures 与 live-rules fixtures 同步使用新 names。
