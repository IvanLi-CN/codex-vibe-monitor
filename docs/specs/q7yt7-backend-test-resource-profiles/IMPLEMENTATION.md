...
- 当前候选将 Cargo registry/git 与 target cache 分离。nextest target key 同时绑定 lockfile、manifest 和 Rust source fingerprint；迁移时 Lint 可用同一三路径集合只读恢复旧 lockfile-only cache，但 archive producer 与 PR smoke 只恢复分离的 registry 与 source-key target，避免重复解包 legacy `target` 并使 smoke 的 private test profile 复用 archive producer 的依赖。source-key target 只可在 archive build 成功后保存，失败 run 不得写入后续 cold receipt 可恢复的 cache。test-only profile 固定为 `debug=0`；`codegen-units=256` 会改变 profile fingerprint 并使已恢复依赖全部失效，因此被撤回。clippy 只读取 legacy cache，不再保存独立 target，避免与 nextest 争用。PR Docker smoke 并行构建当前 binary、web bundle 与与生产相同版本的 Xray archive，只封装到私有 `ci-smoke-runtime` target；其 Ubuntu 24.04 runtime 与 host binary 的 glibc 对齐，生产默认 Bookworm `runtime` target 和 release profile 不变。
- Archive/File I/O runner 也会先生成一次真实 current-schema file template。普通 retention/archive DB tests 从它复制到各自唯一文件；四个 legacy migration/backfill tests（包括 blob-link trigger 回填）显式保留 fresh `ensure_schema`，gzip、路径、文件损坏与 write-lock tests 同样不走 template。文件副本的回归覆盖验证模板 mutation 不会泄漏到后续 test DB。
- `CI PR` 和 `CI Main` 将 Rust `Lint & Format Check` 与 `Repository Tooling Checks` 分离，并把原有前端合并 job 拆为 `Front-end Tests`、`Storybook Accessibility Tests`、`Docs & Web Demo Build`；三个 backend jobs 保持不变。Web type-check 从 PR smoke packaging 移到独立 tooling job，避免重复阻塞 Docker smoke。Records Overlay 与 Web Demo E2E 仍运行完整两套 spec、独立 server/report/result 目录；两个 suite 都固定单 worker，并在隔离 server 上并行，避免 Web Demo scene routes 争用同一浏览器 worker 而触发 retry。hosted runner 已提供 Chromium runtime 依赖，因此只下载 Playwright Chromium，不重复 apt 安装字体。Build Artifacts 复用 hosted runner 的 Rust system dependencies，并在 cargo/web compile 的同时下载 private smoke target 所需的 Xray archive；release snapshot 等待完整的拆分拓扑；quality-gates 声明、contract fixtures 与 live-rules fixtures 同步使用新 names。
