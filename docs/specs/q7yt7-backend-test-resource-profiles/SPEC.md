...
- 非 migration 的 in-memory schema 模板只能在 schema parity、独立连接可见性、并发写安全和跨测试隔离全部成立时采用；否则 Stateful SQLite 测试继续使用真实 `ensure_schema`。legacy migration、文件路径、gzip 和 write-lock 测试始终使用真实 schema/file fixture。
