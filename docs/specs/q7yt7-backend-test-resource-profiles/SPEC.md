...
- Cargo registry/git 与 `target` cache 必须分离；target cache key 必须同时绑定 `Cargo.lock`、`Cargo.toml` 与 `src/**/*.rs`，并保留仅按 lockfile 的 restore prefix，避免 clippy 和 nextest 复用不可变陈旧 target。
