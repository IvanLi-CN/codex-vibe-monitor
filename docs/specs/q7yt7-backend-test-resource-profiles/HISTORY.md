...
- host-built PR smoke binary 不能在 Bookworm glibc runtime 运行，私有 smoke target 改为 Ubuntu 24.04；生产 Bookworm runtime 保持默认。hosted runner 已具备 Playwright Chromium runtime 依赖，E2E 仅下载对应浏览器而不重复 apt 安装字体。Web Demo 的 scene route 在同一 worker pool 内并发会触发 Chromium 协议超时与 retry，因此 Records Overlay 和 Web Demo 都保留完整测试、各固定一个 worker，并在隔离 server 上并行。Build Artifacts 复用 hosted runner 已预装的 Rust system dependencies，并在 cargo/web compile 期间下载相同版本的 Xray archive，private target 不再重复执行 downloader stage；smoke binary 使用与 source-key target cache 对齐的 private test profile。
...
- 项目级验收固定为自有 `backend-test` target、确定性规模 fixture 与独立 exact oracle；PR/Main 直接硬门禁，Release 仅消费同一 target SHA 的成功 CI Main 和 runtime-image smoke，不依赖外部验证结果。
