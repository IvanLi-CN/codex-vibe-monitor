...
- host-built PR smoke binary 不能在 Bookworm glibc runtime 运行，私有 smoke target 改为 Ubuntu 24.04；生产 Bookworm runtime 保持默认。hosted runner 已具备 Playwright Chromium runtime 依赖，E2E 仅下载对应浏览器而不重复 apt 安装字体。Web Demo 的 scene route 在两 workers 下会触发 Chromium 协议超时与 retry，因此 Records Overlay 和 Web Demo 都保留完整测试但固定单 worker。Build Artifacts 复用 hosted runner 已预装的 Rust system dependencies，并在 cargo/web compile 期间下载相同版本的 Xray archive，private target 不再重复执行 downloader stage。
