...
- 已收口 Dashboard 自然日同进度比较的前端重复计算：同一时间序列与时区只计算一次，比较桶共用一个本地时间格式器；自然日图表网格复用有界 X 轴刻度，避免分钟级数据首绘时对大量候选刻度重复测宽；初始视口不变时不重复渲染图表。生产构建性能测试使用独立启用的分钟级本地 demo 夹具覆盖首屏、单次更新与持续更新。
...
- 已收紧未来调用的终态状态语义：严格 `response.completed` 成功终态优先于随后 EOF/传输现象；终态后诊断不进入 failure KPI，终态前 client abort 仍按原有 non-success 分类，历史数据保持不变。
...
- `web/src/features/dashboard/dashboardKpiComparisons.ts`
- `web/src/features/dashboard/dashboardTodayActivityChartData.ts`
- `web/src/demo/handlers.ts`
- `web/tests/e2e/dashboard-render-performance.spec.ts`
