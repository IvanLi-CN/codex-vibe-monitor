...
- Given the live routing tab is active, When a route selection, retry, terminal result or model-state transition occurs, Then the API Key model Gantt updates its corresponding time-axis text, state band, available-period allocation intensity, or controlled-recovery marker on the affected combination lane through the bounded `pool.model-routing-live` view without creating new upstream traffic, rebuilding a stable chart layout except at a calendar-header boundary, resetting horizontal scroll, or creating an internal vertical scrollbar.
...
capture_scope=complete live routing page
...
页面流使用登录豁免、纯前端、确定性仿真 fixture 的 `ui_demo`；组件级 Storybook play 覆盖甘特图状态与钻取，并断言甘特容器不具备纵向可滚动容量。完整页面证据使用受控桌面 `1440×900` 与嵌入态移动 `393×852` CSS 视口，均显示所有模型泳道；仅使用本地仿真数据，不访问真实后端。主人已确认该组截图准确反映稳定实例、自然高度与无图内纵向滚动。
