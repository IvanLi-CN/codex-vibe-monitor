...
- The standalone model-routing page receives an HTTP snapshot and a versioned `pool.model-routing-live` subscription only while active. Model routing and live conversations are top-level siblings: Live retains `对话 / 最新记录 / 代理`, while `/model-routing` contains only routing state, attempt and account/invocation drill-down. Its model blocks render native two-dimensional 24-hour Gantt grids: a fixed `API Key #id` combination lane column and a shared Beijing-time axis make observed state intervals proportional color bands, unknown history explicit, and each real selection or retry a separate marker. Account-pool display names and groups are intentionally excluded from this surface. Login health becomes compact by default and each model row loads its own 48-hour evidence only when expanded.
