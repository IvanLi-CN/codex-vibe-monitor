...
- Runtime candidate, model-health and reservation snapshots are refreshed by their mutation events and reconciled no more than once per 60 seconds. Request-time selection reads only the published snapshot and fails closed while it is cold or unavailable; it never falls back to candidate SQL. A mutation fences and clears the prior snapshot before refresh, so a build can install only when its generation is still current. A normal stale build gets one immediate successor; a further race remains fail-closed until the 60-second reconcile, avoiding an unbounded SQLite rebuild loop. The refresh lease releases its reconciliation bit on success, error, or cancellation. Recoveries that add capacity wake NoCandidate waiters only after their current snapshot is published.
- The NoCandidate wait path owns a 32-permit hard bulkhead. A saturated HTTP waiter path records `capacitySaturated` and returns the existing 503 with `Retry-After: 10`; healthy routes and already-upgraded WebSocket streams do not acquire a permit, and released capacity wakes waiters immediately. A release that races a pending or publishing snapshot defers that wake until the refreshed snapshot is installed, then replays it through the same ready gate.
