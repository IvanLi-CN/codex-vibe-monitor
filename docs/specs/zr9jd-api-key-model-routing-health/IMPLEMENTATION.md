...
- Runtime candidate, model-health and reservation snapshots are refreshed by their mutation events and reconciled no more than once per 60 seconds. Request-time selection reads only the published snapshot and fails closed while it is cold or unavailable; it never falls back to candidate SQL. A normal stale rebuild triggers one immediate successor; events during that successor leave one deferred 60-second reconciliation, so sustained mutations cannot starve publication or create an unbounded SQLite rebuild loop. An event refresh retains the last published snapshot for in-memory failover until its replacement is installed, while recoveries that add capacity wake NoCandidate waiters only after that replacement is published.
- The NoCandidate wait path owns a 32-permit hard bulkhead. A saturated waiter path records `capacitySaturated` and returns the existing 503 with `Retry-After: 10`; healthy routes and established streams do not acquire a permit, and released capacity wakes waiters immediately. A release that races a pending or publishing snapshot defers that wake until the refreshed snapshot is installed, then replays it through the same ready gate.
