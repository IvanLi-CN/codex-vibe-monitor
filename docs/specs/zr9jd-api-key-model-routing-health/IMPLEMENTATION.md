...
- Runtime candidate, model-health and reservation snapshots are refreshed by their mutation events and reconciled no more than once per 60 seconds. Request-time selection reads only the published snapshot and fails closed while it is cold or unavailable; it never falls back to candidate SQL.
