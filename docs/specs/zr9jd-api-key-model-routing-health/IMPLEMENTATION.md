...
- Runtime candidate, model-health and reservation snapshots are refreshed by their mutation events and reconciled no more than once per 60 seconds. Request-time selection reads only the published snapshot and fails closed while it is cold or unavailable; it never falls back to candidate SQL. Events arriving during a rebuild coalesce into one successor generation, so sustained mutations cannot starve publication or create an unbounded SQLite rebuild loop. An event refresh retains the last published snapshot for in-memory failover until its replacement is installed, while recoveries that add capacity wake NoCandidate waiters only after that replacement is published.
