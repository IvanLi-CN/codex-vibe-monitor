...
- Runtime candidate, model-health and reservation snapshots are updated by committed mutations and reconciled no more than once per 60 seconds. A committed API Key failure with an exact model atomically publishes a conservative replacement that excludes only that `(account_id, model)` combination; an account-level cooldown excludes the whole account before its reservation can wake another request. Diagnostic-only outcomes do not alter the published candidate view. Success, affinity, and capability observations notify the low-frequency reconcile without clearing the current snapshot or invalidating an established reservation generation. The periodic reconcile restores precise persisted health without an event-driven candidate SQL rebuild. Request-time selection reads only the published snapshot and fails closed while it is cold or unavailable; it never falls back to candidate SQL. The reader validates the refresh epoch before and after cloning the snapshot, and every request-time reservation carries that source generation and revalidates candidate eligibility and model health while holding the cap mutex, so a mutation cannot race an old Arc or a preferred-account shortcut into routing. The live-first handoff owns that generation-bound reservation across model-mapping awaits and rejects it if a replacement snapshot has installed; a live-first failure re-enters snapshot selection rather than resending its fenced account through a preferred-account shortcut. A normal stale build gets one immediate successor; a capacity-increasing event that races that successor gets one independently scheduled follow-up, while other further races remain fail-closed until the 60-second reconcile. This bounds SQLite rebuild work without deferring capacity recovery for a full tick. The refresh lease releases its reconciliation bit on success, error, or cancellation.
- The NoCandidate wait path owns a 32-permit hard bulkhead. A saturated HTTP waiter path records `capacitySaturated` and returns the existing 503 with `Retry-After: 10`; healthy routes and already-upgraded WebSocket streams do not acquire a permit. Capacity-increasing mutations and released reservations emit availability immediately. A committed failure fence publishes its conservative replacement before releasing its reservation, so that release can wake waiters without exposing the prior view. A release during a cold or publishing snapshot also records one replay signal for the next safe snapshot install, while awakened requests remain fail-closed until that snapshot is current.
...
- `cargo check`: passed.
- Full backend verification uses the repository runner profiles (`.github/scripts/run-backend-tests.sh`) or their controlled CI equivalents; bare Cargo full-suite commands are not a validation entry point.
