...
- Runtime candidate, model-health and reservation snapshots are updated by committed mutations and reconciled no more than once per 60 seconds. A committed route failure atomically publishes a conservative in-memory replacement which removes that account before its reservation can wake another request; the periodic reconcile restores the precise persisted model-health state without an event-driven candidate SQL rebuild. Request-time selection reads only the published snapshot and fails closed while it is cold or unavailable; it never falls back to candidate SQL. The reader validates the refresh epoch before and after cloning the snapshot, and every request-time reservation carries that source generation and revalidates it while holding the cap mutex, so a mutation cannot race an old Arc into routing. The live-first handoff owns that generation-bound reservation across model-mapping awaits and rejects it if a replacement snapshot has installed; a live-first failure re-enters snapshot selection rather than resending its fenced account through a preferred-account shortcut. A normal stale build gets one immediate successor; a capacity-increasing event that races that successor gets one independently scheduled follow-up, while other further races remain fail-closed until the 60-second reconcile. This bounds SQLite rebuild work without deferring capacity recovery for a full tick. The refresh lease releases its reconciliation bit on success, error, or cancellation.
