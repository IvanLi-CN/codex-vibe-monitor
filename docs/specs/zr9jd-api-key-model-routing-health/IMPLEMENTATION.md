...
- The standalone model-routing page receives an HTTP snapshot and a versioned `pool.model-routing-live` subscription only while active. Model routing and live conversations are top-level siblings: Live retains `对话 / 最新记录 / 代理`, while `/model-routing` contains only routing state, attempt and account/invocation drill-down. One read-only Frappe Gantt instance owns the shared Beijing-time SVG axis, grid and task rows; model tasks divide the chart into first-level groups, account-model tasks provide `API Key #id` lanes, and SVG-native rectangles and milestone polygons project proportional state intervals and every real selection or retry onto those lanes. Account-pool display names and groups are intentionally excluded from this surface. Login health becomes compact by default and each model row loads its own 48-hour evidence only when expanded.
