...
- Owner-approved mock-only `ui_demo` page captures use the live page's routing tab at desktop 1440x900 and embedded mobile 393x852 CSS viewports. They preserve complete page height through every model group, and the browser plus Storybook checks confirm that the Gantt container has no vertical scroll capacity. The normalized PNGs are stored as `assets/model-routing-gantt-stability-desktop.png` and `assets/model-routing-gantt-stability-mobile.png`.
...
- `bun run lint:web`: passed with repository-existing warnings; no errors remain in the changed files.
