...
- Runtime candidate, model-health and reservation snapshots are refreshed by their mutation events and reconciled no more than once per 60 seconds. Request-time selection reads only the published snapshot and fails closed while it is cold or unavailable; it never falls back to candidate SQL. The reader validates the refresh epoch before and after cloning the snapshot, and every request-time reservation revalidates that snapshot generation while holding the cap mutex, so a mutation cannot race an old Arc into routing. Any delayed model reservation also requires that published snapshot, and a live-first failure re-enters snapshot selection rather than resending its fenced account through a preferred-account shortcut. A normal stale build gets one immediate successor; a capacity-increasing event that races that successor gets one independently scheduled follow-up, while other further races remain fail-closed until the 60-second reconcile. This bounds SQLite rebuild work without deferring capacity recovery for a full tick. The refresh lease releases its reconciliation bit on success, error, or cancellation.
