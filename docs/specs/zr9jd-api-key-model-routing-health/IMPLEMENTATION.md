...
- The NoCandidate wait path owns a 32-permit hard bulkhead. A saturated waiter path records `capacitySaturated` and returns the existing 503 with `Retry-After: 10`; healthy routes and established streams do not acquire a permit, and released capacity wakes waiters immediately.
