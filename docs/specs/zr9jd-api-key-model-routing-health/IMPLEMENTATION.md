...
- The standalone model-routing page receives an HTTP snapshot and a versioned `pool.model-routing-live` subscription only while active. Model routing and live conversations are top-level siblings: Live retains `对话 / 最新记录 / 代理`, while `/model-routing` contains only routing states, decisions, and account/invocation drill-down. Login health becomes compact by default and each model row loads its own 48-hour evidence only when expanded.
