...
- The NoCandidate wait path owns a 32-permit hard bulkhead. A saturated HTTP waiter path records `capacitySaturated` and returns the existing 503 with `Retry-After: 10`; healthy routes and already-upgraded WebSocket streams do not acquire a permit. Every capacity-bearing mutation and released reservation emits availability immediately; a release during a cold or publishing snapshot also records one replay signal for the next safe snapshot install, while awakened requests remain fail-closed until that snapshot is current.
