...
- System Status 在启动时完成 last-good snapshot hydration，后台以最长 60 秒 cadence 维护 SQL 与文件体积结果；HTTP handler 只克隆内存响应，TTL 到期、失效标记或刷新失败都不在请求路径执行 SQLite、`Path::exists` 或目录扫描，失败继续返回 last-good 状态。
