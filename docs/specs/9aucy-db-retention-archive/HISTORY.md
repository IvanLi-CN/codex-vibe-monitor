...
- Historical rollup startup replay treats archive manifests as a resumable keyset. The durable cursor advances only through the bounded candidate window, so restart recovery and a completed-keyset wrap do not require a full manifest or filesystem scan.
- A replay that has persisted row progress retains its archive cursor; an archive whose bounded replay cannot begin advances through the keyset, schedules a short no-audit retry, and is retried after wrap, preventing one oversized compressed artifact from starving later candidates.
- Normal startup preparation now derives its historical-rollup backlog hint from the bounded keyset window, so startup cannot reintroduce a full archive manifest or filesystem scan.
