...
- `codex_invocations` detail prune 产生的 archive 必须标记为 `live_mirror`：它的 canonical record 仍保留在 live SQLite，仅用于 payload observability，不得成为 Summary/rollup repair/archive admission source。live mirror 必须使用与后续 authoritative archive 不冲突的确定性 mirror-only path。该路径仍需 prepared ledger、source identity、manifest 和同事务的结构化字段/raw-owner transition，但不生成 Summary proof。超过 retention 上限并删除 live record 的 archive 必须标记为 `authoritative` 并满足 Archive Publication Proof；无法自动证明角色的 legacy manifest 保持 `unknown`，按 potential source fail closed。
- `live_mirror` 到期后可走独立的 manifest/file TTL 清理路径：清理必须在 writer transaction 中再次确认角色与文件 SHA，不等待 Summary proof，也不得推进 authoritative source boundary。历史 LegacyMonth manifest 若与仍在线 canonical rows 完全匹配可降级为 `live_mirror`；部分匹配保持 `unknown` 并进入 bounded proof recovery，避免旧混合文件继续作为 authoritative source。
