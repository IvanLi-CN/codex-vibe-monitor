...
- Implementation summary:
  ...
  - install icon 已拆为透明 regular、独立 maskable 与 Apple touch 输出；`scripts/export_brand_assets.py` 为每个输出生成内容哈希文件名，`vite.config.ts` 只消费这组六个唯一资源。
  - `site.webmanifest`、`sw.js`、`version.json` 与 `index.html` 走重新校验，service worker 排除运行时元数据后再 precache，内容哈希图标由后端静态服务使用 immutable 缓存。
...
- generated regular/maskable/Apple icon contract with content-hashed filenames
...
- `scripts/export_brand_assets.py` 与 `web/scripts/check-pwa-assets.py`
  - 以 traced product mark 导出独立 regular、maskable 与 Apple touch 资源，并检查尺寸、透明度、safe circle、内容哈希文件名、manifest purpose、HTML 引用与 SW precache 边界。
...
- `web/tests/pwa/installable-runtime.spec.ts` 与 `web/tests/pwa/test-server.mjs`
  - 验证稳定 manifest identity、当前图标响应、缓存策略与 SW 不缓存运行时 manifest/version 元数据。
