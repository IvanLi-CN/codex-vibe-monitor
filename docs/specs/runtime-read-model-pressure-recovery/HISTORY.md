...
- Summary cold start now excludes Legacy Detail Mirror raw identity work from its first Projection build. The pressure-gated generic backfill resumes its durable cursor only after Bootstrap publication, so a large or slow `unknown` mirror population cannot hold every Summary window in cold-start `503`; unresolved files remain fail-closed while independently proven selections persist.
...
- Live-tail recovery now identifies source changes by `(row_id, invoke_id, occurred_at)` and persists a bounded target-watermark checkpoint. ACKs or restart replays already absorbed by the immutable Projection no longer create a false broad gap; missing durable identity remains fail-closed until reconciliation proves it. Pressure-deferred, reconciling, and live-gap readiness are reported as payload-free health diagnostics, while the release gate keeps recent windows exact.
