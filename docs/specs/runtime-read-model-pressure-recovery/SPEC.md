...
- All-time coverage uses independent global and account scope versions across manifest, rollup, replay and verified Snapshot V2 proof. A committed terminal tail advances the independent live-tail cursor and bounded overlay; it does not invalidate a completed historical checkpoint or trigger full live admission. Only a changed coverage input restarts its affected scope, and an unknown impact remains fail-closed.
