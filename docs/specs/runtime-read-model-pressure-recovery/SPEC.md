...
Summary、后台回填、hourly rollup 与 terminal batch writer 共享 SQLite 的有限写入能力。Summary hydration 不能因 archive manifest、高基数历史或与 `StatsResponse` 无关的原始 payload 字节而放弃一个本可精确恢复的快照；压力门拒绝低优先级工作也不能演变为毫秒级重试、日志风暴或无动作审计；hourly rollup 与 P2 flush 不得延迟 P1 terminal durability；遗留长期 interval migration 不应反复执行全窗反关联扫描并阻塞 P1。

这些是四个相互关联但可独立交付的运行态恢复问题，但它们必须共享同一条边界：SQLite 是耐久与恢复事实源，健康 Summary HTTP 是精确内存读模型，低优先级 maintenance 不得反向挤占代理 P1。
...
- 让 hourly rollup 与 P2 batch flush 在压力下有界让出，保持 terminal P1 写入与可恢复队列优先级。
...
- hourly rollup 与 SQLite terminal batch writer 的低优先级 admission、锁处理与 P1 优先级合同。
...
- Projection 的 canonical record 只能保存构造 `StatsResponse` 所需的已规范化字段；原始 invocation payload、raw response、未消费错误正文及其他大文本不得作为 Summary read-model 的常驻字节或 admission 输入。
...
- archive manifest、历史行数或原始文本字节超过固定 admission 预算时，系统必须使用受控的 compact rollup/boundary 恢复；不得因为正常历史规模或与 Summary 无关的原始文本而把合法选择永久降级为初始 hydration 失败。
...
- `SQLite Pressure Defer` 是数据库访问前的低优先级拒绝；它不读取或修改 durable progress，而是为每个 task 与 pressure generation 只登记一次内存 scheduler next-eligibility deadline。cooldown 尚未到期时，重复 notification、ticker 或输入变化不得重新派发同一 task；只在 deadline 到期或 pressure 已重新 eligible 时允许一次重新选择。
...
#### Hourly Rollup And P2 Writer Admission

- hourly rollup、P2 system-task finalization 与 P2 batch flush 是低优先级数据库工作；它们必须在进入 SQLite 前遵守共享 pressure admission，并在 `BUSY`/`LOCKED` 后以一次有界延期让出，而非固定短间隔重复 retry。
- P1 terminal durability 不得等待 P2 rollup 或 P2 flush 的长事务、重复 lock retry 或不受控 backlog drain。P2 被延后时必须保留可恢复队列与明确优先级，不能丢弃 terminal facts。
- stale queue、lock retry 与 pressure defer telemetry 必须区分来源和优先级；同一 pressure generation 不得形成每个 ticker 或每条 terminal 的告警风暴。
- 任何低优先级 writer 的 gate 拒绝、空 pass 或 cancellation 都不得增加 `system_task_runs` 审计；真实非压力错误保留可诊断失败路径。
...
3. 低优先级 backfill 遇到关闭的 pressure gate 时只注册一次内存 scheduler future-eligibility deadline；对应 deadline 或重新 eligible 的事件只重新选择一次候选任务，执行前仍检查 durable progress，durable progress 保持不变。Account Activity V2 coverage repair 在同一 permit 内完成这个 due 检查与 repair，避免嵌套获取 global gate。
4. hourly rollup 与 P2 batch writer 在压力下先让出并保留可恢复队列；P1 terminal 写入可先完成，低优先级 work 只在下一次 eligible deadline 或明确恢复事件重新尝试。
...
- low-priority rollup/P2 lock 不能被当成 terminal P1 的正常等待；它必须可归因、可延后且不重复短退避。
...
- Given 同一 cooldown 内重复的 gate notification、ticker 与输入 wake，When 同一 startup backfill task 已有 pending deadline，Then 不重新派发或逐次记录 defer；deadline 到期或 gate 恢复后恰好重新选择一次。
...
- Given hourly rollup 或 P2 batch flush 返回实际 SQLite lock，When P1 terminal work 与其并发，Then P2 只保留一次可恢复延期、P1 可继续排空，且没有固定短 retry、无动作审计或重复 stale 告警。
...
- [ ] hourly rollup/P2 writer 的 low-priority admission、lock attribution 与 terminal P1 优先级明确且可测试。
...
- Summary Ticket 使用 production-shaped archive 与大原始 payload fixture、HTTP SQL/file counter 与 exactness 对账；writer Ticket 使用受控 lock/backlog fixture；long-term Ticket 使用 `EXPLAIN` 与 bounded-migration fixture。
