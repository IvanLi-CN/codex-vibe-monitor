...
- all-time reconciliation 必须以 durable generation-fenced checkpoint 分别提交 global/account manifest proof cursor、rollup cursor 与 aggregate accumulator；每个 microbatch 只读取并提交有界 page。finalization 只能从已完成 scope 的 checkpoint 原子发布精确结果，不能重新全扫历史。运行期间发现 fence mismatch 必须取消该阶段、释放 refresh ownership 并交接 bounded rolling rebuild；全历史收敛不得使独立 Exact-Ready 的 `current`、rolling 或 calendar selection 因共享 finalization 而超过 freshness budget。
...
- Snapshot V2 page order and cursor proof MUST use the deterministic `(UTC occurred_at, id)` key. The generic ID-ordered hourly-rollup pager is not a Summary proof. A pre-V2 ID-only cursor without complete V2 authority MUST be reset for that manifest before recovery continues; semantic or manifest identity failures MUST be finite unavailable outcomes keyed by manifest SHA, while source-read and SQLite pressure failures remain bounded retryable outcomes.
- Snapshot V2 page progress MUST NOT be treated as coverage authority. Each completed non-mirror invocation manifest MUST have a durable Summary Coverage Obligation until an identity-bound V2 final-proof marker verifies every page, manifest SHA, ordered key, row count, coverage endpoint, payload hash and semantic decode. A `Complete` attempt outcome without that marker MUST remain eligible for recovery; only final proof resolves the obligation and advances the affected global/account coverage fence. Page or manifest mutation MUST revoke the marker and reopen the obligation. Legacy terminal outcomes MUST map to a finite range-local gap and MUST be reactivated only when the manifest SHA changes.
- Historical Summary Coverage Recovery MUST advance without HTTP or SSE ownership. Each bounded turn obtains the global database-pressure permit before progress reads, alternates AllTime and V2 work, and preserves an uncommitted cursor on pressure, deadline, lock, or write failure. A newly verified V2 proof MUST trigger a bounded metadata-only publication so the recovered selection becomes available without client interest. Global and account all-time aggregates MUST retain the coverage fence that actually produced them; a newer rolling generation fence cannot renew or suppress finalization of a retained aggregate. An unchanged ready checkpoint MUST NOT repeat all-time finalization. The V2 due queue prioritizes new current-30-day candidates and eligible due retries before its monotonic archive-ID fairness sweep; a retryable candidate remains excluded until `next_probe_at`, so a new tail cannot starve older relevant authority or create a tight retry loop.
...
- Given legacy `unknown` detail mirror 数量超过通用 backfill 的单次候选页、且该队列受 pressure defer，When 正常启动 Summary，Then Summary Startup Recovery Gate 不得打开、解压或校验任何 raw legacy archive，随后发布独立已证明的 Bootstrap Projection；identity cursor 只在 Projection 发布后由 pressure-gated generic backfill 恢复。一个不可读或不匹配 archive 保持 `unknown` 并按相交 selection fail-closed，但不得令全部合法窗口持续 cold-start `503`，且关闭 SQLite 后已发布的合法 current/1d/rolling HTTP read 仍为零 SQL/文件 I/O。
...
- Given representative-scale validation requests a bounded `current` limit, When the exact oracle compares the published Projection, Then `current` counts and totals cover precisely the requested newest-N prefix while rolling/calendar windows retain their full exact range totals.
...
- Given overflowed boundary manifest page 的某个 exact range 需要 raw archive replacement，When Bootstrap 或 Rolling 建立 Projection，Then 它只记录该 range/current-rank 的 unavailable proof，绝不开始 paged raw archive hydration；All-Time checkpoint 在 Bootstrap 发布后独立执行该 raw recovery。
- Given 前一 Projection 已有 Historical Live Coverage Proof 且新 live high-watermark 只增加 recent-tail records，When Rolling refresh，Then 它不得执行全 historical live group-by，仍在 rolling deadline 内发布 exact current/rolling Projection。Given 一个新 ID 回填到该历史 overlap，Then Rolling 先发布相交时间范围的 unavailable proof，独立 background pass 完成后才清除该 proof；整个 Projection 不得因该 pass 失败而变 stale unavailable。
- Given overflowed all-time history 的 checkpoint 已提交若干 manifest/rollup page，When 服务重启、deadline 取消或 generation 变化，Then 新进程从匹配 fence 的已提交 cursor/accumulator 继续，或只废弃该旧 generation；不得重新扫描已完成 page，也不得发布 mixed-generation `all`。
