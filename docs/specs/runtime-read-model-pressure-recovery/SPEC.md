...
- all-time finalization 使用独立的有界阶段预算；它运行期间必须以同一 generation fence 维持已发布 rolling Projection 的 freshness。全历史收敛不得使独立 Exact-Ready 的 `current`、rolling 或 calendar selection 因共享 finalization 而超过 freshness budget。
