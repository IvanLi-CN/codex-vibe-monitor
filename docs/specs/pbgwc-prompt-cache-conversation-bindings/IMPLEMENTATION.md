...
- Storybook `DrawerRouting`, `DrawerRoutingMobile`, and `DrawerOperations` evidence at `./assets/conversation-routing-*-storybook.png` covers the complete route view, all-model fallback plus normalized buckets, five fitted mobile tabs, full reset confirmation, and concrete model-filtered routing events.
