...
- 工作对话卡片、上游账号卡片的 recent 调用行与账号详情调用记录使用相同的紧凑延迟合同：秒值最多保留一位小数；按十分之一秒四舍五入后达到 `100 s` 时显示整数。TTFT 使用 `firstTokenMs`，响应耗时始终使用 `tUpstreamStreamMs`；调用级 TTFT 只能归属同一调用的最终真实 upstream attempt，持久化列表与两条工作流 hydration 路径均按 `attempt_index DESC, id DESC` 选择该行，`budget_exhausted_final` 等伪终态不参与选择，较早 retry 显示 `--`。HTTP、SSE 与客户端运行时合并都必须保留已测得的 `firstTokenMs`，并且只能在该值已测得时声明“响应中”，不得以 `tUpstreamTtfbMs`、流耗时或 `now - occurredAt` 的经过时长替代。请求或排队中两项未产生时显示 `--`，响应中必须已有并显示 TTFT，而尚未结束的响应耗时显示 `--`。TTFT 使用与其他成功指标一致的绿色。
