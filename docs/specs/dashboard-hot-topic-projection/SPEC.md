...
- 直接非代理 terminal row replacement 与 in-flight-to-terminal transition 必须通过 durable recovery generation 使 global、account 和 hot-topic minute selection 回退到 exact；schema 安装 trigger 后，runtime startup 必须在 HTTP 读服务前推进同一 recovery generation。projection supervisor 作为 P2 延后运行时不得留下 replacement 窗口：它必须分批将 coverage 标为 warming，并仅在 generation 未变化时清除 recovery marker；清除后精确回退可重新预热对应 selection。读路径若异常发现 fence 缺失或 recovery pending，不得复用 ready coverage 或触发会使其可复用的 P2 warm，而必须保持 exact fallback；account 的 projection snapshot 失效回退必须替换已载入 aggregate，不得叠加，并保留预填充的 reporting buckets 以接收 exact records。
