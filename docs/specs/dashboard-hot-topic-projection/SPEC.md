...
- 直接非代理 terminal row replacement 必须通过 durable coverage invalidation 使 `all` selection 回退到 exact；account 的 projection snapshot 失效回退必须替换已载入 aggregate，不得叠加，并保留预填充的 reporting buckets 以接收 exact records。
