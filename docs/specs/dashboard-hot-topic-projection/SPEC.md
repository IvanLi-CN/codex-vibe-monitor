...
- 直接非代理 terminal row replacement 必须通过 durable coverage invalidation 使 `all` selection 回退到 exact；durable fence 尚未安装时，global HTTP、account HTTP 与 hot-topic 均不得复用 ready coverage 或触发会使其可复用的 P2 warm，而必须保持 exact fallback，直至启动恢复完成失效；account 的 projection snapshot 失效回退必须替换已载入 aggregate，不得叠加，并保留预填充的 reporting buckets 以接收 exact records。
