...
- 直接非代理 terminal row replacement 必须通过 durable coverage invalidation 使 `all` selection 回退到 exact；schema/startup 必须在 HTTP 读服务前安装或刷新 durable fence，projection supervisor 作为 P2 延后运行时也不得留下 replacement 窗口；读路径若异常发现 fence 缺失，不得复用 ready coverage 或触发会使其可复用的 P2 warm，而必须保持 exact fallback；account 的 projection snapshot 失效回退必须替换已载入 aggregate，不得叠加，并保留预填充的 reporting buckets 以接收 exact records。
