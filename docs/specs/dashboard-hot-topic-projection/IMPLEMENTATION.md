...
- 最终 attempt 的实时 TTFT 例外由同一条 attempt 证据 SQL 与 workflow mapper 实现：调用仍为 running 时，最终真实 attempt 必须明确处于 `responding` / `streaming_response`，才可使用调用级有限非负 `firstTokenMs`；仅处于 `waiting_first_byte` 的 retry 继续屏蔽调用行上可能残留的旧 TTFT。聚合与 in-flight 查询分别对 `first_token_ms` 和 `t_upstream_stream_ms` 使用最终 attempt timing SQL，Dashboard persisted baseline 也以该 TTFT 推断 phase。
