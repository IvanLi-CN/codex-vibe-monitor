...
- 提供一个 Dashboard 专用的后端批量读接口，一次返回账号级摘要与最多 16 条 recent 调用记录，禁止前端 fanout 账号详情或 `window-usage` 做 N+1 聚合；账号卡再按活动快照统一展示窗口截断。
...
- `web/src/features/dashboard/DashboardWorkingConversationsSection.tsx` 及新增账号视图组件：右上 tabs、badge、usage disabled 回退、账号卡布局与动态 recent 调用展示窗口。
...
- 活跃 `today / 1d / 7d` 窗口缺失的闭合小时 coverage 必须优先后台修复。每轮 selection 最多选择 `2` 个 bucket，selection 本身必须在 SQLite progress handler 与 Rust-side coverage scan 的共享 `2s` deadline 内完成；超时中断查询并放弃该轮，不得只在结果返回后检查 elapsed。repair 只在同一 deadline 仍有剩余时开始，并在每个实际修复事务内完成 exact 聚合、幂等 upsert、watermark 与 marker；owner 请求路径只读取 coverage 状态，不得触发 repair。selection 必须使用 `archive_batches` 的 normalized epoch coverage partial index；仍无边界的 legacy manifest 继续按 Shanghai `month_key` 兜底。
...
- Dashboard 共享 UI Tooltip 必须允许指针从触发区移入浮层阅读；移动过程中保持显示，进入浮层后继续保持，只有指针离开浮层及过渡通道后才关闭。触发区获得键盘焦点时必须取消待执行的指针关闭并保持可读；触屏长按提示在释放后关闭，除非键盘焦点仍留在触发区。点击固定语义保持不变。
...
- recent bridge 作为 recent 区标题行右侧统计例外，必须显示完整状态文字；运行态必须拆成 `排队中 / 请求中 / 响应中`，数值来自账号级 `inProgressPhaseCounts`，终态继续使用账号级 `successCount / failureCount / nonSuccessCount`，并与左侧“最近 N 条调用”标题保持同一垂直对齐节奏。
- 单账号卡下半部分必须展示当前范围内最近 N 条调用记录，复用现有紧凑调用行语言，而不是再做卡中卡；N 由同一活动快照内账号最大进行中调用数计算并限制在 4 到 16，返回不足时只显示实际行数，不得依赖展开、滚动或裁切。
...
- `GET /api/stats/dashboard-activity/recent` 必须接收第一阶段响应的 `rangeStart/rangeEnd/snapshotId`，用一次批量读取返回所有活动账号最多 16 条 bounded recent rows；不得逐账号发 SQL，也不得以并发 N 个 SQL 伪装批量读取。响应必须回显快照边界，前端只合并当前请求序列与当前快照一致的结果，前端再按统一窗口 N 截断。
...
- 账号卡顶部显示 `displayName` 与必要 badge；主体显示账号级 KPI；底部显示按统一窗口 N 截断的 recent 调用记录。
...
- 当某账号范围内只有失败 / 中断调用时，请求分解、Token 分解与按 N 截断的 recent 记录仍需稳定显示，不得因为缺少 success 样本而隐藏整卡。
...
| `GET /api/stats/upstream-account-activity`             | http-endpoint       | external      | Add            | None                     | backend/stats   | Dashboard account activity tab            | range 聚合 + effective routing rule + fixed recent 16 bounded query |
...
- Given 查看 Dashboard 共享 UI Tooltip，When 指针从触发区移向浮层，Then 浮层在过渡期间保持显示，指针进入后可停留阅读；When 指针离开浮层与过渡通道，Then 浮层关闭。
...
- Given 活动快照内账号的最大 `inProgressInvocationCount` 为 `M`，When 查看账号卡底部，Then 所有账号统一显示窗口 `N = clamp(M, 4, 16)`，按 `occurredAt DESC` 排序；单账号返回不足 N 条时只显示实际行数。
...
- Given 查看账号卡 recent 区标题行，When 右侧存在 recent bridge 统计，Then 显示完整状态文字，并与左侧“最近 N 条调用”标题保持同一垂直对齐。
- Given 查看账号卡内 recent 调用记录，When 与对话卡片调用记录对照，Then recent 行显示同一套 `Hit`、`Token` 与成本三字段摘要，不出现可见的 `IN / CW / C / O / T` 或 `RQ / UP / ED / TT` 正文片段，且 N 条记录（或返回的更少记录）完整留在卡内。
...
- Given 后端账号活动接口需要账号摘要与 recent 记录，When 发起请求，Then 单次 batch endpoint 固定按 `recentLimit=16` 返回，不依赖前端 fanout `upstream-account detail` 或 `window-usage`。
...
- Given N 个活动账号，When 第一阶段使用 `includeRecent=false`，Then 不执行 recent query；When 第二阶段加载 recent，Then 使用一次批量读取，后端每账号最多返回 16 条，前端按 `clamp(max(accounts[].inProgressInvocationCount), 4, 16)` 截断。
...
- source_type: storybook_canvas
  story_id_or_title: `dashboard-workingconversationssection--upstream-account-tab`
  scenario: `mobile390`
  evidence_note: 验证相同账号 tab 在移动视口下收敛为单列卡片，并保留摘要区与该基线 fixture 的最近 4 条调用记录；动态 7/16 条窗口由专门 Storybook 场景覆盖。
  image:
