...
- 活跃 `today / 1d / 7d` 窗口缺失的闭合小时 coverage 必须优先后台修复。每轮最多选择 `2` 个 bucket，selection 与 repair 共用最多 `2s`；selection 本身必须由 SQLite progress budget 约束，超时中断查询并放弃该轮，不得只在结果返回后检查 elapsed。selection 必须使用 `archive_batches` 的 normalized epoch coverage partial index；仍无边界的 legacy manifest 继续按 Shanghai `month_key` 兜底。每个实际修复在同一事务内完成 exact 聚合、幂等 upsert、watermark 与 marker；owner 请求路径只读取 coverage 状态，不得触发 repair。
