...
- Dashboard 工作区 `对话` tab 的 recent/current 调用错误摘要，以及 `上游账号` tab recent 行错误摘要，必须统一保持单行省略；摘要文本本身就是 tooltip trigger，hover / focus / long-press 时使用 UI 库 tooltip 展示完整错误，不得依赖浏览器原生 `title` 作为最终交互。
...
- source_type: storybook_canvas
  ...
  evidence_note: 验证 Dashboard `对话` tab 当前调用错误摘要保持单行省略，不撑宽卡片；hover 摘要文本时只通过共享 UI tooltip 展示完整上游错误，不再依赖原生 `title`。
  image:
  PR: include
...
- source_type: storybook_canvas
  ...
  evidence_note: 验证 Dashboard `上游账号` tab recent 行错误摘要同样保持单行省略，不撑宽 row；hover 摘要文本时通过同一共享 UI tooltip 展示完整错误，实现与对话卡片一致的错误披露语义。
  image:
  PR: include
