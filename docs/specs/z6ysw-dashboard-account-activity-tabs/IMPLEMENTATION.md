...
- 已实现：Dashboard 当前进行中、重试与阶段计数改由后端 runtime store 单次读取生成版本化 `dashboardActivityLive` SSE 快照；前端按 revision 覆盖顶部与账号卡 live 字段，旧 HTTP reconcile 不再把新状态回写为 0，重连时服务端立即种入当前快照。
