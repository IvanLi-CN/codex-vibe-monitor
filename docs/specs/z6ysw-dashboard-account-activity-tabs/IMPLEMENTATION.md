...
- 已实现账号活动 v2 hourly rollup：`upstream_account_stats_hourly` 新增独立 v2 聚合字段与 replay target，Dashboard full 和 upstream-account 共用“covered full hours rollup + uncovered contiguous hours exact fallback + boundary exact tail” builder。live 与 archive repair 使用独立 cursor，marker 完成前不消费默认零值；活跃窗口 selection 最多选择 `2` buckets，并由 SQLite progress handler 与 Rust-side coverage scan 共享 `2s` deadline，repair 只在剩余预算内启动，历史无进展回填按 `15s -> 1m -> 5m -> 15m` 退避。
- 已实现 active coverage selection budget：archive manifest coverage bounds 归一化到 epoch 并通过 completed-invocation partial indexes 查询；legacy 无边界 manifest 保留 Shanghai month fallback。priority selection 在 SQLite progress handler 内受 `2s` deadline 约束，超时中断查询并跳过该轮，不再只在结果返回后检查耗时；production QueryBuilder 的 EXPLAIN 与 20k archive fixture 覆盖该计划与预算。
