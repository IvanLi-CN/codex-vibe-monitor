...
- Downstream feature flag: `OPENAI_PROXY_WEBSOCKET_ENABLED=false` 时，WebSocket upgrade 返回 HTTP `503` JSON error；设置为 `true` 后才允许 downstream WS。普通 HTTP proxy 不受该开关影响。
- Upstream default flag: `OPENAI_PROXY_UPSTREAM_WEBSOCKET_DEFAULT_ENABLED=false` 时，proxy 不主动连接上游 WS；设置为 `true` 后，downstream WS 请求才会尝试上游 WS。普通 HTTP 请求仍走 HTTP，除非后续版本实现并启用可验证 HTTP→WS 事件桥接。
...
- Failure:
  ...
  - `OPENAI_PROXY_UPSTREAM_WEBSOCKET_DEFAULT_ENABLED=false` 或所有候选都带 no-ws tag：WebSocket upgrade 返回 `503`，不建立不可靠隧道。
...
Storybook canvas 证据展示设置页中的 WebSocket 运行期开关。两个开关从启动环境变量读取并以只读状态呈现，避免误导为可热保存配置。

![Settings WebSocket runtime gates](./assets/settings-websocket-runtime-gates.png)
