...
- Note: 账号详情统计已有 minute/hourly read-model 与有界 tail 契约，但生产复查确认 `window-usage` 仍可从 payload 提取账号归属并在多账号选择上形成宽范围 SQLite 读取。该缺口由 high-frequency runtime storage-plane work 收敛；在完成每账号 selection、coverage readiness 与 in-flight coalescing 前，不得宣称 read-model 已消除在线重算。
