...
- 新 terminal 记录必须写结构化 nullable `codex_invocations.upstream_account_id`。旧行仅由 pressure-gated、按 selection cursor 的小批回填修复；匹配 legacy 行未完成回填时必须返回 preparing，健康读路径不得把 NULL 误算为 unassigned 或从 JSON 表达式读取账号。
