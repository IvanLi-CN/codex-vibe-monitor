...
- 新 terminal 记录必须写结构化 nullable `codex_invocations.upstream_account_id`。旧行仅由 pressure-gated、maintenance coordinator 准入的小批后台回填修复；rolling progress 使用账号和窗口时长，reset-anchored 与 future-reset 窗口必须保留不同 generation。回填与受影响小时 rollup rebuild 必须同事务完成，并有行数上限；超限时保留 cursor 并 deferred，且 rebuild 优先使用结构化账号列。schema-startup readiness marker 使 `window-usage` 只因当前活动窗口内“payload 指定账号但结构化列缺失”的 legacy 行未完成回填而返回 preparing；NULL 的合法 unassigned terminal 不得阻断 readiness。健康统计路径不得将 JSON 归属作为数据输入或 materialize payload；pending marker 下的有界 readiness proof 只允许用 JSON predicate 判定 matching-row existence，并以 durable cursor 约束完成缓存。详情响应在该准备窗口保留 nullable usage 或共享 StoragePlane last-good，绝不启动单账号全窗兼容读。
