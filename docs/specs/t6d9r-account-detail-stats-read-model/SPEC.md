...
- 归属回填每次最多更新 32 行，事务目标为 200ms；仅在受影响小时的 source set 不超过 200 行时同步重算。更密集的小时提交 additive assignment 后保留 coverage hole，owner response 继续 preparing 并由独立 repair 收口，禁止扩大迁移锁窗口。
- 新 terminal 记录必须写结构化 nullable `codex_invocations.upstream_account_id`。旧行仅由 pressure-gated、maintenance coordinator 准入的小批后台回填修复；rolling progress 使用账号和窗口时长，reset-anchored 与 future-reset 窗口必须保留不同 generation。该迁移只填充 additive indexed column：hourly materializer 已以 `COALESCE(structured, attempt/payload)` 归属，因此不得在同一迁移事务重复重建宽小时 rollup。coverage repair 独立为可恢复的有界维护工作，先取得 maintenance admission，再取得 background permit；取消、压力或预算超时必须重新排队。schema-startup readiness marker 使 `window-usage` 只因当前活动窗口内“payload 指定账号但结构化列缺失”的 legacy 行未完成回填而返回 preparing；NULL 的合法 unassigned terminal 不得阻断 readiness。健康统计路径不得将 JSON 归属作为数据输入或 materialize payload；pending marker 下的有界 readiness proof 只允许用 JSON predicate 判定 matching-row existence，completion cache 必须同时绑定精确的账号/窗口/reset selection 与 durable cursor，不能跨 rolling minute 复用。详情响应在该准备窗口保留 nullable usage 或共享 StoragePlane last-good，绝不启动单账号全窗兼容读。
- 无准确 baseline 或 archive coverage hole 时返回 `202 { items: [], readiness: "preparing", retryAfterMs }` 与 `Retry-After: 1`，不得以全窗 archive/raw fallback 伪造成功响应。已有 last-good 最多服务 60 秒，且只可用于已验证相同的账号、窗口与 reset 配置；命名 live coverage hole 同样必须保持 preparing，不得用 last-good 覆盖。并发完成时以更高 durable cursor 判定新鲜度。archive exact fallback 在读取前必须验证 completed manifest SHA-256。
