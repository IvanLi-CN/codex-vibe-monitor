...
- 2026-07-05: capture 转发提速先按“不劣化功能”收口：大 body 读取切到 replay snapshot/file-backed 控制面，并补齐响应首字节/raw writer 耗时证据；请求体上游发送继续使用完整语义后的 replayable path，以保护 encrypted owner、prompt-cache binding、rewrite 和 failover replay。
