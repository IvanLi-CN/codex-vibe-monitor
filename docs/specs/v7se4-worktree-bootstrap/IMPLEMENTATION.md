...
- `format-staged-files.sh --check` 以与三个 formatter 相同的路径过滤从 index 读取已暂存文件，直接检查工作树的同一目标 hunk。installer 只对精确匹配受管模板的 pre-commit 加入该检查，已标记的旧 standard 模板作为迁移前身重装；然后 Lefthook 继续以 `stage_fixed` 暂存 formatter 结果。
