...
- `scripts/worktree-setup.sh` 为 root Bun、web Bun、docs Bun、Cargo 保存 per-worktree digest 状态；仅恢复首次、缺失或失效 surface，Cargo `ok` 状态要求 registry archive 存在，failed digest 自动抑制，自动入口遇锁跳过，手动入口串行重试，`--force` 全量执行。
...
- 两份 smoke 都将锁定版本的 Lefthook 二进制复制到临时 repo 外路径；`scripts/test-worktree-bootstrap.sh` 使用真实 linked worktree 与 fake `bun`/`cargo` 验证主 worktree no-op、首次/重复/选择性恢复、Cargo 缺失恢复、失败抑制、手动重试、force、隔离 advisory 锁、手动串行与 copy-missing-only。
- README 与 AGENTS 已说明按指纹自动恢复、手动失败码、force 和 locked 参数。

## Remaining Gaps

- 无已知实现缺口；Lefthook PATH 前置条件、状态无敏感字段、Cargo presence 与按指纹恢复已由维护文档与 smoke 覆盖。
