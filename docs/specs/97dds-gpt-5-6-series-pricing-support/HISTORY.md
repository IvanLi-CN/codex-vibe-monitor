...
- 2026-08-27: Tightened the shared grouped Dashboard context cluster around the model identity: the model segment is fixed at 20px, reasoning uses one tone-matched marker with 4px sibling spacing, internal vertical separators are removed, and missing reasoning is omitted without changing model or FAST accessibility.
- 2026-08-27: Owner-approved the final Storybook component evidence as one dark-theme and one light-theme desktop capture of the same GPT-5.6 `max` state; mobile remains automated coverage only.
