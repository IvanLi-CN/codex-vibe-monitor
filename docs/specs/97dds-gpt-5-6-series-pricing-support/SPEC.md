...
- Given a WebSocket terminal refresh targets an existing terminal invocation, when the incoming usage omits any previously known dimension, decreases any previously known count, or adds no previously unknown dimension, then the persisted usage, cost, and rollups remain unchanged; a refresh is allowed only when it retains every known dimension, does not decrease any known count, and supplies at least one previously unknown dimension.
