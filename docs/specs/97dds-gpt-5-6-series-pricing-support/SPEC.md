...
- Given a later stream event reports only `input_tokens_details.cache_write_tokens`, when usage is parsed, then the exact cache-write count is updated without erasing previously parsed input, output, cache-read, reasoning, or total Tokens.
