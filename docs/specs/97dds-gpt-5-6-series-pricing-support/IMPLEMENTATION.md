...
- GPT-6 pricing regressions cover actual `default`/`standard` tiers using Standard rates, missing tier handling, request-only tier hints, unsupported actual tiers, and supported Fast pricing; GPT-5.6 tier behavior remains unchanged.
...
- Repair Batch 11 preserves known cost fields when a richer WebSocket usage refresh has no cost evidence, while unsupported actual tiers still replace cost with unknown. The refresh now atomically validates the persisted row against its read snapshot, so legitimate richer values advance while stale snapshots are rejected. Targeted refresh regressions and Rust check/Clippy passed.
