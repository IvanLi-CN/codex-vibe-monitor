...
- Passed on the rebased candidate: `stateful-sqlite` (1,316 tests), `archive-file-io` (274 tests), `cargo fmt --all -- --check`, `cargo check --locked --all-targets --all-features`, and `cargo clippy --locked --all-targets --all-features -- -D warnings`. The independent-pool migration regression also passed in isolation.
- Passed on the rebased candidate: full Web unit suite (1,557 passed, 6 skipped), `bun run typecheck:web`, Web production build, and `bun run lint:web` (85 warnings, no errors).
