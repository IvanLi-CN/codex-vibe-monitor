...
- Passed: Storybook interaction tests for invocation exact-cache-write details and GPT-6 identity; the Settings page interaction story remains manual because its existing page-wide axe audit reports unrelated pre-existing switch-label, heading-order, and pricing-input-label violations.
- Passed: `stateful-sqlite` (1,315 tests), `archive-file-io` (269 tests), `cargo fmt --all -- --check`, `cargo check --locked --all-targets --all-features`, and `cargo clippy --locked --all-targets --all-features -- -D warnings`.
- Passed: full Web unit suite (1,548 passed, 6 skipped), `bun run typecheck:web`, Web production build, and `git diff --check`.
- Passed after the final pricing-input spacing adjustment: Settings unit tests (4 passed), `bun run typecheck:web`, Web production build, and `bun run lint:web` (86 existing warnings, no errors).
- Passed: targeted Storybook interactions for invocation cache-write detail and GPT-6 model identity. The Settings page story remains manual because the existing page-wide axe audit reports unrelated accessibility violations; the complete mock-only Settings page is captured from `ui_demo`.
