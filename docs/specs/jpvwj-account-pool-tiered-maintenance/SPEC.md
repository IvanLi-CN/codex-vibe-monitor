...
- 新增按组、标签或母号维度的分层策略。
- 为 exhausted 账号增加“按 reset 时间精确唤醒”的单独定时器。
...
- Rust tests：schema migration/default fallback、routing settings partial update、tier resolver 排序与分层、到期过滤、异常账号主层保留、健康账号超过 `100` 的次频溢出。
