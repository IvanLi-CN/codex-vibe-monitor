...
- 若当前编辑会话尚未对任何字段产生未提交修改，则同账号的 detail refresh / 手动 refresh / SSE open-resync 仍必须把编辑表单保持为最新详情快照；不得因为进入了 `编辑` 页签就永久停留在第一次打开时的旧值。
- 若账号 A 的保存成功回包晚于详情抽屉关闭并以同一账号 A 重新打开后的新编辑会话，该旧回包不得重播种这次新会话的草稿；只有仍属于当前活跃编辑会话的保存成功响应，才允许刷新草稿为最新已保存值。
- 若用户从账号 A 切换到其它账号后又切回 A，则这次返回后的 A 必须视为新的编辑会话；切走前为 A 发出的旧保存成功回包不得重播种切回后的新草稿。
- 若用户在账号 A 点击 `保存` 后继续修改同一编辑会话中的字段，则较早那次保存成功回包不得覆盖这些后续输入；只有当前草稿仍等于发起保存时的快照，成功响应才允许重播种草稿。
...
- Given 用户打开账号 A 的 `编辑` 页签但尚未修改任何字段，When 同账号的静默 refresh/detail reload/SSE open-resync 返回了更新后的详情，Then 编辑表单必须继续跟随这份最新详情，避免用户在仍是 pristine 的情况下提交过期值。
- Given 账号 A 的保存请求已经发出，When 用户先关闭详情抽屉再重新打开同一个账号 A 并开始新的编辑会话，Then 较早那次保存成功回包不得覆盖这次新会话已经输入的草稿；只有仍匹配当前活跃编辑会话的成功响应才允许重播种草稿。
- Given 账号 A 的保存请求已经发出，When 用户先切到账号 B 再切回账号 A 并开始新的编辑会话，Then 较早那次针对 A 的保存成功回包不得覆盖这次新会话已经输入的草稿；只有仍匹配当前活跃编辑会话的成功响应才允许重播种草稿。
