...
- `invocation_rollup_hourly` 仅作为 overall 调用次数、Token 与成本的完整性证明；`overall`、`model`、`upstream` 明细仍只从 live/可读 archive 调用记录重建。候选必须按日和每小时通过证明后才可替换持久化行。
- 后台刷新立即校验触及日期，并每小时扫描小时保留窗口，包含 canonical 小时表无行但长期表仍有非零残留的日期。检测到差异后只在每轮重建一个队列日期；无法证明完整时保留旧行，若尚无旧行则保留空结果，并在修复队列清空前持续维持既有 `error` 状态，以 `1m/5m/15m/60m` 持久化退避重试。
...
- 后端 SQLite 测试覆盖部分候选不能覆盖完整行、历史日/小时完整性审计与自动修复、源数据不足时保留旧值并暴露 `error`、补齐源数据后的恢复，以及 SQLite `BUSY/LOCKED` 有界重试。
