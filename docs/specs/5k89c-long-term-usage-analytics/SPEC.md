...
- interval state 写入、日期重建替换、保留期删除和兼容清理均按至多 `512` 行的短事务推进。每个事务开始前重新接受低优先级写入准入，并在压力或 shutdown 时停在已提交批次、保留 dirty marker 和 last-good rollup；日期替换在首个 live daily 写入前必须原子切换到持久 last-good snapshot，直到完整新日发布。首次全量物化只由专用 refresher 执行，terminal projection worker 不得绕过该准入路径。不得为了追赶积压延长写锁或推进未完成 cursor。
