...
- 墙时区间的 durable state 每个调用只保存一条 canonical interval，携带模型和上游身份；读取或更新 rollup 时才派生 `overall`、`model`、`upstream` 的日/小时并集，不能将同一区间持久化为六份维度和粒度组合。旧的展开状态可以在兼容迁移后渐进清理，未迁移的日期必须保持 dirty，直到 canonical baseline 已提交。
- interval state 的替换、保留期清理和兼容清理都按至多 `512` 行的短事务推进。每个事务开始前重新接受低优先级写入准入，并在压力或 shutdown 时停在已提交批次、保留 dirty marker 和 last-good rollup；不得为了追赶积压延长写锁或推进 cursor。
- 目标自然日读取对标准的上海 `occurred_at` 文本使用半开范围 seek；RFC3339 与其他历史格式走独立兼容分支并按真实 epoch 校验。跨日调用仍必须进入目标日重建，且兼容分支不得改变 archive replay、Summary 或公开 API 语义。
