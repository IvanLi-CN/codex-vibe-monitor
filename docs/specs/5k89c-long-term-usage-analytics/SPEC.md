...
- interval state 写入、日期重建替换、保留期删除和兼容清理均按至多 `512` 行的短事务推进。每个事务开始前重新接受低优先级写入准入，并在压力或 shutdown 时停在已提交批次、保留 dirty marker 和 last-good rollup；不得为了追赶积压延长写锁或推进未完成 cursor。
