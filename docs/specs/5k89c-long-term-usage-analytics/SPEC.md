...
- `invocation_rollup_hourly` 仅作为 overall 调用次数、Token 与成本的完整性证明；其中全量字段服务于运营汇总，`terminal_count`、`terminal_tokens`、`terminal_cost` 与长期统计相同地排除 `running`/`pending` 调用。每次有界 live/archive 增量写入都必须将 `terminal_proof_complete` 保持为 `0`；只有所有已完成调用 archive 均可读取的完整 live/archive 扫描后，与 canonical 全量字段一致的桶才可标记为 `1`。任一已完成 archive 缺失或不可读时，必须撤销全部 `terminal_proof_complete`，不得以剩余来源认证局部数据；全量对账结果中缺失的既有可信桶同样必须撤销该标记，不能保留陈旧证明。每小时审计即使所有桶当前均已认证，也必须重新核验已完成 archive 的可用性；后续丢失或损坏会撤销证明并通过既有 `error` 状态隐藏统计。证明撤销必须先持久化为 `error`，同一轮后续读取失败也不得恢复为 `ready`；来源可用性错误未消除前，每次后续刷新都必须重试完整对账，不得等待下一小时。旧桶同样必须以完整来源对账后才可取得该标记；无法对账时保持未证明，绝不能把默认零值视为真实空值。`overall`、`model`、`upstream` 明细仍只从 live/可读 archive 调用记录重建。候选必须按日和每小时通过可信终态证明后才可替换持久化行。
- 后台刷新立即校验触及日期，并每小时扫描当前可连续重建的小时保留窗口。归档物理删除前必须扫描原始调用，按最晚实际墙时终点推进独立的完整性来源下界；请求尝试归档还必须复核其账号映射对应的调用来源，任一来源无法读取或匹配时保留归档。删除分两阶段：先在一个 SQLite 事务中将 `cleanup_state` 设为 `delete_pending` 并持久化来源下界，归档仍维持 `completed` 可读；文件删除成功或确认不存在后，才在后续事务删除相关元数据。文件删除或元数据清理失败必须留下 `delete_pending` 供后续重试，不得丢失 manifest。历史遗留的已缺失文件只有在原有 materialization、replay 与保留期门槛均已通过后才能直接收尾其元数据；不得从 manifest 覆盖范围猜测新的来源下界。暂时不可读的调用归档不得以覆盖日期推断时长上界：其开始日及后续候选一律禁止 UPSERT，保留现有行并进入既有 `error`/重试合同。没有任何 canonical 终态指标的日期（包括只含 `running`/`pending` 的桶）是该日应为空的完整性证明，但仅携带零调用、零 Token、零成本的墙时续段不构成残留；含未证明小时桶的日期必须跳过审计和替换。审计必须同时发现任一维度的非零日级或小时级残留，并在验证后原子清空全部维度。检测到差异后只在每轮重建一个队列日期；无法证明完整时保留旧行，若尚无旧行则保留空结果，并在可重建修复队列清空前持续维持既有 `error` 状态，以 `1m/5m/15m/60m` 持久化退避重试。
- 归档删除的阶段化和收尾都必须绑定同一个 `id`、dataset、路径、SHA-256 与 `delete_pending` 身份。收尾在 SQLite `BEGIN IMMEDIATE` 事务中再次验证文件哈希；旧月度归档写入必须在替换文件前撤销待删除状态。因而重写先于收尾时保留新文件和新 manifest，收尾先于重写时写入随后可安全创建新的 manifest。
...
- 后端 SQLite 测试覆盖部分候选不能覆盖完整行、终态证明不受活跃调用污染、有界增量在完整来源对账前保持未证明、来源对账后缺失的可信桶撤销证明、已认证 archive 随后丢失时的小时审计降级、旧 canonical 桶在源数据不完整时保持未证明、历史日/小时完整性审计与自动修复、canonical 零日清空任一非零维度残留、跨午夜墙时续段、不可读归档阻断候选发布、归档删除失败后的持久化重试和元数据事务回滚、源数据不足时保留旧值并以持久化退避持续暴露 `error`、补齐源数据后的恢复，以及 SQLite `BUSY/LOCKED` 有界重试。
