...
- 已实现：Dashboard open-range terminal totals 在 persistence enqueue 成功后同步 fan-out 到共享内存 baseline；`dashboard.activity.current` 的固定 5 秒 deadline 只发布内存 snapshot，不再失效 DB cache。baseline 由初始 build 与最多每 60 秒一次的 reconcile 维护；reconcile 失败保留 last-good totals，live overlay 继续更新。
