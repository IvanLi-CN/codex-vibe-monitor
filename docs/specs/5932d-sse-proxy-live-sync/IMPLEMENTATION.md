...
- Local-patch dashboard timeseries now keep a low-frequency silent authoritative reconcile while the page remains visible, and they skip invocation seed fetches entirely when the authoritative response reports no in-flight buckets. This prevents stale seeded/live deltas from surviving indefinitely in `useTimeseries` local state after a long-lived tab misses a terminal SSE cleanup.
...
- `cd web && bun run test -- src/hooks/useTimeseries.integration.test.tsx src/hooks/useTimeseries.test.ts`
