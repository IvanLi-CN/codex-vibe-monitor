...
- KaisouMail 地址规则是 `localPart@subdomain.rootDomain`，可用 root domain、TTL 边界与地址规则由 `GET /api/meta` 动态提供。
...
- 配置改为 `UPSTREAM_ACCOUNTS_KAISOUMAIL_BASE_URL`、`UPSTREAM_ACCOUNTS_KAISOUMAIL_API_KEY`、`UPSTREAM_ACCOUNTS_KAISOUMAIL_DEFAULT_MAIL_DOMAIN`、`UPSTREAM_ACCOUNTS_KAISOUMAIL_DEFAULT_SUBDOMAIN`，旧 MoeMail env 明确拒绝并提示迁移。
...
- `GET /api/meta` 无需登录即可读取 root domains、TTL 和地址规则；服务端用 `domains[]` 判断 `subdomain.rootDomain` 是否属于可用 root domain。
- `POST /api/mailboxes` 请求包含 `localPart`、`subdomain`、`mailDomain`、`expiresInMinutes`，用于系统生成临时邮箱。
...
- Given 只设置旧 `UPSTREAM_ACCOUNTS_MOEMAIL_*`，When 服务启动配置解析，Then 报错提示改用对应 `UPSTREAM_ACCOUNTS_KAISOUMAIL_*`。
