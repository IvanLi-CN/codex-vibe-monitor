...
- Before opening or advancing the directory iterator, the worker MUST obtain maintenance admission. Admission refusal MUST perform no directory I/O and persist a retry at least five minutes later. For the bounded directory-read slice, it MUST retain the background pressure slot while releasing the SQLite write-coordinator permit, so another background task cannot enter during filesystem traversal while P1 and interactive writes remain eligible.
