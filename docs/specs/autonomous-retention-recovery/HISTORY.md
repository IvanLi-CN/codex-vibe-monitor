...
- The raw-orphan-sweep throughput follow-up replaces full-directory selection and per-file owner-table
  ...
  no schema objects. Directory reads retain the background pressure slot but release the SQLite
  write-coordinator permit; final unlink locking is nonblocking while maintenance admission is held,
  and an item-level lock or unlink failure does not prevent later candidates in the same slice from
  running.
