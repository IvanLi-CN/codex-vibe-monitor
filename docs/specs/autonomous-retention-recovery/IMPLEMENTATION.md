...
- Archive publishers and cleanup finalizers share a directory-level filesystem lock in addition to SQLite admission; mirror cleanup also requires an archive-root ownership proof. Legacy replacement keeps a restore copy until sync and commit succeed, so a failed replacement cannot leave a stale manifest pointing at new bytes.
