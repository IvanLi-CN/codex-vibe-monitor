...
- Codex namespace compatibility is learned per upstream account. A `502` containing `Upstream request failed` after an actual `image_gen` injection marks only that account as unsupported, skips same-account retries, and makes later active Codex rewrites select another compatible account. Automatic candidate selection and header-sticky reuse apply the same final rewrite-aware requirement. A successful request that actually injected or replaced the namespace records `supported`, allowing an operator's explicit supported override to serve as a bounded retest rather than a permanent bypass. This signal does not alter ordinary response, hosted image-tool, or account-health capability axes. Its observed state, reason, and explicit supported/unsupported override are available in account detail so an operator can deliberately retest an upstream after it is repaired.
