...
- Given Records 展开详情已打开且 owner 切到某次尝试的“响应体”面板，When 响应体包含长且不可断行的 SSE/JSON 字段，Then 页面外层宽度保持稳定、不出现新的文档级横向滚动；SSE/NDJSON 列表外层不得横向滚动，额外宽度只允许由单个事件/行卡片内部 scroller 承担。
...
- source_type: storybook_canvas
  ...
  state: compact desktop model-performance tooltip with single-line model identity rows
...
- source_type: storybook_canvas
  ...
  state: compact mobile model-performance drawer with shared identity capsule
  requested_viewport: 390x844
  viewport_strategy: storybook-viewport
  margin_policy: require_margin
  evidence_surface: component
...
- A recognized `keep_original` request records `outcome=no_change` and `reason=already_current` even when it is sent through live-first forwarding; this audit path must not require body decoding or disable a raw compressed/file-backed request snapshot.
