# 请求日志可观测性增强（IP / Cache Tokens / 分阶段耗时 / Session ID）（#z9h7v）
...
- 当前 `/api/invocations` 虽已包含 token 与成本，但缺少请求方来源信息（IP）、会话标识（session id）与易读的阶段耗时展示。
...
- 在不变更 SQLite 表结构的前提下，补齐请求级上下文字段：`requesterIp`、`codexSessionId`、`endpoint`、`failureKind`。
...
- session id 提取优先级固定：请求体候选 JSON 指针 > 请求头候选键。
- `build_proxy_payload_summary` 在成功/失败路径都包含 `requesterIp` 与 `codexSessionId`（缺失时为 null）。
- `/api/invocations` 返回新增字段：`requesterIp`、`codexSessionId`、`endpoint`、`failureKind`。
- 前端主表新增 `Cache Tokens` 与 `Latency` 列（`First byte / Total`），详情区展示完整阶段耗时。

### SHOULD

- 历史/非代理记录缺字段时前端统一展示 `—`，不抛错。
- 保持向后兼容，不影响现有 SSE、统计接口与设置页。
...
- 请求进入 `/v1/chat/completions` 或 `/v1/responses` 采集路径时，后端提取 IP 与 session id，并随 payload 一并落库。
...
- 若 session id 候选键全部未命中，返回 `null` 并在前端显示 `—`。
- 若阶段耗时缺失（旧记录），前端逐项显示 `—`。
...
- Given 请求携带 `x-forwarded-for` 与 `metadata.codex_session_id`，When 请求完成并查询 `/api/invocations`，Then 返回 `requesterIp`、`codexSessionId`、`cacheInputTokens` 与阶段耗时字段。
- Given 请求无转发头且 body 无 session id，When 请求完成，Then 前端详情对应字段显示 `—` 且页面无错误。
...
- [x] M2: 后端采集与接口输出增强（IP/session/payload 投影）。
- [x] M3: 前端表格与文案升级（主表 + 详情）。
- [x] M4: 回归验证通过并完成本地提交。

## 风险 / 开放问题 / 假设（Risks, Open Questions, Assumptions）

- 风险：上游/客户端 session id 字段命名不统一，可能出现无法命中的正常缺失。
- 开放问题：是否将 session id 同步写入独立列（本次不做）。
