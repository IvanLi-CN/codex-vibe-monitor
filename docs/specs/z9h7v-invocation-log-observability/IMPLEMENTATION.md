...
- 账号详情已将最终调用记录表替换为真实上游调用记录列表：按账号从 `pool_upstream_request_attempts` 读取最近 7 天主库数据，按 `occurred_at DESC, id DESC` 分页。每次重试都有独立记录；首屏显示模型、endpoint、状态、重试、代理、三段延迟、tokens 与费用，低频诊断字段按需展开。列表以 `(invoke_id, occurred_at)` 关联最终 invocation，模型按 `response_model ?? request_model ?? model` 选择。
