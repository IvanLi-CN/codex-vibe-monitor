...
- API key/OAuth 请求与 WebSocket 上游建立会在 pending attempt 已写入、但仍等待上游 headers 或 session frame 时发布一次 snapshot；后续终态继续沿用既有发布路径，保证长连接请求不会只在结束后才出现在账号请求列表。
...
- `ModelPerformanceTrigger` 在桌面通过可点击、可聚焦的 Tooltip 展示总计及按累计时长排序的模型行，在窄屏通过详情抽屉展示无横向滚动的指标网格；入口仍挂在总览与账号区域，但展示的是完整范围性能明细而非实时 1 分钟值。owner-facing 明细显式拆成 `墙钟时长 / 累计时长 / 并行数` 三列，并补充说明“跨模型重叠时模型行墙钟和可能大于总计”。桌面浮窗自适应至 `72rem`；详细模式命中 GPT-5.6 专用身份方案时模型行只显示图标/思考程度分组胶囊，不重复显示模型名称，移动抽屉复用同一语义；简单模式显示完整模型名。模型性能与用量明细现在复用全局简单/详细模式；后端 `modelGroups` 为模型性能简单模式提供精确聚合，两个逻辑浮窗分别持久化排序规则，桌面表头与移动工具栏均可操作。
- Dashboard 分组排序状态仅保存在前端：模式键跨模型性能/用量明细浮窗共享，排序键分别按 `model-performance` 与 `usage-breakdown` 隔离；排序入口改为表头直接点击，不使用原生下拉；详细模型排序固定使用名称/思考程度四规则，简单模式屏蔽思考程度选项并保留其详细模式记忆。用量简单分组在前端按模型精确合并 Token 与成本桶，仅缓存命中率与总计表头可排序，缓存写入/读取和输出保持纯展示，排序主值为 Token；桌面表格保持固定列宽，一般手机视口改用纵向指标列表，避免任何横向滚动条。
