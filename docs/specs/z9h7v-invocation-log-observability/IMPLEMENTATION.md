...
- 运行态号池快照在请求算法确定后同步携带当前 attempt 的 `requestCompressionAlgorithm`，因此 SSE、Live 与 Dashboard 不必等待终态写入才显示该值；`budget_exhausted_final` 是合成终态，不参与 live 或 archive 的最终请求压缩选择，真实最终 attempt 的空值仍保持空值而不回退到更早重试。
