...
- Account window usage now has a bounded StoragePlane: terminal rows carry a nullable structured account id, rollup and exact boundary reads are coalesced per selection, and uncovered archive buckets report preparation instead of triggering a full-range fallback.
...
- Activity baselines record the sequence of durable queued terminals they already include, terminal batches index accounts once and derive model and latency responses once per affected aggregate rather than once per delta, and open summary bases capture SQLite and terminal watermarks under the writer barrier.
- Moving activity and summary bases now rebase in the producer-owned runtime reconcile when an activity calendar anchor changes or either topic's rolling duration crosses the established `60s` boundary, including when no terminal arrives. Activity keeps a rebase-only range anchor so frequent terminal slices cannot reset that boundary while advancing the public range. Terminal delivery only isolates a stale base and remains on the zero-SQL typed path; inactive stale bases stay dirty until ownership returns, a failed rebase stays isolated for the next reconcile, and a byte-identical replacement only swaps typed state without advancing an SSE cursor.
