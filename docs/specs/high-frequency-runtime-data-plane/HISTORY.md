...
- 2026-08-16: `/v1/responses` 的 capture path 从完整 body 后再发送升级为可回放 live-first；以路由元数据完整性、关闭默认值、稳定对照和直接首响应测量约束该优化，避免速度收益以错误选路或失真统计换取。
- `/v1/responses` live-first 灰度不再按账号组筛选；设置面只保留启用开关和实验组占比，避免空账号组配置造成静默不生效。
