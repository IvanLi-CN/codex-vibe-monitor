...
- Activity baselines record the sequence of durable queued terminals they already include, terminal batches derive model and latency responses once per affected aggregate rather than once per delta, and open summary bases capture SQLite and terminal watermarks under the writer barrier.
