...
- Issue #759 将 network timeseries 与 network recent 收敛到借用式 typed materializer：timeseries 只替换 live point，recent 直接引用 network slice，避免完整 cached topic 深拷贝。两个 topic 由同一个 `1s` network projection cadence 驱动，移除订阅任务的独立 recent 定时器；相同 revision 不会重建 frame、序列化或推进 cursor。
