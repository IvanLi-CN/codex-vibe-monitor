...
`GET /api/stats/summary` 使用启动 hydration 的 `SummaryProjection` 作为唯一 HTTP 真值。projection 保留按账号和 UTC bucket 的 rollup/archive coverage、usage/model 与延迟聚合、bounded recent-N 索引、48 小时 live exact tail、受支持 rolling/calendar 窗口的归档边界行、runtime overlay 和 maintenance last-good；完整历史由 compact hourly rollup 与 account coverage 派生，归档在后台流式归并后释放。归档账号发现和 legacy archive 的实测 coverage 都缓存于不可变 revision，并受 bounded distinct budget 保护；超限或不可安全发现时保留上一份 projection，不把不完整集合发布为成功 revision。recent hydration 只按时间顺序读取 bounded prefix；若溢出前缀无法证明某账号的 requested-N，则在 HTTP 内存路径返回既有 unavailable 而不伪造缩短结果。公开 rolling duration 限制为 30 天；48 小时以内允许任意分钟/小时粒度，超过 48 小时仅接受整日粒度，其他输入在 handler 参数验证阶段返回既有 400，不进入 hub、projection 或 SQLite。请求仅规范化选择并在内存中派生精确 `StatsResponse`。48 条 LRU 只缓存已序列化响应，不能限制受支持选择。typed runtime/persistence events 进入合并触发器；后台维护器以单飞、250ms debounce、10 秒最小间隔和 4 秒 deadline 发布新 revision，并在 15 秒 freshness 预算内保留 exact last-good；`all` 聚合维护独立成功刷新年龄，普通窗口刷新不会延长它。System Status 在 60 秒缓存上限前启动并以 4 秒 deadline 限制其后台 durable refresh。请求不执行 SQLite 或文件 I/O，刷新失败超过预算采用端点 unavailable 契约。
