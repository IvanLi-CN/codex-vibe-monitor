...
Dashboard full-topology contract now opens two real `topic_sse_stream` connections for `dashboard.activity.current`, `stats.summary.current`, `dashboard.network-timeseries.window`, and `dashboard.network-recent.current`, then exercises the production broadcast listener, active-subscriber producer and shared network-recent cadence registration. The deterministic baseline records one current-slice build and revision with zero cadence misses, no independent network or terminal slice work, one business-payload handoff per affected topic, JSON overlays for activity and summary, and separate network-topic materialization and serialization. It also verifies zero live-path database reads and that a second owner receives the same serialized live frame by `Arc` identity without increasing per-revision serialization.
