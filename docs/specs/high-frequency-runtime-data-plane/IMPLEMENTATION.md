...
`GET /api/stats/summary` 使用后台 hydration 发布的 `SummaryProjection` 作为唯一 HTTP 真值。projection 保留按账号和 UTC bucket 的 rollup/archive coverage、usage/model 与延迟聚合、bounded recent-N 索引、48 小时 live exact tail、受支持 rolling/calendar 窗口的归档边界行、runtime overlay 和 maintenance last-good；完整历史由 compact hourly rollup 与 account coverage 派生，归档在后台流式归并后释放。归档账号发现和 legacy archive 的实测 coverage 都缓存于不可变 revision，并受 bounded distinct budget 保护；全量聚合只有在 archive materialization、每小时 global/account rollup key、对应 scope 的 completed replay marker 与 archive account manifest 共同证明时才发布，legacy 无边界 manifest 的 global baseline 仅保留既有 materialization 证明。normal 与 paged manifest admission 都要求 `upstream_activity_manifest_refreshed_at` 证明账号 ID 列表已完成；仅观察到一组账号 ID 不能让遗漏的 archive-only account 以新鲜零值通过。completed replay marker 绑定当前 manifest SHA，因而已 replay 的 usage breakdown 不会触发读侧 archive fallback，也不会让替换后的 archive 继承旧 proof。缺少任一 account manifest、scope key 或 replay marker 时，该 account 聚合保留 last-good 或返回既有 unavailable，绝不借用 global aggregate。materialized archive 绝不再作为 raw delta 加回 compact baseline；缺少其 durable scope proof 只会阻止不完整 revision 发布。超限或不可安全发现同样不发布不完整 revision。任何 archive raw-fallback bucket 同时加入 persisted-live replacement hydration，因此 archive/live ID exclusion 始终保留丰富的 live copy，而不会使缺 scope rollup 的 bucket 漏计；exact bucket/range 在 4096 个小时封顶，超过上限会 fail closed 而非逐小时展开。recent hydration 只按时间顺序读取 bounded prefix；若溢出前缀无法证明某账号的 requested-N，则在 HTTP 内存路径返回既有 unavailable 而不伪造缩短结果。公开 rolling duration 限制为 30 天；48 小时以内允许任意分钟/小时粒度，超过 48 小时仅接受整日粒度，其他输入在 handler 参数验证阶段返回既有 400，不进入 hub、projection 或 SQLite。请求仅规范化选择并在内存中派生精确 `StatsResponse`，不依赖 URL-keyed serialized-response cache；per-key snapshot compatibility state 不能限制受支持选择。typed runtime/persistence events 进入合并触发器；后台维护器以单飞、250ms memory-only 调度相位、mutation 专用 250ms debounce、10 秒最小间隔和 4 秒 runtime deadline 发布新 revision，并在 15 秒 freshness 预算内保留 exact last-good。HTTP listener 建立后立即发布 readiness；首次 Summary hydration 在 listener 之后以独立的 30 秒有限 deadline 运行，以允许有界历史基线完成，System Status 仍使用 4 秒 deadline；两者均支持 shutdown cancellation 与有界压力退避。尚无快照时端点保持既有 unavailable，失败刷新保留 fresh last-good，绝不让 HTTP 请求执行 SQLite 或文件 I/O。`all` 聚合维护独立成功刷新年龄，普通窗口刷新不会延长它。System Status 在每轮 60 秒缓存上限前启动并以相同提前周期及 4 秒 deadline 限制后台 durable refresh，raw-metrics 覆盖态会立即原子投影进已有内存响应。
