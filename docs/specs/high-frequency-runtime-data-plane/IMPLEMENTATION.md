...
- Activity terminal bases retain the aggregate state not present in the response wire shape. Each terminal slice updates total stats, model performance, account latency and the bounded recent-invocation projection before one shared frame is serialized; this requires neither SQLite reads nor complete invocation broadcasts.
