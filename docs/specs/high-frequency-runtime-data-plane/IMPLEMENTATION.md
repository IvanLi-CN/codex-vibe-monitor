...
The account window handler delegates to `src/upstream_accounts/storage_plane.rs`; it no longer constructs a cross-account raw query itself. The plane coalesces identical selections, records rollup/boundary telemetry, and returns a cold `202 preparing` response when an archive coverage hole cannot be answered without an unbounded scan. Legacy assignment backfill updates at most 32 rows per transaction and exactly rematerializes only an affected hour whose source set has at most 200 rows; denser hours remain explicit coverage holes for independent repair.

Terminal persistence writes `upstream_account_id` into `codex_invocations`, and schema ensure creates partial indexes for structured account reads and bounded legacy attribution probes. A persistent readiness marker distinguishes only rows whose payload names an account but whose structured column is absent; legitimate unassigned terminals remain ready. New structured databases are ready immediately, while existing legacy data is repaired in pressure-gated, coordinator-admitted batches of at most 32 rows and 200ms. Owner preflight never materializes payload; concurrent requests for the same normalized account selection share its summary, cursor and legacy-readiness reads. Its durable cursor is scoped to those accounts, so an unrelated terminal does not invalidate a healthy selection. When the marker is pending, preflight uses a bounded matching-row proof and caches completion only while that scoped cursor is unchanged. The completion cache is capped at 128 entries. Rolling backfill progress is keyed by account plus window duration; reset-anchored and future-reset windows retain distinct generations, so a rolling minute does not restart cursor work while a reset change never skips older rows. Attribution backfill changes only the additive indexed column: hourly materialization already resolves `COALESCE(structured, attempt/payload)` and therefore does not need a duplicate wide rebuild in the same transaction. Coverage repair remains independent, is admitted to the maintenance writer before it takes a background pressure permit, keeps its dequeued batch recoverable through cancellation, and has a two-second execution budget. A matching in-window legacy row keeps the batch window response in `preparing` until its cursor pass completes, preventing a successful response from omitting it. The account usage builder reads minute/hour rollups by account/range coverage, reserves any partial hour for exact rows, and uses half-open bucket boundaries with an id-bounded, 5,000-row live tail. A full-hour coverage hole excludes partial minute rollups for that hour before folding its exact bucket, preventing a raw fallback from double-counting a partial materialization. Legacy archive rows recover their structured account id from payload when that additive archive column does not exist. Archive exact fallback and bootstrap account-rollup rebuild verify the completed manifest SHA-256 before opening an archive file, preserving existing rollups when the source is missing or mismatched; archive replay markers carry the same hash and an old marker without it remains uncovered. A live cursor does not certify a missing time bucket as zero; delayed terminals remain eligible through the bucket-specific exact tail without also being replayed by the broad cursor tail. Detail reads retain nullable usage or the shared StoragePlane result during preparation and never use a full-window compatibility fallback. The web hook retries preparation at 1s, 2s, then 5s, caps a server hint at five seconds, coalesces all preparing account ids into the next retry, and actively aborts outstanding window-usage requests when its roster selection changes or unmounts. Cursor participates in build coalescing, while compatible last-good selects the highest durable cursor for the same account/window configuration for at most 60 seconds; an incomplete preflight returns preparing instead of an account-only cache match. The 128-entry coordinator evicts only a non-flight LRU entry; it rejects a new selection only when every retained entry is in flight. Abandoned leaders complete their selection as preparing so coalesced waiters cannot remain blocked.
