...
- The `legacy` kill switch keeps the pre-existing `DashboardActivityLive`, JSON-overlay, and network-recent subscription cadence paths intact. Terminal totals continue through the exact Records refresh path.
