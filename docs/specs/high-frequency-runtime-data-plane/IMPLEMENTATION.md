...
Terminal persistence writes `upstream_account_id` into `codex_invocations`, and schema ensure creates a partial `(upstream_account_id, occurred_at, id)` index. Existing rows are repaired in a pressure-gated batch of at most 200 rows with selection-scoped durable `startup_backfill_progress` cursors. A matching unassigned legacy row keeps the window response in `preparing` until its cursor pass completes, preventing a successful response from omitting it. The account usage builder reads minute/hour rollups, excludes minute-covered rows from exact fallback, and uses half-open bucket boundaries with an id-bounded, 5,000-row live tail. A larger tail is explicitly preparing rather than a widened raw read. The web hook retries preparation at 1s, 2s, then 5s and cancels the retry when its roster selection changes or unmounts. The 128-entry coordinator rejects a new selection only when every retained entry is in flight, preserving the hard bound without evicting an active singleflight.
