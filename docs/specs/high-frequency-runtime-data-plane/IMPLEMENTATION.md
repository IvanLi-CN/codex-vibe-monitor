...
Dashboard full-topology contract opens two real `topic_sse_stream` connections for `dashboard.activity.current`, `stats.summary.current`, `dashboard.network-timeseries.window`, and `dashboard.network-recent.current`, then exercises the production broadcast listener, active-subscriber producer and shared network-recent cadence registration. The deterministic contract records one current-slice and one network-slice build and revision with zero cadence misses, no complete business-snapshot handoff, no JSON overlay, zero live-path database reads, and one typed materialization and serialization per affected topic. A second owner receives the same serialized live frame by `Arc` identity without increasing per-revision serialization.

Runtime projection delivery now maintains independent current/phase, network/rate and terminal-total dirty generations with non-extending `250ms`, `1s` and `5s` deadlines. Auto mode broadcasts typed current, network and terminal slice messages only; cached Dashboard activity, summary, network-timeseries and network-recent bases are materialized in place and serialized directly into shared frames. Network-only changes retain the current revision, while terminal totals refresh only from the terminal slice. `legacy` keeps the original complete `DashboardActivityLive` delivery path as the operational rollback contract. Unit coverage verifies deadline independence, non-extension, terminal revision suppression and current-slice immunity to network-only changes.
