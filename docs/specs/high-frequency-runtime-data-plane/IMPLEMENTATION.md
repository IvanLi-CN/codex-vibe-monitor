...
Terminal persistence writes `upstream_account_id` into `codex_invocations`, and schema ensure creates a partial `(upstream_account_id, occurred_at, id)` index. Existing rows are repaired in a pressure-gated batch of at most 200 rows with selection-scoped durable `startup_backfill_progress` cursors. The progress identity contains the exact account/window ranges, so reset movement or a larger active window revisits newly relevant legacy rows instead of skipping them behind an old cursor. Each changed row triggers an in-transaction rebuild of only its affected hourly buckets, and the structured column takes precedence over payload fallback during that rebuild. A matching in-window legacy row keeps the batch window response in `preparing` until its cursor pass completes, preventing a successful response from omitting it. The account usage builder reads minute/hour rollups by account/range coverage, reserves any partial hour for exact rows, and uses half-open bucket boundaries with an id-bounded, 5,000-row live tail. A live cursor does not certify a missing time bucket as zero; delayed terminal rows remain eligible through the bucket-specific exact tail without also being replayed by the broad cursor tail. Existing detail responses retain their former exact single-account compatibility read only while the batch StoragePlane is preparing. The web hook retries preparation at 1s, 2s, then 5s and actively aborts outstanding window-usage requests when its roster selection changes or unmounts. Cursor participates in build coalescing, but last-good reuse is restricted to the same account/window configuration for at most 60 seconds. The 128-entry coordinator evicts only a non-flight LRU entry; it rejects a new selection only when every retained entry is in flight. Abandoned leaders complete their selection as preparing so coalesced waiters cannot remain blocked.
