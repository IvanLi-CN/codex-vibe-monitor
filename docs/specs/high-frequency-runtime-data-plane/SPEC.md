...
- New terminal writes populate nullable `codex_invocations.upstream_account_id`; legacy rows are filled by a pressure-gated, selection-scoped cursor-ordered backfill. Probe and repair only inspect rows inside the requesting selection's active window. A selection with matching in-window legacy rows remains `preparing` until its bounded pass has assigned them, so `NULL` rows cannot be silently omitted from a successful window response.
