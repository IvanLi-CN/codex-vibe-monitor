...
- **Application Panel:** `surface-panel` 是页面级主容器，负责与背景分离，不应再嵌套等价的强边框 card。
- **Card Surface:** `surface-card` 是普通 `Card` 默认外观，使用克制边框、轻阴影和主题 tonal background。
- **Subtle Surface:** `surface-subtle` 用于 card 内部的列表项、状态行、chip 组和表单二级块。
- **Inset Surface:** `surface-inset` 用于设置页、配置组和需要表现“内部区块”的低对比容器。
- **Field Surface:** `field-surface` 是 Input、Select 等可编辑控件的默认底色和边框入口。
- **Menu Surface:** `menu-surface` 是 Select / menu / popover list 这类浮层列表的默认表面入口。
- **Dialog Chrome:** `dialog-chrome-surface` 只用于 dialog / drawer 的 sticky header、footer 或确认操作栏，不能滥用到普通内容区。
...
**The Tokenized-Surface Rule.** 新增普通 card、内部块、输入框、菜单和 dialog chrome 时优先使用共享 surface class，而不是在页面内重新组合 `border-base-300/*`、`bg-base-100/*` 或高透明白底。页面私有 surface 只有在表达业务状态、图表语义或已经有 feature spec 约束时才允许存在。
...
- **Background:** 普通内容优先使用 `surface-card` / `surface-subtle` / `surface-inset` / `surface-panel`；不要在页面里直接复制 `bg-base-100/*` 透明面板。
- **Shadow Strategy:** 静态 card 轻阴影或无阴影；浮层、drawer、dialog 才使用强阴影。
- **Border:** 默认边框来自共享 surface token；状态面板可用语义色低透明边框，但必须低饱和且不压过正文。
- **Internal Padding:** 密集区从 `p-3` 到 `p-4`；普通 panel 用 `p-5`；大型设置区可用 `p-6`。

### Inputs / Fields

- **Style:** 输入框使用 `h-10`、`rounded-lg` 和 `field-surface`，由共享 token 控制背景与边框。
...
- **Do** 区分 filled-content 与 tone-ink：实底 CTA / icon-disc 用 `*-content`，低透明 chip / badge / marker / summary metric 用 tone-ink。
...
- **Don't** 在 `bg-warning/12`、`bg-accent/15`、`bg-info/10` 这类低透明语义底上继续使用 `text-warning-content`、`text-accent-content` 或其它 `*-content` token。
