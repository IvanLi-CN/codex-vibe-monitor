...
- `OPENAI_PROXY_IMAGE_HANDSHAKE_TIMEOUT_SECS`：`/v1/images/generations` 与 `/v1/images/edits` 等待上游首字节的默认超时，默认 `300` 秒；运行时可由四层 timeout 策略覆盖
- `OPENAI_PROXY_REQUEST_READ_TIMEOUT_SECS`：请求体读取总超时
- `OPENAI_PROXY_ENCRYPTED_SESSION_OWNER_ROUTING_ENABLED`：加密对话路由绑定的首次初始化默认值；默认关闭，只有数据库里该设置还没初始化时才会吃 env
...
- `VITE_APP_RUNTIME`：应用运行时，值只能是 `live` 或 `demo`；`demo` 只使用浏览器内存与 MSW mock，不回退到真实 API
- `VITE_DEPLOY_BASE`：静态资源与 MSW worker 的部署基路径；GitHub Pages demo 使用 `${DOCS_BASE}demo/`
- `VITE_APP_PORT`：Vite app/demo 的监听端口；demo 默认 `60083`，实际开发应使用 worktree 租约端口
...
- App dev：`60080`
- Web Demo dev：`60083`（开发时使用租约端口覆盖）
