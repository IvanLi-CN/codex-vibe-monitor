...
- 开发与验收面：公开 docs-site 负责使用说明，Web Demo 负责 mock-only 全路由预览，Storybook 负责页面与组件证据。
...
2. 想体验完整产品路由但不连接服务，打开 [Web Demo](/demo/)。
3. 想看界面与组件证据，再去 [Storybook](/storybook.html)。
...
7. [Web Demo](/demo/)：公开、mock-only 的全产品路由预览入口。
8. [Storybook](/storybook.html)：页面与组件证据入口。

## 文档分工

- `docs-site/docs/` 负责 public docs：项目介绍、自部署入口、开发入口、Web Demo 和 Storybook 导航。
