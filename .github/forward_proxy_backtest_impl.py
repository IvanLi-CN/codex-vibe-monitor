...
def fallback_profile(context: SimulationContext, key: str) -> ProxyProfile:
    return ProxyProfile(key, 0, 0.65, context.success_latencies, context.failure_latencies, ["handshake_timeout"])


def profile_for(context: SimulationContext, key: str) -> ProxyProfile:
    return context.profiles.get(key) or fallback_profile(context, key)


def simulate_seed(context: SimulationContext, seed: int, requests: int) -> Dict[str, object]:
    rng = random.Random(seed)
    runtime_seed = {key: state for key, state in (context.seed_runtime or {}).items() if key in context.allowed_keys}
    machine = ForwardProxyStateMachine(context.algo, context.proxy_keys, context.insert_direct, runtime_seed or None)
    ...
    for _ in range(requests):
        ...
        selections[key] += 1
        profile = profile_for(context, key)
        success = rng.random() < profile.success_rate
        fallback = context.success_latencies if success else context.failure_latencies
        latency = sample_latency(profile, success=success, rng=rng, fallback=fallback)
        if success:
            success_count += 1
        else:
            failure = rng.choice(profile.failure_kinds) if profile.failure_kinds else "handshake_timeout"
            failures[failure] += 1
        if latency is not None:
        ...
        if probe_key is not None:
            probe_profile = profile_for(context, probe_key)
            probe_success = rng.random() < probe_profile.success_rate
            probe_fallback = context.success_latencies if probe_success else context.failure_latencies
            probe_latency = sample_latency(probe_profile, success=probe_success, rng=rng, fallback=probe_fallback)
            machine.record_attempt(probe_key, probe_success, probe_latency, is_probe=True)
            machine.finish_probe(now_secs)
    top1 = max(selections.values()) if selections else 0
    return {
        "seed": seed,
        "success_rate": success_count / requests,
        "p95_latency_ms": percentile(latencies, 95.0),
        "top1_share": top1 / requests,
        "top1_proxy_key": selections.most_common(1)[0][0] if selections else "",
        "failure_kind_distribution": dict(failures),
    }


def run_simulation(algo: AlgoConfig, profiles: Dict[str, ProxyProfile], seeds: List[int], requests: int, insert_direct: bool, seed_runtime: Optional[Dict[str, RuntimeState]]) -> Dict[str, object]:
    proxy_keys = sorted(key for key, profile in profiles.items() if profile.observed_attempts > 0 and (insert_direct or key != DIRECT_KEY))
    if not proxy_keys:
        proxy_keys = sorted(key for key in profiles if insert_direct or key != DIRECT_KEY)
    if insert_direct and DIRECT_KEY not in proxy_keys:
        proxy_keys.append(DIRECT_KEY)
    context = SimulationContext(
        algo, profiles, proxy_keys, set(proxy_keys),
        [latency for profile in profiles.values() for latency in profile.success_latencies if latency is not None],
        [latency for profile in profiles.values() for latency in profile.failure_latencies if latency is not None],
        insert_direct, seed_runtime,
    )
    per_seed = [simulate_seed(context, seed, requests) for seed in seeds]
    ...
    return {"seeds": per_seed, "success_rate_mean": statistics.mean(success_values) if success_values else 0.0, "p95_latency_mean": statistics.mean(p95_values) if p95_values else None, "top1_share_mean": statistics.mean(top1_values) if top1_values else 0.0}
...
def load_backtest_data(db_path: Path, security_checks: Dict[str, bool]) -> tuple[List[AttemptRow], List[str], Dict[str, RuntimeState], bool]:
    conn, readonly_uri_mode = open_readonly_connection(db_path)
    try:
        security_checks["db_readonly_uri_mode"] = readonly_uri_mode
        return load_attempts(conn), load_runtime_keys(conn), load_runtime_states(conn), load_insert_direct(conn)
    finally:
        conn.close()


def evaluate_algorithms(args: argparse.Namespace, algos: List[AlgoConfig], seeds: List[int], attempts: List[AttemptRow], runtime_keys: List[str], runtime_states: Dict[str, RuntimeState], insert_direct: bool) -> tuple[Dict[str, object], Dict[str, object]]:
    ...
    profiles = build_profiles(attempts, sorted(set(runtime_keys) | set(attempt_proxy_keys)))
    results: list[Dict[str, object]] = []
    for name in ("v1", "v2"):
        algo = algo_by_name[name]
        results.append({"trace_replay": trace_replay(attempts, algo, trace_proxy_keys, insert_direct=insert_direct), "simulation": run_simulation(algo, profiles, seeds, args.requests, insert_direct, runtime_states)})
    return results[0], results[1]


def write_backtest_result(output_prefix: Path, result: Dict[str, object], redact: bool) -> None:
    ...
    write_markdown_report(output_md, result, redact=redact)
...
def main() -> int:
    ...
    attempts, runtime_keys, runtime_states, insert_direct = load_backtest_data(db_path, security_checks)
    if not attempts:
        raise SystemExit("forward_proxy_attempts is empty; cannot run backtest")
    baseline, candidate = evaluate_algorithms(args, algos, seeds, attempts, runtime_keys, runtime_states, insert_direct)
    ...
    if any(item.startswith("security_check_failed:") for item in acceptance["failed_rules"]):
        print("acceptance_pass=False\nfailed_rules=")
        print("\n".join(f"- {item}" for item in acceptance["failed_rules"]))
        return 1
    result = {"generated_at": dt.datetime.now(dt.timezone.utc).isoformat(), "db_path": str(db_path), "config": {"algos": [algo.name for algo in algos], "seeds": seeds, "requests": args.requests, "redaction_enabled": not args.no_redact}, "security_checks": security_checks, "baseline": baseline, "candidate": candidate, "acceptance": acceptance}
    write_backtest_result(output_prefix, result, redact=not args.no_redact)
