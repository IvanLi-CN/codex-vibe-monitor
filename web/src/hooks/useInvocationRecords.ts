...
export interface UseInvocationRecordsResult {
  ...
  updateDraft: <K extends keyof InvocationRecordsDraftFilters>(
    key: K,
...
interface SearchState {
  filters: Omit<
    InvocationRecordsQuery,
    "page" | "pageSize" | "sortBy" | "sortOrder" | "snapshotId"
  >;
  snapshotId: number;
  generation: number;
}
...
export function useInvocationRecords(): UseInvocationRecordsResult {
  ...
  const loadRecordsPage = useCallback(
    async (
      nextPage: number,
      nextPageSize: number,
      nextSortBy: InvocationSortBy,
      nextSortOrder: InvocationSortOrder,
    ) => {
      ...
      try {
        ...
        const { response: visibleResponse, preservedKeys } = mergeTransientInFlightRecords(
          response,
          recordsRef.current,
          {
            filters,
  ...
  useInvocationRecordsRealtime({
    ...
    onRecordsChange: (next, meta) => {
      ...
      const matchingVisibleInsertKeys = currentLiveMergeState.matchingVisibleInsertKeys.filter(
        (key) => nextVisibleKeySet.has(key),
      );
      ...
      for (const record of meta.payload.records) {
        ...
        if (
          (currentVisibleKeySet.has(key) || countedFilteredOutBaselineKeys.includes(key)) &&
  ...
  const api = useMemo(
    ...
    [
      ...
      isSearching,
      isSummaryLoading,
      liveMergeState.matchingVisibleInsertKeys.length,
      page,
      pageSize,
      records,
      recordsError,
