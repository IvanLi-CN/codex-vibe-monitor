...
export function useExternalApiKeys() {
  ...
  const [revealedSecret, setRevealedSecret] = useState<RevealedExternalApiKeySecret | null>(null);
  ...
  const rotateKey = useCallback(async (id: number) => {
    ...
    try {
      ...
      setItems((current) =>
        sortExternalApiKeys([...current.filter((item) => item.id !== id), response.key]),
      );
      setRevealedSecret({
        action: "rotate",
        key: response.key,
  ...
  const disableKey = useCallback(async (id: number) => {
    ...
    try {
      ...
      setItems((current) => current.map((item) => (item.id === id ? response.key : item)));
      setError(null);
      return response;
    } catch (err) {
      const message = err instanceof Error ? err.message : String(err);
