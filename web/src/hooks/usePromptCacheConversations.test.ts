...
describe("usePromptCacheConversations sync guards", () => {
  it("returns zero delay when refresh window is already open", () => {
    const delay = getPromptCacheSseRefreshDelay(
      10_000,
      10_000 + PROMPT_CACHE_SSE_REFRESH_THROTTLE_MS,
    );
    expect(delay).toBe(0);
  });
  ...
  it("throttles open resync inside cooldown window", () => {
    const allowed = shouldTriggerPromptCacheOpenResync(
      30_000,
      30_000 + PROMPT_CACHE_OPEN_RESYNC_COOLDOWN_MS - 1,
    );
