  ...
  fetchParallelWorkStats:
    vi.fn<
      (options?: {
        ...
        timeZone?: string;
        signal?: AbortSignal;
      }) => Promise<ParallelWorkStatsResponse>
    >(),
}));
...
function createStats(): ParallelWorkStatsResponse {
  const stats: ParallelWorkStatsResponse = {
...
function Probe({
  ...
  const { data, isLoading, error } = useParallelWorkStats(range, bucket);
  return (
    <div>
      <div data-testid="loading">{isLoading ? "true" : "false"}</div>
      <div data-testid="error">{error ?? ""}</div>
      <div data-testid="minute-count">
        {String(data?.minute7d.points[0]?.parallelCount ?? 0)}
      </div>
    </div>
  );
}
...
describe("useParallelWorkStats helpers", () => {
  ...
  it("enforces the open-resync cooldown unless forced", () => {
    ...
    expect(
      shouldTriggerParallelWorkOpenResync(
        0,
        PARALLEL_WORK_OPEN_RESYNC_COOLDOWN_MS + 1,
      ),
    ).toBe(true);
  ...
  it("only retries transient request failures", () => {
    expect(
      shouldRetryParallelWorkError(
        new ApiRequestError(400, "Request failed: 400"),
      ),
    ).toBe(false);
    expect(
      shouldRetryParallelWorkError(
        new ApiRequestError(429, "Request failed: 429"),
      ),
    ).toBe(true);
    expect(
      shouldRetryParallelWorkError(
        new ApiRequestError(503, "Request failed: 503"),
      ),
    ).toBe(true);
...
describe("useParallelWorkStats", () => {
  it("refetches with the latest range and bucket when filters change", async () => {
    apiMocks.fetchParallelWorkStats.mockResolvedValue(createStats());

    render(<Probe range="today" bucket="15m" />);
    await flushAsync();

    act(() => {
      root?.render(<Probe range="7d" bucket="1h" />);
    });
    await flushAsync();

    expect(apiMocks.fetchParallelWorkStats).toHaveBeenNthCalledWith(
      1,
      expect.objectContaining({ range: "today", bucket: "15m" }),
    );
    expect(apiMocks.fetchParallelWorkStats).toHaveBeenNthCalledWith(
      2,
      expect.objectContaining({ range: "7d", bucket: "1h" }),
    );
  });
  ...
  it("throttles records-triggered silent refreshes to at most once per minute", async () => {
    ...
    expect(
      host?.querySelector('[data-testid="minute-count"]')?.textContent,
    ).toBe("1");
  ...
  it("queues SSE-open refreshes instead of aborting an in-flight request", async () => {
    let resolveRefresh: ((value: ParallelWorkStatsResponse) => void) | null =
      null;
  ...
  it("queues a follow-up silent refresh when SSE-open arrives during hydration", async () => {
    let resolveInitialLoad:
      | ((value: ParallelWorkStatsResponse) => void)
      | null = null;
    apiMocks.fetchParallelWorkStats
      .mockImplementationOnce(
        () =>
  ...
  it("keeps auto-retrying transient server failures", async () => {
    ...
    apiMocks.fetchParallelWorkStats
      .mockRejectedValueOnce(
        new ApiRequestError(503, "Request failed: 503 gateway timeout"),
      )
      .mockResolvedValueOnce(createStats());
