...
describe("useForwardProxyLiveStats sync guards", () => {
  it("returns zero delay when SSE refresh window is already open", () => {
    const delay = getForwardProxySseRefreshDelay(
      10_000,
      10_000 + FORWARD_PROXY_SSE_REFRESH_THROTTLE_MS,
    );
    expect(delay).toBe(0);
  });
  ...
  it("throttles reconnect backfill in cooldown window", () => {
    const allowed = shouldTriggerForwardProxyOpenResync(
      30_000,
      30_000 + FORWARD_PROXY_OPEN_RESYNC_COOLDOWN_MS - 1,
    );
