...
describe("resolveStatsBucketOptions", () => {
  it("returns the default range buckets when the backend has no extra limits", () => {
    expect(
      resolveStatsBucketOptions("1mo").map((option) => option.value),
    ).toEqual(["6h", "12h", "1d"]);
  });
  ...
  it("falls back to the daily option when backend support no longer matches the stale UI bucket list", () => {
    expect(
      resolveStatsBucketOptions("thisWeek", ["1d"]).map(
        (option) => option.value,
      ),
    ).toEqual(["1d"]);
  });
...
describe("BUCKET_OPTION_KEYS", () => {
  it("offers 5-minute buckets for ranges up to one day", () => {
    expect(BUCKET_OPTION_KEYS.today.map((option) => option.value)).toContain(
      "5m",
    );
    expect(BUCKET_OPTION_KEYS["1d"].map((option) => option.value)).toContain(
      "5m",
    );
  });
