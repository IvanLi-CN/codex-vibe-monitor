...
export default function StatsPage() {
  ...
  const [range, setRange] =
    useState<(typeof RANGE_OPTIONS)[number]["value"]>("today");
  ...
  const effectiveBucket = useMemo(() => {
    ...
    return resolveStatsBucketValue(
      serverBucket ?? requestedBucket,
      rawBucketOptions,
    );
  }, [rawBucketOptions, requestedBucket, timeseries?.effectiveBucket]);
  const bucketOptions = useMemo(
    () =>
      rawBucketOptions.map((option) => ({
        ...option,
  ...
  const selectedBucketLabel = useMemo(
    () =>
      bucketOptions.find((option) => option.value === effectiveBucket)?.label,
    [bucketOptions, effectiveBucket],
  );
  ...
  } = useParallelWorkStats(range, effectiveBucket);
        ...
        <div className="surface-panel-body gap-4">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div className="section-heading">
              ...
              {failureSummaryError ? (
                <p className="section-description text-error">
                  {failureSummaryError}
                </p>
              ) : (
          ...
          <div className="metric-grid w-full grid-cols-1 sm:grid-cols-4">
            <div className="metric-cell">
              <div className="metric-label">
                {t("stats.errors.summary.service")}
              </div>
              <div className="metric-value text-error text-2xl">
                {failureSummaryLoading
                  ? "—"
                  : (failureSummary?.serviceFailureCount ?? 0)}
              </div>
            </div>
            <div className="metric-cell">
              <div className="metric-label">
                {t("stats.errors.summary.client")}
              </div>
              <div className="metric-value text-warning text-2xl">
                {failureSummaryLoading
                  ? "—"
                  : (failureSummary?.clientFailureCount ?? 0)}
              </div>
            </div>
            <div className="metric-cell">
              <div className="metric-label">
                {t("stats.errors.summary.abort")}
              </div>
              <div className="metric-value text-info text-2xl">
                {failureSummaryLoading
                  ? "—"
                  : (failureSummary?.clientAbortCount ?? 0)}
              </div>
            </div>
            <div className="metric-cell">
              <div className="metric-label">
                {t("stats.errors.summary.actionable")}
              </div>
              <div className="metric-value text-secondary text-2xl">
                {failureSummaryLoading
                  ? "—"
                  : (failureSummary?.actionableFailureCount ?? 0)}
              </div>
            </div>
          </div>
          {errorsError ? (
