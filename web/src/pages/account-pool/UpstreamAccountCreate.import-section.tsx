// biome-ignore-all lint/suspicious/noExplicitAny: import row payloads are normalized at the controller boundary
  ...
  return (
    <>
      <label className="field md:col-span-2">
        <span className="field-label">{t(`${importKeyPrefix}.fileInputLabel`)}</span>
        <Input
          key={importInputKey}
          type="file"
      ...
      <label className="field md:col-span-2">
        <span className="field-label">{t(`${importKeyPrefix}.paste.label`)}</span>
        <textarea
          className={cn(
            ...
            importPasteError ? "border-error/70 focus-visible:ring-error" : "",
          )}
          ...
          placeholder={t(`${importKeyPrefix}.paste.placeholder`)}
          autoCapitalize="none"
          spellCheck={false}
        ...
        {importPasteError ? (
          ...
          <p className="mt-2 inline-flex items-center gap-2 text-sm text-base-content/65">
            ...
            {t(`${importKeyPrefix}.paste.validating`)}
          </p>
        ) : (
          <p className="mt-2 text-sm text-base-content/65">{t(`${importKeyPrefix}.paste.hint`)}</p>
        )}
        <div className="mt-3 flex flex-wrap gap-2">
          <Button
            type="button"
            variant="secondary"
            onClick={() => void handleValidateImportedOauthPasteDraft()}
            disabled={!writesEnabled || importPasteBusy || importPasteDraft.trim().length === 0}
          >
          ...
          <Button
            ...
            disabled={importPasteBusy || importPasteDraft.length === 0}
          >
            {t("accountPool.upstreamAccounts.import.paste.clearDraft")}
          </Button>
        </div>
      </label>
      <div className="md:col-span-2 rounded-2xl border border-base-300/80 bg-base-200/35 p-4">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <p className="text-sm font-semibold text-base-content">
              {t(`${importKeyPrefix}.selectedFilesTitle`)}
            </p>
            <p className="mt-1 text-sm text-base-content/65">
              {importSelectionLabel ?? t(`${importKeyPrefix}.selectedFilesEmpty`)}
            </p>
          </div>
          <Button
            type="button"
        ...
        {importSelectionFeedback?.messages?.length ? (
          <Alert variant={importSelectionFeedback.variant} className="mt-3">
      ...
      <label className="field md:col-span-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            type="button"
            size="icon"
            variant={hasGroupSettings(importGroupName) ? "secondary" : "outline"}
        ...
        {importGroupProxyState.error ? (
          <p className="mt-2 text-xs text-error">{importGroupProxyState.error}</p>
        ) : null}
      ...
      <div className="md:col-span-2 flex flex-wrap justify-end gap-2">
        ...
        <Button
          ...
          disabled={
            !writesEnabled || importFiles.length === 0 || Boolean(importGroupProxyState.error)
          }
        >
