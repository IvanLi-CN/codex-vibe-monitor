          ...
          <table className="w-full table-fixed text-sm">
            ...
            <tbody>
              {batchRows.map((row: any, index: number) => {
                const rowGroupProxyError = resolveRequiredGroupProxyState(row.groupName).error;
                ...
                const needsIdentityConfirmation = status === "needs_identity_confirmation";
                const isBusy = row.busyAction != null;
                const isMailboxBusy = row.mailboxBusyAction != null;
                const metadataLocked =
                  !writesEnabled || isBusy || isMailboxBusy || row.metadataBusy;
                const oauthLocked =
                  !writesEnabled ||
                  isBusy ||
                  isMailboxBusy ||
                ...
                const rowHasActiveOauthUrl = isActivePendingOauthSession(row.session);
                  ...
                  >
                    <td className="px-3 py-4">
                      <Tooltip
                        content={buildActionTooltip(
                          rowInvited
                            ? t("accountPool.upstreamAccounts.batchOauth.tooltip.invitedTitle")
                            : t("accountPool.upstreamAccounts.batchOauth.tooltip.notInvitedTitle"),
                          rowInvited
                            ? t("accountPool.upstreamAccounts.batchOauth.tooltip.invitedBody")
                            : t("accountPool.upstreamAccounts.batchOauth.tooltip.notInvitedBody"),
                        )}
                      >
                        <button
                          type="button"
                        ...
                        <div className="field min-w-0 gap-2 whitespace-nowrap">
                              ...
                              <OauthMailboxChip
                                ...
                                editor={{
                                  ...
                                  onDraftValueChange: (value) =>
                                    handleBatchMailboxEditorValueChange(row.id, value),
                          ...
                          <div className="relative">
                            <Input
                              ...
                              className="min-w-0"
                              onChange={(event) =>
                                handleBatchMetadataChange(row.id, "displayName", event.target.value)
                              }
                              onBlur={() =>
                                handleBatchCompletedTextFieldBlur(row.id, "displayName")
                              }
                              onKeyDown={handleBatchCompletedTextFieldKeyDown}
                            />
                            {duplicateNameError ? (
                        ...
                        <label className="field min-w-0 gap-2 whitespace-nowrap">
                          ...
                          <div className="flex min-w-0 items-center gap-2">
                            <UpstreamAccountGroupCombobox
                              ...
                              onValueChange={(value) => handleBatchGroupValueChange(row.id, value)}
                              disabled={metadataLocked}
                            ...
                            <Button
                              type="button"
                              size="icon"
                              variant={hasGroupSettings(row.groupName) ? "secondary" : "outline"}
                          ...
                          {rowGroupProxyError ? (
                            <p className="text-xs text-error">{rowGroupProxyError}</p>
                          ) : null}
                        </label>
                        {row.noteExpanded ? (
                          <label className="field min-w-0 gap-2 whitespace-nowrap">
                            ...
                            <Input
                              ...
                              className="min-w-0"
                              onChange={(event) =>
                                handleBatchMetadataChange(row.id, "note", event.target.value)
                              }
                              onBlur={() => handleBatchCompletedTextFieldBlur(row.id, "note")}
                              onKeyDown={handleBatchCompletedTextFieldKeyDown}
                            />
                          </label>
                        ) : null}
                    ...
                    <td className="px-3 py-4">
                      <div className="grid gap-3">
                        <label className="field min-w-0 gap-2 whitespace-nowrap">
                          <span className="field-label">
                            {t("accountPool.upstreamAccounts.oauth.callbackUrlLabel")}
                          </span>
                          <Input
                            ...
                            className="min-w-0"
                            onChange={(event) =>
                              handleBatchMetadataChange(row.id, "callbackUrl", event.target.value)
                            }
                          />
                        </label>
                        <div className="flex items-center gap-3">
                          <div className="flex flex-wrap items-center gap-2">
                            <BatchOauthActionButton
                              mode={rowHasActiveOauthUrl ? "copy" : "generate"}
                              primaryAriaLabel={
                              ...
                              expiresAtLabel={batchOauthSessionExpiresAtLabel(row.session, t)}
                              manualCopyTitle={t(
                              ...
                              manualCopyValue={batchManualCopyRowId === row.id ? authUrl : null}
                              busy={row.busyAction === "generate"}
                              disabled={
                                rowHasActiveOauthUrl
                                  ? !authUrl || oauthLocked
                                  : oauthLocked || Boolean(rowGroupProxyError)
                              }
                              regenerateDisabled={oauthLocked || Boolean(rowGroupProxyError)}
                              onPrimaryAction={() => {
                                if (rowHasActiveOauthUrl) {
                                  void handleBatchCopyOauthUrl(row.id);
                                  return;
                                }
                                void handleBatchGenerateOauthUrl(row.id);
                              }}
                              onRegenerate={() => void handleBatchGenerateOauthUrl(row.id)}
                              onManualCopyOpenChange={(nextOpen) => {
                            ...
                            {row.mailboxSession ? (
                              <Tooltip
                                content={buildActionTooltip(
                                  ...
                                  row.mailboxStatus?.latestCode?.value ??
                                    t("accountPool.upstreamAccounts.batchOauth.codeMissing"),
                                )}
                              >
                                <Button
                                  type="button"
                            ...
                            >
                              <MotherAccountToggle
                                ...
                                onToggle={() => handleBatchMotherToggle(row.id)}
                              />
                            </Tooltip>
                          ...
                          <div className="ml-auto flex shrink-0 items-center gap-2">
                            ...
                            >
                              <Button
                                ...
                                disabled={oauthLocked || isCompleted}
                              >
                            ...
                            <div className="space-y-3">
                              ...
                              <div className="flex flex-wrap gap-2">
                                <Button
                                  ...
                                  onClick={() =>
                                    void handleResolveBatchOauthEmailChoice(row.id, "verified")
                                  }
                                >
                                ...
                                <Button
                                  ...
                                  disabled={row.metadataBusy}
                                  onClick={() =>
                                    void handleResolveBatchOauthEmailChoice(row.id, "entered")
                                  }
                                >
                        ...
                        {needsIdentityConfirmation && row.session?.identityConfirmation ? (
                          <OAuthIdentityConfirmationAlert
                            identityConfirmation={row.session.identityConfirmation}
                            fallbackDisplayName={row.displayName}
                            confirmBusy={row.busyAction === "confirm"}
                            confirmDisabled={!writesEnabled || isBusy}
                            onConfirm={() => void handleBatchConfirmOauthIdentityOverwrite(row.id)}
                            t={t}
                            compact
                          />
                        ) : null}
                        ...
                        >
                          {statusDetail ??
                            t("accountPool.upstreamAccounts.batchOauth.statusDetail.draft")}
                        </p>
                      </div>
                    </td>
                  </tr>
