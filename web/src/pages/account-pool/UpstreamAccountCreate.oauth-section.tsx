    ...
    <>
            ...
            <Input
              ...
              onChange={(event) => {
                ...
                setOauthDisplayName((current: string) =>
                  resolveDisplayNameAfterEmailChange(current, oauthEmail, nextValue),
                );
                setActionError(null);
                invalidateRelinkPendingOauthSessionForMailboxChange(nextValue);
                if (
                  oauthMailboxSession &&
              ...
              disabled={
                !writesEnabled || oauthMailboxBusyAction != null || session?.status === "completed"
              }
            />
      ...
      <label className="field">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            type="button"
            size="icon"
            variant={hasGroupSettings(oauthGroupName) ? "secondary" : "outline"}
        ...
        {oauthGroupProxyState.error ? (
          <p className="mt-2 text-xs text-error">{oauthGroupProxyState.error}</p>
        ) : null}
      </label>
      <MotherAccountToggle
      ...
      {oauthNeedsIdentityConfirmation && oauthIdentityConfirmation ? (
        <OAuthIdentityConfirmationAlert
          ...
          confirmDisabled={busyAction === "oauth-confirm-identity" || !writesEnabled}
