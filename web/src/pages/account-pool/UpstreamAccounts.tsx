          ...
          <div className="drawer-header px-5 py-4 sm:px-6">
            <div className="flex items-start justify-between gap-4">
              <div className="min-w-0 space-y-1">
                ...
                <h2
                  id="upstream-account-detail-title"
                  className="truncate text-xl font-semibold text-base-content"
                >
                  {title}
                </h2>
              </div>
      ...
      >
        ...
        <div className="space-y-4 px-6 py-6">
          <label className="field">
            <span className="text-sm font-semibold uppercase tracking-[0.14em] text-base-content/82">
              {apiKeyLabel}
            </span>
            <Input
              ref={inputRef}
...
export default function UpstreamAccountsPage() {
  ...
  const [stickyConversationLimit, setStickyConversationLimit] =
    useState<number>(50);
  ...
  const metrics = useMemo(() => {
    const oauthCount = items.filter(
      (item) => item.kind === "oauth_codex",
    ).length;
    const apiKeyCount = items.filter(
      (item) => item.kind === "api_key_codex",
    ).length;
    const needsReauthCount = items.filter(
      (item) => item.status === "needs_reauth",
    ).length;
    const syncingCount = items.filter(
  ...
  const availableGroups = useMemo(() => {
    ...
    return {
      names: Array.from(values).sort((left, right) =>
  ...
  const filteredItems = useMemo(() => {
    ...
    const allLabel = t(
      "accountPool.upstreamAccounts.groupFilter.all",
    ).toLocaleLowerCase();
    const ungroupedLabel = t(
      "accountPool.upstreamAccounts.groupFilter.ungrouped",
    ).toLocaleLowerCase();
  ...
  const {
    ...
    selectedId,
    stickyConversationLimit,
    Boolean(selectedId && isDetailDrawerOpen),
  );
  ...
  return (
    <div className="grid gap-6">
      ...
      <AccountDetailDrawer
        ...
        title={
          selected?.displayName ?? t("accountPool.upstreamAccounts.detailTitle")
        }
        ...
        {!selected ? (
          <div className="flex min-h-[20rem] flex-col items-center justify-center rounded-[1.6rem] border border-dashed border-base-300/80 bg-base-100/45 px-6 text-center">
            ...
            <h3 className="text-lg font-semibold">
              {t("accountPool.upstreamAccounts.detailEmptyTitle")}
            </h3>
            ...
            <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
              <div className="space-y-3">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {selected.planType ? (
                    <Badge variant="secondary">{selected.planType}</Badge>
                  ) : null}
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                {selected.kind === "oauth_codex" ? (
                  <Button
                    ...
                    disabled={busyAction === "relogin" || !writesEnabled}
                  >
                    {busyAction === "relogin" ? (
                      <Spinner size="sm" className="mr-2" />
                    ) : (
                      ...
                      <p className="text-sm text-warning/90">
                        {t(
                          "accountPool.upstreamAccounts.duplicate.warningBody",
                          {
                            ...
                            peers:
                              detail.duplicateInfo.peerAccountIds.join(", "),
                          },
                        )}
                      </p>
                    </div>
                  ...
                  <CardHeader className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                    ...
                    <label className="field w-36">
                      ...
                      <select
                        ...
                        onChange={(event) =>
                          setStickyConversationLimit(Number(event.target.value))
                        }
                      >
                ...
                <Card className="border-base-300/80 bg-base-100/72">
                  ...
                  <CardContent className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                    ...
                    <DetailField
                      ...
                      value={formatDateTime(detail.lastRefreshedAt)}
                    />
                    <DetailField
                      label={t(
                        "accountPool.upstreamAccounts.fields.tokenExpiresAt",
                      )}
                      value={formatDateTime(detail.tokenExpiresAt)}
                    />
                    <DetailField
                    ...
                    <div className="md:col-span-2 xl:col-span-4 rounded-[1.2rem] border border-base-300/80 bg-base-100/75 p-4">
                      <p className="metric-label">
                        {t("accountPool.upstreamAccounts.fields.lastError")}
                      </p>
                      ...
                      <p className="mt-2 text-xs text-base-content/55">
                        {formatDateTime(detail.lastErrorAt)}
                      </p>
                    </div>
                  </CardContent>
                </Card>
