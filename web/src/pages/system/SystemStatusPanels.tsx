...
export function MetricCell({ title, value, hint, tone = "default", badge }: MetricCellProps) {
  const toneClass =
    tone === "primary"
      ? "text-primary"
      : tone === "secondary"
...
export function BreakdownRow({ label, value, hint }: BreakdownRowProps) {
  return (
...
export function OverviewPanel({ status, t }: OverviewPanelProps) {
  ...
  return (
    <section className="surface-panel overflow-hidden" data-testid="system-status-overview">
      <div className="surface-panel-body gap-5">
        ...
        <OverviewPanelDiskSummary status={status} projectDiskBytes={projectDiskBytes} t={t} />
...
function OverviewPanelDiskSummary({
  status,
  projectDiskBytes,
  t,
}: OverviewPanelProps & { projectDiskBytes: number }) {
  const breakdown = [
    ["rawPayloadBytes", status.rawBodies.bytes, "rawPayloadBytesHint"],
    ["archiveBytes", status.archivedBodies.bytes, "archiveBytesHint"],
    ["databaseBytes", status.databaseBytes, "databaseBytesHint"],
    ["otherFilesBytes", status.otherFilesBytes, "otherFilesBytesHint"],
  ] as const;
  return (
    <>
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {breakdown.map(([name, bytes, hint]) => (
          <BreakdownRow
            key={name}
            label={t(`system.status.breakdown.${name}`)}
            value={formatBytes(bytes)}
            hint={t(`system.status.breakdown.${hint}`)}
          />
        ))}
      </div>
    </>
...
export function MetricSection({ title, description, metrics, testId }: MetricSectionProps) {
  return (
...
export function ProjectionHealthSection({ status, t }: OverviewPanelProps) {
...
export function RuntimePressureHealthSection({ status, t }: OverviewPanelProps) {
  ...
  const hotTopicRows = hotTopics
    ? (
      ...
      ).map((name) => [name, hotTopics[name]] as const)
    : [];

  return (
    <RuntimePressureHealthContent
      health={health}
      state={state}
      hotTopics={hotTopics}
      hotTopicRows={hotTopicRows}
      slices={slices}
      delivery={delivery}
      eventBus={eventBus}
      backfill={backfill}
...
type RuntimePressureHealth = NonNullable<SystemStatusResponse["runtimePressureHealth"]>;
type RuntimePressureTopicName =
  | "activity"
  | "summary"
  | "networkTimeseries"
  | "networkRecent"
...
function RuntimePressureHealthContent({
  health,
  state,
  hotTopics,
  hotTopicRows,
  ...
  health: RuntimePressureHealth | null | undefined;
  state: string;
  hotTopics: RuntimePressureHealth["dashboardHotTopics"] | undefined;
  hotTopicRows: Array<
    readonly [
      RuntimePressureTopicName,
      NonNullable<RuntimePressureHealth["dashboardHotTopics"]>[RuntimePressureTopicName],
    ]
  >;
  slices: RuntimePressureHealth["dashboardProjection"]["sliceCounters"] | undefined;
  delivery: {
  ...
  eventBus: RuntimePressureHealth["eventBus"];
  backfill: RuntimePressureHealth["backfill"];
  retention: RuntimePressureHealth["retentionWriteHealth"];
    ...
    >
      <div className="surface-panel-body gap-4">
        ...
        <Alert variant={alertVariant}>
          {t("system.status.runtimePressure.summary", {
            state: t(`system.status.runtimePressure.states.${state}`),
          })}
        </Alert>
        <RuntimePressureHotTopics hotTopics={hotTopics} rows={hotTopicRows} t={t} />
        <RuntimePressureDetails
          health={health}
          slices={slices}
          delivery={delivery}
          eventBus={eventBus}
...
function RuntimePressureHotTopics({
  hotTopics,
  rows,
  t,
}: {
...
function RuntimePressureDetails({
  health,
  slices,
  delivery,
  eventBus,
  ...
  return (
    <details className="rounded-lg border border-base-300/70 bg-base-100/50 px-4 py-3">
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <RuntimePressureResourceRows health={health} slices={slices} delivery={delivery} t={t} />
        <RuntimePressureCoordinationRows
          health={health}
          eventBus={eventBus}
          backfill={backfill}
          retention={retention}
...
function RuntimePressureResourceRows({
  health,
  slices,
  delivery,
  t,
}: {
  health: RuntimePressureHealth;
  slices: RuntimePressureHealth["dashboardProjection"]["sliceCounters"] | undefined;
  delivery: {
    serializationCount: number;
    frameBytesCount: number;
  ...
  return (
    <>
      ...
      <BreakdownRow
        ...
        hint={
          health.dashboardProjection.lastDeferReason ?? health.dashboardProjection.degradedReason
        }
      />
      <BreakdownRow
...
function RuntimePressureCoordinationRows({
  health,
  eventBus,
  backfill,
  retention,
  ...
  return (
    <>
      <BreakdownRow
        ...
        value={t(`system.status.runtimePressure.states.${eventBus?.state ?? "unknown"}`)}
        hint={
          eventBus
      ...
      <BreakdownRow
        ...
        value={t(`system.status.runtimePressure.states.${backfill?.state ?? "unknown"}`)}
        hint={
          backfill
      ...
      />
      <RuntimePressureWriterRows health={health} retention={retention} t={t} />
    </>
  );
}

function RuntimePressureWriterRows({
  health,
  retention,
  t,
}: {
  ...
  const coordinator = health.proxySqliteWriteCoordinator;
  return (
    <>
      <BreakdownRow
        ...
        value={
          coordinator
            ? `${coordinator.p1WaiterCount.toLocaleString()} / ${coordinator.interactiveWaiterCount.toLocaleString()} / ${coordinator.p2WaiterCount.toLocaleString()} / ${coordinator.maintenanceWaiterCount.toLocaleString()}`
            : t("system.status.runtimePressure.states.unknown")
        }
