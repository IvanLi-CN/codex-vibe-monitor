...
function AccountDetailSkeleton() {
  return (
    <div className="grid gap-4">
      {["card-1", "card-2", "card-3"].map((cardKey) => (
        <div key={cardKey} className="h-28 animate-pulse rounded-[1.35rem] bg-base-200/75" />
      ))}
    </div>
  );
}
            ...
            <div className="flex flex-wrap items-center gap-2">
              <Button asChild variant="outline">
                <Link
                  ...
                  state={{ selectedAccountId: detail.id, openDetail: true }}
                >
    ...
    >
      {isLoading ? (
        ...
        <div className="grid gap-4">
          ...
          {accountId != null ? (
            <Button asChild variant="outline" className="w-fit">
              <Link
                to="/account-pool/upstream-accounts"
                state={{ selectedAccountId: accountId, openDetail: true }}
              >
                <AppIcon name="arrow-right-bold" className="mr-2 h-4 w-4" aria-hidden />
                {t("table.accountDrawer.openAccountPool")}
              </Link>
        ...
        <div className="grid gap-5">
            ...
            >
              ...
              <div className="metric-grid">
                ...
                <DetailField
                  ...
                  value={detail.email ?? detail.maskedApiKey ?? ""}
                />
                {detail.kind === "oauth_codex" ? (
                  <>
                    <DetailField
                      label={t("accountPool.upstreamAccounts.fields.accountId")}
                      value={detail.chatgptAccountId ?? ""}
                    />
                    <DetailField
          ...
          {detailTab === "health" ? (
            <div id={tabIds.health.panel} role="tabpanel" aria-labelledby={tabIds.health.tab}>
