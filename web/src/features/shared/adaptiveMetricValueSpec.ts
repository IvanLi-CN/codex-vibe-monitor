...
export function buildAdaptiveMetricSpec(
  ...
  const standardPrecisions =
    kind === "currency" && currencyProfile === "rate"
      ? [2, 1, 0]
      : standardPrecisionCandidates(kind);
  ...
  for (const [index, precision] of standardPrecisions.entries()) {
    const minimumFractionDigits = kind === "currency" && currencyProfile === "rate" ? precision : 0;
    candidates.push({
...
export function buildAdaptiveDurationTextSpec(valueMs: number | null, localeTag: string) {
  ...
  const maximumFractionDigits = Math.abs(seconds) >= 100 ? 1 : Math.abs(seconds) >= 1 ? 2 : 3;
