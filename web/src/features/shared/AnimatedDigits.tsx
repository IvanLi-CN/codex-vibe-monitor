...
export function AnimatedDigits({
  ...
  let digitIndex = 0;
  const characters = text.split("").map((character, position) => ({
    character,
  ...
  return (
    ...
    >
      {characters.map(({ character: ch, key }) => {
        if (/\d/.test(ch)) {
...
function DigitRoll({
  ...
  return (
    ...
    >
      ...
      >
        {path
          .map((digit, position) => ({ digit, key: `digit-${position}` }))
          .map((item) => (
            <span key={item.key} style={{ height: "1em" }}>
              {item.digit}
            </span>
          ))}
      </span>
