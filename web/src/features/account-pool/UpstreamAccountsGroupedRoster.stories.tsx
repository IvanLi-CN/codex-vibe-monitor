...
function makeItem(
  ...
  return {
    ...
    compactSupport:
      id % 8 === 0 ? { status: "unsupported", reason: "Compact channel unavailable" } : null,
    duplicateInfo:
      id % 11 === 0 ? { peerAccountIds: [id + 100], reasons: ["sharedChatgptAccountId"] } : null,
...
const baseGroups = [
  buildGroup(
    ...
    [
      ...
      makeItem(3, {
        displayName: "Fallback Key - Seoul",
        kind: "api_key_codex",
  ...
  buildGroup(
    ...
    {
      concurrencyLimit: 2,
      note: "EMEA production roster with mixed OAuth and API key coverage.",
      boundProxyLabels: ["DE Transit 02"],
    },
  ),
];
...
function expectGridBadgeParity(canvasElement: HTMLElement) {
  ...
  return Promise.all([
    ...
    expect(
      canvas.getByText("Tokyo subscription edge with a deliberately long display name"),
    ).toBeInTheDocument(),
    ...
    expect(
      canvasElement.querySelectorAll('[data-testid="upstream-accounts-group-grid-card-badges"]'),
    ).toHaveLength(4);
  });
}
    ...
    groups: [
      buildGroup(
        ...
        [
          ...
          makeItem(22, {
            groupName: null,
            displayName: "Ungrouped API Key 02",
            kind: "api_key_codex",
            currentForwardProxyState: "unconfigured",
            currentForwardProxyKey: null,
            currentForwardProxyDisplayName: null,
      ...
      return buildGroup(
        ...
        Array.from({ length: 4 + (groupIndex % 5) }, (_, itemIndex) =>
          makeItem(groupIndex * 20 + itemIndex + 100, {
            ...
            groupName: groupId,
            planType: itemIndex % 4 === 0 ? "team" : itemIndex % 3 === 0 ? "free" : "pro",
            currentForwardProxyDisplayName: itemIndex % 3 === 0 ? "JP Edge 01" : "SG Transit 03",
            currentForwardProxyState:
              itemIndex % 6 === 0 ? "pending" : itemIndex % 7 === 0 ? "unconfigured" : "assigned",
            currentForwardProxyKey:
              itemIndex % 7 === 0 ? null : itemIndex % 3 === 0 ? "jp-edge-01" : "sg-transit-03",
          }),
        ),
        {
