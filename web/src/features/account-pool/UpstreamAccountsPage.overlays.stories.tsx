...
export const DetailDrawerEventLocatesAttempt: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await waitFor(() =>
      expect(within(dialog).getByTestId("account-attempt-record-4V7MYPJG")).toHaveTextContent(
        /关键诊断|key diagnostics/i,
      ),
    );
    await expect(
      within(within(dialog).getByTestId("account-attempt-record-4V7MYPJG")).getAllByText(
        "upstream-story-500",
      ).length,
    ).toBeGreaterThan(0);
  },
...
export const DetailDrawerApiKeyEventImpact: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await expect(impacts[0]).toHaveTextContent(/影响范围.*模型|impact scope.*model/i);
    await expect(impacts[0]).toHaveTextContent(
      /受影响模型.*gpt-5\.6-terra|affected models.*gpt-5\.6-terra/i,
    );
    await expect(impacts[0]).not.toHaveTextContent(/全部|all|其他模型|other models/i);
...
export const RoutingDialogRequestCompression: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await expect(dialog).toHaveTextContent(/系统设置/i);
  },
};
