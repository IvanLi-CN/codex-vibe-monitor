...
import { type FormControlSize, formControlSizeVariants } from "../../components/ui/form-control";
...
export function AccountTagFilterCombobox({
  ...
  const prioritizedTagIdSet = useMemo(() => new Set(prioritizedTagIds ?? []), [prioritizedTagIds]);
  const disabledTagIdSet = useMemo(() => new Set(disabledTagIds ?? []), [disabledTagIds]);
  ...
  const triggerTitle =
    selectedTags.length > 0 ? selectedTags.map((tag) => tag.name).join(", ") : undefined;
      ...
      >
        <PopoverTrigger asChild>
          ...
          >
            ...
            <AppIcon
              name="chevron-down"
              className="ml-2 h-4 w-4 shrink-0 text-base-content/45"
              aria-hidden
            />
          </Button>
        </PopoverTrigger>
        <PopoverContent align="start" className="w-[var(--radix-popover-trigger-width)] p-0">
          <Command shouldFilter={false}>
            <CommandInput value={query} placeholder={searchPlaceholder} onValueChange={setQuery} />
            <CommandList>
                ...
                <>
                  <CommandGroup>
                    <CommandItem onSelect={() => onValueChange([])}>
                      <AppIcon
                        name="close"
                        className="mr-2 h-4 w-4 text-base-content/55"
                        aria-hidden
                      />
