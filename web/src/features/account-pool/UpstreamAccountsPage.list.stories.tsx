...
function triggerAutomatedRosterAction(action: AutomatedRosterAction, attempt = 0): number | null {
  ...
  const filterTrigger = Array.from(root.querySelectorAll('button[role="combobox"]')).find(
    (candidate) =>
      /工作状态|work status/i.test(
...
function AutomatedRosterStoryRouter({ action }: { action: AutomatedRosterAction }) {
...
function findDynamicRosterLayoutTab(root: ParentNode, layout: DynamicRosterLayout) {
...
function DynamicRosterStoryRouter({ layout }: { layout: DynamicRosterLayout }) {
...
async function choosePageSize(canvasElement: HTMLElement, pageSize: number) {
  await chooseSelectOption(canvasElement, /每页|page size/i, new RegExp(`^${pageSize}$`, "i"));
}
...
export const MaintenanceEvents: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step("shows the execution records table with the new two-line layout", async () => {
      ...
      await expect(await canvasScope.findByText(/Fixture Billing Team/i)).toBeInTheDocument();
      await expect(await canvasScope.findByText(/deferred|success/i)).toBeInTheDocument();
    });
  },
};
...
export const CompactLongLabels: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    await step("keeps unsupported compact badges aligned and hides supported markers", async () => {
      ...
      const apiKeyRow = await findAccountRow(canvasElement, /Team key - staging/i);
...
export const MissingWindowPlaceholders: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    await step(
      "render missing secondary windows with weak dash placeholders instead of 0%",
      async () => {
        const row = await findAccountRow(canvasElement, /Team key - missing weekly limit/i);
...
export const StatusFilters: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await choosePageSize(canvasElement, 50);
    await chooseCommandOptions(canvasElement, /工作状态|work status/i, [
      /限流|rate limited/i,
      /工作|working/i,
    ]);
    await chooseCommandOptions(canvasElement, /启用状态|enable status/i, [/启用|enabled/i]);
    await chooseCommandOptions(canvasElement, /账号状态|account health/i, [/正常|normal/i]);
    await expect(await canvas.findByText(/Team key - staging/i)).toBeInTheDocument();
    await expect(await canvas.findByText(/Codex Pro - Tokyo/i)).toBeInTheDocument();
  },
};
...
export const UnavailableWorkStatusFilter: Story = {
  ...
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    await choosePageSize(canvasElement, 50);
    await chooseCommandOptions(canvasElement, /工作状态|work status/i, [/不可用|unavailable/i]);

    await expect(
      await canvas.findByText(/Needs reauth unavailable work status/i),
    ).toBeInTheDocument();
    await expect(await canvas.findByText(/Upstream unavailable work status/i)).toBeInTheDocument();
    await expect(
      await canvas.findByText(/Upstream rejected unavailable work status/i),
    ).toBeInTheDocument();
    await expect(canvas.queryByText(/Rate limited filter control/i)).not.toBeInTheDocument();
  },
};
...
export const DegradedWorkStatusFilter: Story = {
  ...
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    await choosePageSize(canvasElement, 50);
    await chooseCommandOptions(canvasElement, /工作状态|work status/i, [/工作降级|degraded/i]);

    await expect(await canvas.findByText(/Plain 429 degraded work status/i)).toBeInTheDocument();
    await expect(await canvas.findByText(/5xx degraded work status/i)).toBeInTheDocument();
    await expect(canvas.queryByText(/Healthy filter control/i)).not.toBeInTheDocument();
  },
};
...
export const BulkSyncSuccessAutoHide: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await waitFor(() => {
      expect(canvas.queryByText(/bulk sync progress|批量同步进度/i)).not.toBeInTheDocument();
    });

    await expect(syncButton).toBeEnabled();
    await expect(
      await canvas.findByText(/2 accounts selected across pages|已跨页选中 2 个账号/i),
    ).toBeInTheDocument();
  },
};
...
export const BulkSyncFailureDismiss: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await expect(progressTitle).toBeInTheDocument();
    await expect(progressTitle.closest(".fixed")).not.toBeNull();
    await expect(await canvas.findByText(/refresh token already rotated/i)).toBeInTheDocument();
...
export const GroupFilterMultiSelectCatalog: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step(
      "keeps account-backed group options available after other filters narrow the roster",
      async () => {
...
export const SlowFilterSwitch: Story = {
  ...
  play: async ({ step }) => {
    await step(
      "blocks the stale roster after the 600ms grace window during a filter switch",
      async () => {
        await waitFor(
          () => {
            expect(
...
export const SlowPageSwitch: Story = {
  ...
  play: async ({ step }) => {
    await step(
      "blocks the stale roster after the 600ms grace window during a page switch",
      async () => {
        await waitFor(
          () => {
            expect(
...
export const CurrentQueryFailure: Story = {
  ...
  play: async ({ step }) => {
    await step(
      "shows the failed current query inline instead of keeping stale rows on screen",
      async () => {
        await waitFor(
          () => {
...
export const UpstreamRejected402: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await expect(await documentScope.findByText(/deactivated_workspace/i)).toBeInTheDocument();
    await expect(
...
export const GroupedView: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step(
      "opens the shared group settings dialog from the grouped summary action",
      async () => {
    ...
    await step(
      "hydrates grouped visible account usage after the light roster first renders",
      async () => {
        const row = await documentScope.findByRole("button", {
...
export const GridView: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    const documentScope = within(canvasElement.ownerDocument.body);
    await step(
      "switches to grouped grid and hides pagination footer plus bulk actions",
      async () => {
        ...
        const groupedRoster = await documentScope.findByTestId("upstream-accounts-grouped-roster");
