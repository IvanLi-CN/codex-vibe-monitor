...
it("uses open mobile rule rows while retaining desktop section surfaces", () => {
  ...
  const sections = Array.from(document.querySelectorAll<HTMLElement>('[data-testid$="-section"]'));
  ...
  expect(
    document.querySelector('[data-testid="effective-routing-rule-status-change-grid"]')?.className,
  ).toContain("grid-cols-2");
  expect(
    document.querySelector('[data-testid="effective-routing-rule-concurrency-editor"]')?.className,
