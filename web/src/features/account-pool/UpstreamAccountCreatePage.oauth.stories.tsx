...
export const ManualMailboxUnsupported: Story = {
  ...
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    await userEvent.type(
      canvas.getByPlaceholderText(/enter a supported mailbox address/i),
      "manual@example.com",
    );
...
export const ReauthDetailOutsideRoster: Story = {
  ...
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    await expect(
      await canvas.findByDisplayValue("Codex Pro - Reauth Hidden Lane"),
    ).toBeInTheDocument();
    ...
    await expect(
      canvas.getByText(/intentionally absent from the current roster page/i),
    ).toBeInTheDocument();
    await userEvent.click(canvas.getByRole("button", { name: /generate oauth url/i }));
    await expect(canvas.getByRole("button", { name: /copy oauth url/i })).toBeEnabled();
  },
...
export const MailboxReady: Story = {
  render: () => {
    const mailboxSession = createMailboxSession(
      "story-mailbox-oauth-ready",
      "oauth-ready@mail-tw.707079.xyz",
    );
    return (
      <AccountPoolStoryRouter
        initialEntry={{
...
export const MailboxRefreshFailed: Story = {
  render: () => {
    const mailboxSession = createMailboxSession(
      "story-mailbox-oauth-failed",
      "failed-oauth@mail-tw.707079.xyz",
    );
    return (
      <AccountPoolStoryRouter
        initialEntry={{
...
export const MailboxDetachedName: Story = {
  render: () => {
    const mailboxSession = createMailboxSession(
      "story-mailbox-oauth-mismatch",
      "oauth-lock@mail-tw.707079.xyz",
    );
    return (
      <AccountPoolStoryRouter
        initialEntry={{
        ...
        state: {
          draft: {
            oauth: {
              ...
              callbackUrl:
                "http://localhost:43210/oauth/callback?code=oauth-duplicate&state=storybook",
