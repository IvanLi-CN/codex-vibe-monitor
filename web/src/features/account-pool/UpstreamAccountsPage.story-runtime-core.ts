  ...
  return Array.from({ length: 24 }, (_, index) => {
    ...
    return {
      ...
      successCount: Math.max(
        0,
        Math.round(baseline + Math.sin((index + seed) / 2.3) * (baseline * 0.3)),
      ),
...
const defaultForwardProxyNodes: ForwardProxyBindingNode[] = [
  {
    ...
    egressIp: "198.51.100.40",
    egressIpCheckedAt: "2026-03-17T12:55:00.000Z",
    egressIpProvider: "ipify",
    egressIpError: null,
  ...
  },
  {
    ...
    protocolLabel: "SS",
    egressIp: "203.0.113.85",
    egressIpCheckedAt: "2026-03-17T12:50:00.000Z",
    egressIpProvider: "ipify",
    egressIpError: null,
    egressIpErrorAt: null,
    penalized: false,
  ...
  },
  {
    ...
    egressIpProvider: "ipify",
    egressIpError: "metadata refresh timed out",
    egressIpErrorAt: "2026-03-17T12:48:00.000Z",
    penalized: true,
    selectable: true,
...
function buildOperationalRosterAccounts(replicaCount = 1) {
  ...
  const specs = Array.from({ length: Math.max(1, replicaCount) }, (_, replicaIndex) =>
    baseSpecs.map((spec, baseIndex) => ({
      ...
      displayName:
        replicaIndex === 0 ? spec.displayName : `${spec.displayName} · lane ${replicaIndex + 1}`,
      replicaIndex,
      baseIndex,
    })),
  ).flat();
  ...
  return specs.map((spec, index) => {
    ...
    const commonOverrides: Partial<UpstreamAccountDetail> = {
      ...
      chatgptAccountId: spec.kind === "oauth_codex" ? `org_mock_${spec.id}` : null,
      chatgptUserId: spec.kind === "oauth_codex" ? `user_mock_${spec.id}` : null,
      maskedApiKey:
...
function buildDynamicOperationalRosterAccounts(phase: number) {
  ...
  return buildOperationalRosterAccounts(2).map((detail, index) => {
    const overrideGroupName = Object.hasOwn(groupOverrides, detail.id)
...
function buildDynamicGroupSettings(phase: number): {
  ...
  if (phase % 2 === 0) {
    return {
      groupNotes: {
        ...
        "staging-overflow": "Cycle A rebinds overflow lanes onto the fallback proxy catalog.",
        analytics: "Cycle A keeps analytics lanes visible as a shorter comparison group.",
      },
      groupBoundProxyKeys: {
  ...
  return {
    groupNotes: {
      ...
      experiments: "Cycle B adds a short experiments group for another height transition target.",
    },
    groupBoundProxyKeys: {
      production: [directBindingKey, subscriptionVlessKey],
      staging: ["drain-node"],
...
export function applyDynamicRosterLiveRefresh(store: StoryStore, fetchCount: number) {
...
export function buildBulkSyncCounts(
...
export function createBulkSyncRows(
  store: StoryStore,
  accountIds: number[],
): BulkUpstreamAccountSyncRow[] {
  return accountIds.map((accountId) => {
    const account =
      store.details[accountId] ?? store.accounts.find((item) => item.id === accountId);
    return {
      accountId,
...
export function storyEnableStatus(
  item: Pick<UpstreamAccountSummary, "enableStatus" | "enabled" | "displayStatus">,
) {
  ...
  const detail: UpstreamAccountDetail = {
    ...
    recentActions: [
      ...
      },
      {
        ...
        httpStatus: 429,
        failureKind: "model",
        invokeId: "story-model-scope-429",
        attemptId: "N7F4K2QA",
        model: "gpt-5.4-mini",
        ...
        modelRouteFailureCount: 5,
        modelRouteCooldownUntil: "2026-07-25T10:04:03.000Z",
        createdAt: "2026-07-25T10:03:18.000Z",
      },
    ],
...
export function toSummary(detail: UpstreamAccountDetail): UpstreamAccountSummary {
  ...
  return {
    ...
    currentForwardProxyDisplayName: normalized.currentForwardProxyDisplayName ?? null,
    currentForwardProxyState: normalized.currentForwardProxyState ?? "unconfigured",
...
export function createStore(): StoryStore {
  ...
  const mixedPlanCoexistenceStory = storyId?.endsWith("--mixed-plan-coexistence") === true;
  const teamSharedOrgCoexistenceStory = storyId?.endsWith("--team-shared-org-coexistence") === true;
  ...
  const oauthRetryTerminalStateStory = storyId?.endsWith("--oauth-retry-terminal-state") === true;
  const quotaExhaustedOauthStory = storyId?.endsWith("--quota-exhausted-oauth") === true;
  const upstreamRejected402Story = storyId?.endsWith("--upstream-rejected-402") === true;
  const degradedWorkStatusStory = storyId?.endsWith("--degraded-work-status-filter") === true;
  const unavailableWorkStatusStory = storyId?.endsWith("--unavailable-work-status-filter") === true;
  const missingWindowPlaceholdersStory =
    storyId?.endsWith("--missing-window-placeholders") === true;
  const bulkSyncSuccessStory = storyId?.endsWith("--bulk-sync-success-auto-hide") === true;
  const bulkSyncFailureStory = storyId?.endsWith("--bulk-sync-failure-dismiss") === true;
