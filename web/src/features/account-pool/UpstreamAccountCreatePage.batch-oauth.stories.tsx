...
export const NeedsIdentityConfirmation: Story = {
  ...
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    await expect(
      canvas.getByText(/identity overwrite confirmation required|需要确认身份覆盖/i),
    ).toBeInTheDocument();
    ...
    await expect(
      canvas.getByRole("button", { name: /confirm identity overwrite|确认身份覆盖/i }),
    ).toBeEnabled();
  },
};
...
export const Ready: Story = {
  render: () => (
    <AccountPoolStoryRouter initialEntry="/account-pool/upstream-accounts/new?mode=batchOauth" />
  ),
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    ...
    try {
      ...
      await expect(
        canvas.getByDisplayValue(/https:\/\/auth\.openai\.com\/authorize/i),
      ).toBeInTheDocument();
      await expect(
        canvas.getByRole("button", { name: /complete oauth login/i }),
      ).toBeInTheDocument();
...
export const GenerateAutoCopyFallback: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    try {
      ...
      await expect(
        documentScope.getByText(/copy failed\. select the auth url field and copy it manually\./i),
      ).toBeInTheDocument();
      await expect(
        documentScope.getByText(/https:\/\/auth\.openai\.com\/authorize/i),
      ).toBeInTheDocument();
    } finally {
      Object.defineProperty(navigator, "clipboard", {
        configurable: true,
...
export const PendingDefaultGroupSync: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await expect(canvas.getByRole("button", { name: /copy oauth url/i })).toBeEnabled();
    await expect(
      canvas.queryByText(/generate a fresh oauth url for this row/i),
...
export const MailboxPopoverEdit: Story = {
  render: () => {
    const mailboxSession = createMailboxSession(
      "story-mailbox-batch-edit",
      "batch-edit@mail-tw.707079.xyz",
    );
    return (
      <AccountPoolStoryRouter
        initialEntry={{
  ...
  play: async ({ canvasElement }) => {
    ...
    await expect(
      within(document.body).getByRole("button", { name: /cancel mailbox edit/i }),
    ).toBeInTheDocument();
...
export const MailboxPopoverAutoCreate: Story = {
  render: () => {
    const mailboxSession = createMailboxSession(
      "story-mailbox-batch-autocreate",
      "batch-edit@mail-tw.707079.xyz",
    );
    return (
      <AccountPoolStoryRouter
...
export const MailboxAttachPending: Story = {
  ...
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    await expect(
      within(document.body).getByRole("button", { name: /submit mailbox/i }),
    ).toBeDisabled();
...
export const ActionTooltips: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await expect(
      within(document.body).getByText(/generate a fresh oauth url for this row/i),
    ).toBeInTheDocument();
    ...
    await new Promise((resolve) => window.setTimeout(resolve, 330));
    await expect(
      within(document.body).getByRole("button", { name: /regenerate oauth url/i }),
    ).toBeInTheDocument();
...
export const MailboxReady: Story = {
  render: () => {
    const mailboxSession = createMailboxSession(
      "story-mailbox-batch-ready",
      "batch-row@mail-tw.707079.xyz",
    );
    return (
      <AccountPoolStoryRouter
        initialEntry={{
  ...
  play: async ({ canvasElement }) => {
    ...
    await expect(canvas.getByRole("button", { name: /copy verification code/i })).toHaveTextContent(
      "556677",
    );
  },
};
...
export const InvitedBadgeTooltip: Story = {
  render: () => {
    const mailboxSession = createMailboxSession(
      "story-mailbox-batch-invited",
      "batch-invited@mail-tw.707079.xyz",
    );
    return (
      <AccountPoolStoryRouter
        initialEntry={{
  ...
  play: async ({ canvasElement }) => {
    ...
    await expect(
      within(document.body).getByText(/this mailbox already received a workspace invite email/i),
    ).toBeInTheDocument();
  },
};
...
export const MailboxRefreshFailed: Story = {
  render: () => {
    const mailboxSession = createMailboxSession(
      "story-mailbox-batch-failed",
      "failed-batch@mail-tw.707079.xyz",
    );
    return (
      <AccountPoolStoryRouter
        initialEntry={{
...
export const MailboxDetachedName: Story = {
  render: () => {
    const mailboxSession = createMailboxSession(
      "story-mailbox-batch-mismatch",
      "batch-locked@mail-tw.707079.xyz",
    );
    return (
      <AccountPoolStoryRouter
        initialEntry={{
              ...
              rows: [
                {
                  ...
                  callbackUrl:
                    "http://localhost:43210/oauth/callback?code=batch-duplicate&state=storybook",
              ...
              rows: [
                {
                  ...
                  sessionHint:
                    "Codex Pro - Tokyo is ready. Continue with the remaining rows when you are done here.",
                  duplicateWarning: {
                    accountId: 101,
                    displayName: "Codex Pro - Tokyo",
                    peerAccountIds: [103],
            ...
            batchOauth: {
              ...
              rows: [
                ...
                },
                {
                  ...
                  groupName: "production",
                  callbackUrl:
                    "http://localhost:43210/oauth/callback?code=batch-pending&state=storybook",
                ...
                },
                {
                  ...
                  groupName: "production",
                  callbackUrl:
                    "http://localhost:43210/oauth/callback?code=batch-clean&state=storybook",
                  session: createCompletedSession("story-batch-clean-done", 104),
                  sessionHint:
                    "Codex Pro - Nagoya is ready. Continue with the remaining rows when you are done here.",
                },
                {
                  id: "row-4",
