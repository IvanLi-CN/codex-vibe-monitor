...
describe('TodayStatsOverview', () => {
  it('keeps the original in-progress tile and adds the queued value before success', () => {
    ...
    const tokenTile = host?.querySelector('[data-testid="today-stats-value-total-tokens"]')?.closest(
  ...
  it('renders the new natural-day KPI helper semantics inline', () => {
    ...
    expect(host?.querySelector('[data-testid="today-stats-secondary-response-time-avg-total"]')?.textContent).toContain(
      '1.39 s',
    )
  ...
  it('renders the response-time card with recent-window and day-average latency values', () => {
    ...
    const avgTotal = host?.querySelector('[data-testid="today-stats-secondary-response-time-avg-total"]')?.textContent ?? ''
