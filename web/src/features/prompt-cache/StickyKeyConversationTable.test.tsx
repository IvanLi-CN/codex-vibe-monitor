...
function findButton(labels: string[]) {
  return (
    Array.from(document.querySelectorAll("button")).find((button): button is HTMLButtonElement =>
      labels.some(
        (label) =>
...
describe("StickyKeyConversationTable", () => {
  ...
  it("renders activity-window inactivity notes with the selected hour range", () => {
    ...
    expect(
      html.includes(
        "2 conversation(s) were hidden because they had no activity in the last 3 hour(s).",
