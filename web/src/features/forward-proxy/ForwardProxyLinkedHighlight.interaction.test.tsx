...
function hover(element: Element, clientX: number, clientY: number) {
  act(() => {
    element.dispatchEvent(
      new MockPointerEvent("pointerover", {
        bubbles: true,
        clientX,
        clientY,
        pointerType: "mouse",
    ...
    element.dispatchEvent(
      new MockPointerEvent("pointerenter", {
        bubbles: true,
        clientX,
        clientY,
        pointerType: "mouse",
...
function leave(element: Element) {
  act(() => {
    element.dispatchEvent(
      new MockPointerEvent("pointerout", { bubbles: true, pointerType: "mouse" }),
    );
    element.dispatchEvent(
      new MockPointerEvent("pointerleave", { bubbles: true, pointerType: "mouse" }),
    );
...
describe("ForwardProxyLiveTable linked chart highlight", () => {
  it("links request bars and weight trend points for the same row bucket", () => {
    ...
    const requestSurface = document.querySelector(
      '[aria-label="Proxy A 近 24 小时请求量图"]',
    ) as HTMLElement;
    const weightSurface = document.querySelector(
      '[aria-label="Proxy A 近 24 小时权重趋势图"]',
    ) as HTMLElement;
    const requestChart = document.querySelector(
      '[data-chart-kind="proxy-request-trend"]',
    ) as HTMLElement;
    const weightChart = document.querySelector(
