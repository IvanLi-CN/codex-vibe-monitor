...
function buildVisibleBarHeights(
  successCount: number,
  failureCount: number,
  scaleMax: number,
    ...
    rows: [
      {
        ...
        value: numberFormatter.format(bucket.successCount),
        tone: "success",
      },
...
export function ForwardProxyRequestTrendChart({
  ...
  const tooltipData = useMemo(
    () =>
      buckets.map((bucket) =>
        buildRequestTooltipData(bucket, localeTag, tooltipLabels, numberFormatter),
      ),
            ...
            return (
              <div
                ...
                className={cn(
                  ...
                  isActive &&
                    "z-[1] border-primary/60 bg-base-200/70 shadow-[0_0_0_1px_rgba(96,165,250,0.15)]",
                )}
                {...getItemProps(index)}
              >
                {isActive ? (
                  <span className="absolute inset-0 bg-primary/8" aria-hidden="true" />
                ) : null}
