  ...
  return (
    <div className="metric-cell">
      ...
      <div className={`metric-value ${toneClass ?? ""}`}>
        {loading ? "…" : <AnimatedDigits value={value} />}
      </div>
    </div>
  );
}

export function InvocationRecordsSummaryCards({
  focus,
  summary,
  isLoading,
  error,
}: InvocationRecordsSummaryCardsProps) {
  ...
  const formatMs = (value?: number | null) =>
    value == null ? "—" : `${numberFormatter.format(value)} ms`;
  ...
  return (
    <div className="space-y-3">
      {showInlineError ? (
        <Alert variant="error">{t("records.summary.loadError", { error: error ?? "" })}</Alert>
      ) : null}
