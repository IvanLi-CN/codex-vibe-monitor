      ...
      <div className="relative z-10 mb-4 flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-base-content/65">
            Shared surface tokens across bubble variants.
          </p>
        </div>
    ...
    <div className={className}>
      <label className="field">
        ...
        <div className="relative">
          <Input
            readOnly
            value="proxy.example.com/gateway"
...
export const StateGallery: Story = {
  ...
  play: async ({ canvasElement }) => {
    const bubbles = canvasElement.ownerDocument.body.querySelectorAll(
...
export const InputCornerError: Story = {
  ...
  render: (args) => (
    <InputCornerHarness message={args.message} variant={args.variant} className="max-w-xl" />
  ),
  play: async ({ canvasElement }) => {
  ...
  render: (args) => (
    <div className="grid gap-4">
      <p className="text-sm text-base-content/70">
        The yellow card intentionally uses `overflow-hidden` and a narrow width.
      </p>
...
export const ViewportEdgeAware: Story = {
  ...
  render: (args) => (
    <div className="flex min-h-[70vh] items-end justify-end rounded-[1.6rem] border border-base-300/70 bg-base-200/55 p-4">
      <InputCornerHarness message={args.message} variant={args.variant} className="w-[14rem]" />
    </div>
  ),
  play: async ({ canvasElement }) => {
    const doc = within(canvasElement.ownerDocument.body);
