...
function useAutoDismiss(notificationId: string, pending: boolean, onDismiss: (id: string) => void) {
...
export function MotherSwitchUndoToast({
  ...
  const groupLabel =
    notification.payload.groupName ?? t("accountPool.upstreamAccounts.groupFilter.ungrouped");
  const message = useMemo(() => {
    if (
      notification.payload.newMotherDisplayName &&
      notification.payload.previousMotherDisplayName
    ) {
