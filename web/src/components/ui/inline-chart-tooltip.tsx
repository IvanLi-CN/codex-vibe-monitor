...
export function InlineChartTooltipSurface({
  ...
  useLayoutEffect(() => {
    ...
    const updatePosition = () => {
      ...
      const maxX = Math.max(
        TOOLTIP_PADDING,
        ownerWindow.innerWidth - tooltipRect.width - TOOLTIP_PADDING,
      );
      const maxY = Math.max(
        TOOLTIP_PADDING,
  ...
  return (
    <div
      ref={containerRef}
      className={cn(
        "relative overflow-visible rounded-lg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/70",
        className,
      )}
    ...
    >
      ...
      <div className={cn("relative", chartClassName)}>
        {children({ activeIndex: state.activeIndex, highlightedIndex, getItemProps })}
      </div>
