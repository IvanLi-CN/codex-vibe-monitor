...
export function ForwardProxyRequestTrendChart({
  ...
  const chartHeight = isDialog ? 32 : 40
          ...
          {buckets.map((bucket, index) => {
            ...
            return (
              <div
                ...
                className={cn(
                  isDialog
                    ? 'relative flex h-8 min-w-0 flex-1 cursor-pointer flex-col overflow-hidden rounded-[3px] border border-transparent bg-base-300/35 transition-[transform,background-color,border-color,box-shadow] duration-150 ease-out motion-reduce:transition-none'
