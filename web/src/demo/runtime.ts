...
export function sceneFromLocation(location: Location | undefined = typeof window === 'undefined' ? undefined : window.location): DemoScene {
  if (!location) return 'operational'
  const hashSearch = location.hash.includes('?') ? location.hash.slice(location.hash.indexOf('?') + 1) : ''
...
export function themeFromLocation(location: Location | undefined = typeof window === 'undefined' ? undefined : window.location): DemoTheme {
