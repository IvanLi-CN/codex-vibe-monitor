...
describe('demo MSW handlers', () => {
  ...
  it('serves a dense, linked operational dataset across records, pool, detail, and system pages', async () => {
    ...
    const records = await recordsResponse.json() as { records: Array<{ id: number; invokeId: string; upstreamAccountId: number; promptCacheKey: string }> }
    const accounts = await accountsResponse.json() as { items: Array<{ id: number; groupName: string | null; boundProxyKeys: string[] }> }
    const events = await eventsResponse.json() as { items: Array<{ accountDisplayName: string; forwardProxyKey: string | null }> }
    ...
    expect(tasks.items.map((item) => item.status)).toEqual(expect.arrayContaining(['success', 'running', 'failed']))
    ...
    const [detailResponse, attemptsResponse, accountResponse] = await Promise.all([
      ...
      fetch(`http://demo.invalid/api/pool/upstream-accounts/${selectedRecord?.upstreamAccountId}?includeRecentActions=true`),
    ])
    const detail = await detailResponse.json() as { id: number; abnormalResponseBody: { available: boolean } }
    const attempts = await attemptsResponse.json() as Array<{ invokeId: string; upstreamAccountId: number }>
    const account = await accountResponse.json() as { id: number; history: unknown[]; recentActions: unknown[] }

    expect(detail.id).toBe(selectedRecord?.id)
    expect(attempts[0]).toMatchObject({ invokeId: selectedRecord?.invokeId, upstreamAccountId: 101 })
  ...
  it('returns populated proxy and prompt-cache surfaces rather than empty placeholders', async () => {
    ...
    const proxy = await proxyResponse.json() as { nodes: Array<{ last24h: unknown[]; weight24h: unknown[] }> }
    const cache = await cacheResponse.json() as { conversations: Array<{ upstreamAccounts: unknown[]; recentInvocations: unknown[] }> }
    const proxyHistory = await proxyHistoryResponse.json() as { nodes: Array<{ buckets: unknown[] }> }

    expect(proxy.nodes).toHaveLength(5)
    expect(proxy.nodes.every((node) => node.last24h.length > 0 && node.weight24h.length > 0)).toBe(true)
