  ...
  return [
    {
      ...
      upstreamAccountName: "alpha@demo.invalid",
      ...
      inputTokens: 12520,
      outputTokens: 0,
      cacheInputTokens: 10880,
      cacheWriteTokens: 1640,
      reasoningTokens: 480,
      reasoningEffort: "high",
      totalTokens: 12520,
      cost: 0.014,
    ...
    },
    {
      ...
      occurredAt: "2026-07-10T09:23:00.000Z",
      createdAt: "2026-07-10T09:23:00.000Z",
      source: "proxy",
      proxyDisplayName: "Tokyo demo relay",
      upstreamAccountId: 101,
      upstreamAccountName: "alpha@demo.invalid",
      ...
      endpoint: "/v1/responses",
      model: "gpt-5.6-sol",
      status: attention ? "http_502" : "success",
      requestedServiceTier: "auto",
      serviceTier: "auto",
      inputTokens: 9320,
      outputTokens: 882,
      cacheInputTokens: 7311,
      cacheWriteTokens: 2009,
      reasoningTokens: 360,
      reasoningEffort: "medium",
      totalTokens: 10202,
      cost: 0.0092,
      tUpstreamTtfbMs: 184,
      tTotalMs: 1882,
      errorMessage: attention ? "simulated upstream timeout" : null,
      failureClass: attention ? "service_failure" : null,
      failureKind: attention ? "upstream_timeout" : null,
    },
    {
      id: 9003,
      invokeId: "demo-invocation-9003",
      occurredAt: "2026-07-10T09:17:00.000Z",
      createdAt: "2026-07-10T09:17:00.000Z",
      source: "proxy",
      proxyDisplayName: "Tokyo demo relay",
      upstreamAccountId: 102,
      upstreamAccountName: "backup-key",
      upstreamAccountPlanType: "api",
      ...
      inputTokens: 4110,
      outputTokens: 295,
      cacheInputTokens: 2980,
      cacheWriteTokens: 1130,
      reasoningTokens: 0,
      totalTokens: 4405,
      cost: 0.0037,
      tUpstreamTtfbMs: 146,
    ...
    demoModel.snapshot.scene === "empty"
      ...
      : Array.from({ length: 12 }, (_, index) => ({
          bucketStart: new Date(
            Date.parse("2026-07-10T00:00:00Z") + index * 3_600_000,
          ).toISOString(),
          bucketEnd: new Date(
            Date.parse("2026-07-10T00:00:00Z") + (index + 1) * 3_600_000,
          ).toISOString(),
...
function promptCacheConversations() {
  if (demoModel.snapshot.scene === "empty") {
    return {
      rangeStart: "2026-07-10T09:00:00Z",
      rangeEnd: demoNow(),
      selectionMode: "count",
      selectedLimit: 50,
  ...
  return {
    ...
    conversations: [
      {
        promptCacheKey: "demo-conversation-a",
...
function forwardProxyLive() {
  ...
  const node = (
    demoModel.snapshot.settings.forwardProxy as { nodes: Array<Record<string, unknown>> }
  ).nodes[0];
  return {
    rangeStart: "2026-07-10T00:00:00Z",
    rangeEnd: demoNow(),
    ...
    nodes: [
      {
        ...
        last24h: [
          {
            bucketStart: "2026-07-10T08:00:00Z",
            bucketEnd: "2026-07-10T09:00:00Z",
            successCount: demoModel.snapshot.scene === "attention" ? 12 : 17,
            failureCount: demoModel.snapshot.scene === "attention" ? 6 : 1,
          },
        ],
        weight24h: [
          {
...
function accountList() {
  ...
  return {
    ...
    groups: [
      {
        groupName: "production",
        note: "Simulated primary pool",
        accountCount: 1,
        boundProxyKeys: ["demo-tokyo"],
      },
      {
        groupName: "standby",
        note: "Simulated recovery capacity",
        accountCount: 1,
        boundProxyKeys: [],
      },
    ],
    ...
    metrics: {
      ...
      apiKey: items.filter((item) => item.kind === "api_key").length,
      attention: demoModel.snapshot.scene === "attention" ? 1 : 0,
    },
  };
}
...
async function handleRequest(request: Request) {
  ...
  if (pathname === "/api/stats/forward-proxy/timeseries")
    return json({ rangeStart: "2026-07-10T00:00:00Z", rangeEnd: demoNow(), nodes: [], points: [] });
  ...
  if (pathname.startsWith("/api/stats/prompt-cache-conversation-bindings/"))
    return json({ bindingKind: "none", upstreamAccountId: null, groupName: null });
  ...
  if (pathname === "/api/invocations/suggestions")
    return json({
      model: { items: [{ value: "gpt-5.6-sol", count: 12 }], hasMore: false },
      proxy: { items: [], hasMore: false },
      endpoint: { items: [], hasMore: false },
      failureKind: { items: [], hasMore: false },
      promptCacheKey: { items: [], hasMore: false },
  ...
  if (pathname.endsWith("/response-body"))
    return json({ content: '{"demo":true}', encoding: "identity", truncated: false });
  ...
  if (pathname === "/api/system/tasks")
    return json({
      ...
      items: [
        {
          id: 1,
          taskKind: "demo_refresh",
          triggerKind: "manual",
          status: "success",
          summary: "simulated refresh completed",
          detail: "Demo-only task record.",
      ...
      items: [
        {
          ...
          accountId: 101,
          occurredAt: demoNow(),
          detail: "Simulated account sync.",
        },
      ],
    });
  if (pathname === "/api/pool/upstream-accounts/window-usage") return json({ items: [] });
  if (pathname === "/api/pool/forward-proxy-binding-nodes")
    return json((demoModel.snapshot.settings.forwardProxy as { nodes: unknown[] }).nodes);
  if (pathname === "/api/pool/tags" && request.method === "GET")
    return json({
      writesEnabled: true,
      items: [
        { id: 1, name: "primary", accountCount: 1, groupCount: 1, updatedAt: demoNow() },
        { id: 2, name: "fallback", accountCount: 1, groupCount: 1, updatedAt: demoNow() },
      ],
    });
  ...
  if (pathname.includes("/sticky-keys"))
    return json({ totalMatched: 0, conversations: [], hasMore: false, nextCursor: null });
  if (/^\/api\/pool\/upstream-accounts\/\d+$/.test(pathname) && request.method === "GET")
    return json(demoModel.snapshot.accounts[0]);
