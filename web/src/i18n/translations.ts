...
const baseTranslations = {
  en: {
    ...
    'accountPool.upstreamAccounts.hints.scopeTitle': 'This OAuth token is missing API scopes',
    'accountPool.upstreamAccounts.hints.scopeBody': 'Re-authorize this account once after deploying the scope fix so it can receive the new API scopes. If it still fails afterward, verify the OpenAI org/project role for this account.',
    'accountPool.upstreamAccounts.hints.permissionTitle': 'This OAuth account still lacks API permissions',
    'accountPool.upstreamAccounts.hints.permissionBody': 'Check the upstream error details first. If this token was created before the scope rollout, re-authorize once; if it still fails, verify the OpenAI org/project role for this account.',
  ...
  zh: {
    ...
    'accountPool.upstreamAccounts.hints.scopeTitle': '这个 OAuth token 缺少 API scopes',
    'accountPool.upstreamAccounts.hints.scopeBody': '请在部署这次 scope 修复后，对该账号重新授权一次，让它拿到新的 API scopes。若重新授权后仍失败，再检查这个账号在 OpenAI org / project 中的角色权限。',
    'accountPool.upstreamAccounts.hints.permissionTitle': '这个 OAuth 账号仍然缺少 API 权限',
    'accountPool.upstreamAccounts.hints.permissionBody': '先看上游错误详情。如果这个 token 是在 scope 修复前授权的，请重新授权一次；如果仍失败，再检查这个账号在 OpenAI org / project 中的角色权限。',
