...
const ROUTES: RouteCase[] = [
  ...
  },
  {
    ...
    waitFor: async (page) => {
      await expect(
        page.getByRole("heading", { name: /系统状态|System status/ }).first(),
      ).toBeVisible();
    },
  },
];
...
async function forceUpdateBanner(page: Page) {
  await page.waitForFunction(
    () =>
      typeof (window as Window & { __DEV_FORCE_UPDATE_BANNER__?: () => void })
        .__DEV_FORCE_UPDATE_BANNER__ === "function",
  );
  await page.evaluate(() => {
    (
      window as Window & { __DEV_FORCE_UPDATE_BANNER__?: () => void }
