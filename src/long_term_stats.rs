...
const LONG_TERM_PROJECTION_INTERVAL_BASELINE_VERSION: i64 = 1;
const LONG_TERM_PROJECTION_INTERVAL_STATE_VERSION: i64 = 2;
const LONG_TERM_PROJECTION_WRITE_BATCH_ROWS: usize = 512;
const LONG_TERM_PROJECTION_LEGACY_MIGRATION_INVOCATIONS_PER_FLUSH: usize = 512;
const LONG_TERM_PROJECTION_LEGACY_CLEANUP_BATCHES_PER_FLUSH: usize = 4;
...
#[derive(Debug, Default)]
struct LongTermProjectionWriteBatches {
    rows: u64,
    transactions: usize,
    largest_transaction_rows: usize,
}

#[derive(Debug, Default)]
struct LongTermProjectionIntervalInsert {
    inserted: Vec<LongTermProjectionIntervalSegment>,
    transactions: usize,
    largest_transaction_rows: usize,
}

#[derive(Debug, Default)]
struct LongTermProjectionLegacyMigration {
    deleted: LongTermProjectionWriteBatches,
    migrated_invocations: usize,
    complete: bool,
}

impl LongTermProjectionWriteBatches {
    fn record(&mut self, rows: u64) {
        self.rows = self.rows.saturating_add(rows);
        self.transactions = self.transactions.saturating_add(1);
        self.largest_transaction_rows = self.largest_transaction_rows.max(rows as usize);
    }
}
...
struct LongTermProjectionWriteControl<'a> {
    ...
    after_commit: Option<&'a (dyn Fn() + Send + Sync)>,
    outer_admitted: bool,
}

impl LongTermProjectionWriteControl<'_> {
    fn begin(
        self,
        operation: &'static str,
    ) -> Result<Option<crate::db_pressure::DbBackgroundPermit>> {
        if self.cancel.is_some_and(CancellationToken::is_cancelled) {
            bail!("long-term projection write cancelled before transaction");
        }
        if self.outer_admitted {
            return Ok(None);
        }
        Ok(Some(self.gate.try_begin_background(operation).map_err(
            |reason| {
                anyhow::anyhow!(
...
async fn ensure_long_term_projection_correction_trigger(pool: &Pool<Sqlite>) -> Result<()> {
    ...
    sqlx::query(
        ...
        WHEN NOT (
          ...
          AND NOT EXISTS (
            SELECT 1
            FROM long_term_projection_interval_state
            WHERE invocation_row_id = NEW.id
          )
        )
        BEGIN
...
fn build_long_term_projection_event(row: &LongTermInvocationRow) -> LongTermProjectionEvent {
    ...
    let bucket_dates = daily
        ...
        .collect::<HashSet<_>>();
    let segments = collect_long_term_projection_interval_segments(row);
...
async fn flush_long_term_projection(state: &AppState, trigger: &'static str) -> Result<()> {
    ...
    let result = match flush_long_term_projection_inner(state, trigger).await {
        Err(error)
            if error
                .to_string()
                .contains("write deferred by database pressure") =>
        {
            defer_long_term_projection_terminal_repair(state, "writer_pressure").await;
            debug!(projection = "long_term", trigger, flush_outcome = "deferred", defer_reason = "writer_pressure", error = %error, "long-term projection write deferred by database pressure");
            Ok(0)
        }
        Err(error) if error.to_string().contains("write cancelled") => {
            debug!(
                projection = "long_term",
                trigger,
                flush_outcome = "cancelled",
                "long-term projection stopped at a transaction boundary"
            );
            Ok(0)
        }
        result => result,
    ...
    if let Err(error) = &result {
        crate::db_pressure::global_db_pressure_gate()
            .record_error("long_term_projection_flush", error);
    }
...
async fn flush_long_term_projection_inner(state: &AppState, trigger: &'static str) -> Result<u64> {
    ...
    let write_control = LongTermProjectionWriteControl {
        cancel: Some(&state.shutdown),
        gate,
        after_commit: None,
        outer_admitted: false,
    };
    let _initial_permit = match write_control.begin("long_term_projection_flush_start") {
        Ok(permit) => permit,
        Err(error) if error.to_string().contains("database pressure") => {
            let mut runtime = state.long_term_projection_runtime.lock().await;
            ...
            debug!(projection = "long_term", trigger, gate_outcome = "deferred", defer_reason = "writer_pressure", reason = %error, "long-term projection flush deferred by database pressure gate");
            return Ok(0);
        }
    ...
    let interval_state_needs_baseline =
        long_term_projection_interval_state_needs_baseline(&state.pool).await?;
    ...
    } else if interval_state_needs_baseline
        || long_term_rollups_exist(&state.pool).await?
            && (cursor == 0
                || matches!(
                    state_row.status.as_str(),
    ...
    if let Some(baseline_cursor) = baseline_cursor {
        ...
        let _permit = write_control.begin("long_term_projection_baseline_queue")?;
        ...
        if let Some(event) = repair_event {
            ...
            if !long_term_projection_allows_expensive_repair(trigger) {
                ...
                let _permit = write_control.begin("long_term_projection_repair_queue")?;
                ...
                if long_term_projection_repairs_are_deferred(&state.pool, &repair_dates).await? {
                    ...
                    if let Some(error) = rebuild_error {
                        let _permit = write_control.begin("long_term_projection_repair_defer")?;
    ...
    let legacy_interval_migration = migrate_legacy_long_term_projection_intervals(
        &state.pool,
        LONG_TERM_PROJECTION_LEGACY_MIGRATION_INVOCATIONS_PER_FLUSH,
        write_control,
    )
    .await?;
    if legacy_interval_migration.complete
        && long_term_projection_interval_state_needs_migration_finalize(&state.pool).await?
    {
        mark_long_term_projection_interval_state_migrated(&state.pool, write_control).await?;
    }
    ...
    debug!(
        ...
        legacy_interval_migration_rows = legacy_interval_migration.deleted.rows,
        legacy_interval_migrated_invocations = legacy_interval_migration.migrated_invocations,
        legacy_interval_migration_complete = legacy_interval_migration.complete,
...
async fn prune_long_term_projection_hourly_retention(
    ...
    prune_long_term_projection_hourly_retention_with_control(
        pool,
        retention_days,
        LongTermProjectionWriteControl {
            cancel: None,
            gate: crate::db_pressure::global_db_pressure_gate(),
            after_commit: None,
            outer_admitted: false,
        },
    )
...
async fn prune_long_term_projection_hourly_retention_with_control(
    ...
    let hourly =
        delete_long_term_projection_hourly_rollups_before(pool, retention_start_epoch, control)
            .await?;
    let legacy_intervals = delete_legacy_long_term_projection_hourly_intervals_before(
        pool,
        &retention_start_date.to_string(),
        control,
    )
    .await?;
    Ok((hourly.rows, legacy_intervals.rows))
}
...
async fn long_term_projection_interval_state_needs_baseline(pool: &Pool<Sqlite>) -> Result<bool> {
    Ok(sqlx::query_scalar::<_, i64>(
        "SELECT COALESCE(interval_state_version, 0) < ?1 FROM long_term_projection_state WHERE consumer = ?2",
    )
    .bind(LONG_TERM_PROJECTION_INTERVAL_BASELINE_VERSION)
    .bind(LONG_TERM_PROJECTION_CONSUMER)
    .fetch_one(pool)
    .await?
        != 0)
...
async fn long_term_projection_interval_state_needs_migration_finalize(
    pool: &Pool<Sqlite>,
) -> Result<bool> {
    Ok(sqlx::query_scalar::<_, i64>(
        "SELECT COALESCE(interval_state_version, 0) < ?1 FROM long_term_projection_state WHERE consumer = ?2",
    )
    .bind(LONG_TERM_PROJECTION_INTERVAL_STATE_VERSION)
    .bind(LONG_TERM_PROJECTION_CONSUMER)
    .fetch_one(pool)
    .await?
        != 0)
...
async fn load_long_term_projection_rows_for_date(
    ...
    let query = long_term_projection_live_rows_query(upstream_account_sql);
...
fn long_term_projection_live_rows_query(upstream_account_sql: &str) -> String {
    format!(
        r#"
        SELECT * FROM (
          SELECT inv.id, inv.invoke_id, inv.occurred_at, inv.status, inv.model,
          ...
          WHERE LOWER(TRIM(COALESCE(inv.status, ''))) NOT IN ('running', 'pending')
            AND instr(COALESCE(inv.occurred_at, ''), 'T') = 0
          ...
          WHERE LOWER(TRIM(COALESCE(inv.status, ''))) NOT IN ('running', 'pending')
            AND instr(COALESCE(inv.occurred_at, ''), 'T') = 0
            AND inv.occurred_at < ?1
            AND inv.t_total_ms IS NOT NULL
          ...
          WHERE LOWER(TRIM(COALESCE(inv.status, ''))) NOT IN ('running', 'pending')
            AND instr(COALESCE(inv.occurred_at, ''), 'T') > 0
            AND CAST(strftime('%s', inv.occurred_at) AS INTEGER) < ?4
            AND (
              CAST(strftime('%s', inv.occurred_at) AS INTEGER) >= ?3
              OR (
                inv.t_total_ms IS NOT NULL
                AND inv.t_total_ms > 0
                AND CAST(strftime('%s', inv.occurred_at) AS INTEGER) + inv.t_total_ms / 1000.0 >= ?3
              )
            )
        )
        ORDER BY occurred_at ASC, id ASC
        "#,
        upstream_account_sql = upstream_account_sql,
    )
}
...
async fn build_long_term_projection_date_rebuild(
    ...
    for row in &mut rows {
        ...
        interval_segments.extend(collect_long_term_projection_interval_segments(row));
...
async fn commit_long_term_projection_date_rebuilds_with_control(
    ...
    for rebuild in rebuilds {
        let _permit = control.begin("long_term_projection_interval_invalidate")?;
        ...
        drop(_permit);
        control.committed();
        insert_long_term_projection_interval_segments(pool, &rebuild.interval_segments, control)
            .await?;
        let protected_invocation_ids = rebuild
            .interval_segments
            .iter()
            .map(|segment| segment.invocation_row_id)
            .collect::<HashSet<_>>();
        delete_long_term_projection_interval_state_for_range_except(
            pool,
            rebuild.start_epoch.saturating_mul(1000),
            rebuild.end_epoch.saturating_mul(1000),
            &protected_invocation_ids,
            control,
        )
        .await?;
        delete_long_term_projection_legacy_interval_state_for_date(
            pool,
            &rebuild.bucket_date,
            control,
        )
    ...
    let _permit = control.begin("long_term_projection_rollup_replace")?;
    let mut transaction = pool.begin().await?;
    for rebuild in rebuilds {
    ...
    if let Some(cursor) = next_cursor {
        sqlx::query(
            "INSERT INTO long_term_projection_state (consumer, cursor_row_id, interval_state_version, last_flush_at, last_error) VALUES (?1, ?2, ?3, datetime('now'), NULL) ON CONFLICT(consumer) DO UPDATE SET cursor_row_id = MAX(long_term_projection_state.cursor_row_id, excluded.cursor_row_id), interval_state_version = MAX(long_term_projection_state.interval_state_version, excluded.interval_state_version), last_flush_at = excluded.last_flush_at, last_error = NULL, updated_at = datetime('now')",
        )
        .bind(LONG_TERM_PROJECTION_CONSUMER)
        .bind(cursor)
        .bind(LONG_TERM_PROJECTION_INTERVAL_BASELINE_VERSION)
...
async fn delete_long_term_projection_interval_state_for_range(
    pool: &Pool<Sqlite>,
    start_ms: i64,
    end_ms: i64,
    control: LongTermProjectionWriteControl<'_>,
) -> Result<LongTermProjectionWriteBatches> {
    delete_long_term_projection_interval_state_for_range_except(
        pool,
        start_ms,
        end_ms,
...
async fn delete_long_term_projection_interval_state_for_range_except(
    pool: &Pool<Sqlite>,
    start_ms: i64,
    end_ms: i64,
    protected_invocation_ids: &HashSet<i64>,
    control: LongTermProjectionWriteControl<'_>,
) -> Result<LongTermProjectionWriteBatches> {
    let mut batches = LongTermProjectionWriteBatches::default();
    loop {
        let _permit = control.begin("long_term_projection_interval_delete")?;
        let mut transaction = pool.begin().await?;
        let mut query = QueryBuilder::<Sqlite>::new(
            "DELETE FROM long_term_projection_interval_state WHERE rowid IN (SELECT rowid FROM long_term_projection_interval_state WHERE interval_start_ms < ",
        );
        query
            .push_bind(end_ms)
            .push(" AND interval_end_ms > ")
            .push_bind(start_ms);
        if !protected_invocation_ids.is_empty() {
            query.push(" AND invocation_row_id NOT IN (");
            let mut separated = query.separated(", ");
            for invocation_row_id in protected_invocation_ids {
                separated.push_bind(invocation_row_id);
            }
            query.push(")");
        }
        query
            .push(" LIMIT ")
            .push_bind(LONG_TERM_PROJECTION_WRITE_BATCH_ROWS as i64)
            .push(")");
        let deleted = query
            .build()
            .execute(&mut *transaction)
        ...
        drop(_permit);
        control.committed();
        batches.record(deleted);
        if deleted < LONG_TERM_PROJECTION_WRITE_BATCH_ROWS as u64 {
            break;
        }
    }
    Ok(batches)
...
async fn delete_long_term_projection_legacy_interval_state_for_date(
    pool: &Pool<Sqlite>,
    bucket_date: &str,
    control: LongTermProjectionWriteControl<'_>,
) -> Result<LongTermProjectionWriteBatches> {
    let mut batches = LongTermProjectionWriteBatches::default();
    loop {
        let _permit = control.begin("long_term_projection_legacy_interval_date_delete")?;
        let mut transaction = pool.begin().await?;
        let deleted = match sqlx::query(
            "DELETE FROM long_term_projection_intervals WHERE rowid IN (SELECT rowid FROM long_term_projection_intervals WHERE bucket_date = ?1 LIMIT ?2)",
        )
        .bind(bucket_date)
...
async fn migrate_legacy_long_term_projection_intervals(
    pool: &Pool<Sqlite>,
    max_invocations: usize,
    control: LongTermProjectionWriteControl<'_>,
) -> Result<LongTermProjectionLegacyMigration> {
    let rows = match sqlx::query_as::<_, LongTermProjectionIntervalStateRow>(
        "SELECT invocation_row_id, COALESCE(MAX(CASE WHEN dimension = 'model' THEN series_key END), 'other') AS model_series_key, COALESCE(MAX(CASE WHEN dimension = 'upstream' THEN series_key END), 'other') AS upstream_series_key, MIN(interval_start_ms) AS interval_start_ms, MAX(interval_end_ms) AS interval_end_ms FROM long_term_projection_intervals GROUP BY invocation_row_id ORDER BY invocation_row_id LIMIT ?1",
    )
    .bind(max_invocations as i64)
    .fetch_all(pool)
    .await
    {
        Ok(rows) => rows,
        Err(error) if error.to_string().contains("no such table") => {
            return Ok(LongTermProjectionLegacyMigration {
                complete: true,
                ..Default::default()
            });
        }
    ...
    let segments = rows
        ...
        .map(|row| LongTermProjectionIntervalSegment {
            ...
            model_series_key: row.model_series_key,
            upstream_series_key: row.upstream_series_key,
            interval_start_ms: row.interval_start_ms,
            interval_end_ms: row.interval_end_ms,
        })
        .collect::<Vec<_>>();
    let inserted = insert_long_term_projection_interval_segments(pool, &segments, control).await?;
    let invocation_ids = segments
        .iter()
        .map(|segment| segment.invocation_row_id)
        .collect::<Vec<_>>();
    let deleted = delete_legacy_long_term_projection_intervals_for_invocations(
        pool,
        &invocation_ids,
        LONG_TERM_PROJECTION_LEGACY_CLEANUP_BATCHES_PER_FLUSH,
        control,
    )
    .await?;
    let complete = match sqlx::query_scalar::<_, i64>(
        "SELECT NOT EXISTS(SELECT 1 FROM long_term_projection_intervals LIMIT 1)",
    )
    .fetch_one(pool)
    .await
    {
        Ok(empty) => empty != 0,
        Err(error) if error.to_string().contains("no such table") => true,
        Err(error) => return Err(error.into()),
    };
    Ok(LongTermProjectionLegacyMigration {
        deleted,
        migrated_invocations: inserted.inserted.len(),
        complete,
    })
}

async fn delete_legacy_long_term_projection_intervals_for_invocations(
    pool: &Pool<Sqlite>,
    invocation_ids: &[i64],
    max_batches: usize,
    control: LongTermProjectionWriteControl<'_>,
    ...
    if invocation_ids.is_empty() {
        return Ok(LongTermProjectionWriteBatches::default());
    }

    let mut batches = LongTermProjectionWriteBatches::default();
    for _ in 0..max_batches {
        let remaining =
            match legacy_long_term_projection_intervals_exist_for_invocations(pool, invocation_ids)
                .await
            {
        ...
        let _permit = control.begin("long_term_projection_legacy_migration_cleanup")?;
        let mut transaction = pool.begin().await?;
        let mut query = QueryBuilder::<Sqlite>::new(
            "DELETE FROM long_term_projection_intervals WHERE rowid IN (SELECT rowid FROM long_term_projection_intervals WHERE invocation_row_id IN (",
        );
        {
            let mut separated = query.separated(", ");
            for invocation_id in invocation_ids {
                separated.push_bind(invocation_id);
            }
        }
        query
            .push(") LIMIT ")
...
async fn legacy_long_term_projection_intervals_exist_for_invocations(
    pool: &Pool<Sqlite>,
    invocation_ids: &[i64],
) -> Result<bool> {
    let mut query = QueryBuilder::<Sqlite>::new(
        "SELECT EXISTS(SELECT 1 FROM long_term_projection_intervals WHERE invocation_row_id IN (",
    );
    {
        let mut separated = query.separated(", ");
        for invocation_id in invocation_ids {
...
async fn mark_long_term_projection_interval_state_migrated(
    pool: &Pool<Sqlite>,
    control: LongTermProjectionWriteControl<'_>,
) -> Result<()> {
    let _permit = control.begin("long_term_projection_interval_migration_complete")?;
    sqlx::query(
        "INSERT INTO long_term_projection_state (consumer, cursor_row_id, interval_state_version, last_flush_at) VALUES (?1, 0, ?2, datetime('now')) ON CONFLICT(consumer) DO UPDATE SET interval_state_version = MAX(long_term_projection_state.interval_state_version, excluded.interval_state_version), updated_at = datetime('now')",
    )
    .bind(LONG_TERM_PROJECTION_CONSUMER)
    .bind(LONG_TERM_PROJECTION_INTERVAL_STATE_VERSION)
    .execute(pool)
...
async fn delete_long_term_projection_hourly_rollups_before(
    pool: &Pool<Sqlite>,
    ...
    let mut batches = LongTermProjectionWriteBatches::default();
    loop {
        let _permit = control.begin("long_term_projection_hourly_retention")?;
        let mut transaction = pool.begin().await?;
...
async fn delete_legacy_long_term_projection_hourly_intervals_before(
    pool: &Pool<Sqlite>,
    ...
    let mut batches = LongTermProjectionWriteBatches::default();
    loop {
        let _permit = control.begin("long_term_projection_legacy_hourly_retention")?;
        let mut transaction = pool.begin().await?;
        let deleted = match sqlx::query(
...
fn collect_long_term_projection_interval_segments(
    ...
    let Some(interval_start_ms) = parse_long_term_timestamp_ms(&row.occurred_at) else {
        return Vec::new();
    };
    let Some(interval_end_ms) = row
        .t_total_ms
        .filter(|value| value.is_finite() && *value > 0.0)
        .map(|value| interval_start_ms.saturating_add(value.round() as i64))
    else {
        return Vec::new();
    };
    if !is_success_status(row.status.as_deref(), row.error_message.as_deref())
    ...
    let model = normalize_long_term_model(row);
    let reasoning = normalize_long_term_reasoning(row.reasoning_effort.as_deref());
    let (upstream_series_key, _) = normalize_long_term_upstream(row);
    vec![LongTermProjectionIntervalSegment {
        invocation_row_id: row.id,
        model_series_key: long_term_model_series_key(&model, &reasoning),
        upstream_series_key,
        interval_start_ms,
        interval_end_ms,
    }]
...
async fn load_long_term_projection_interval_index(
    ...
    let ranges = dates
        ...
        .filter_map(|date| {
            NaiveDate::parse_from_str(date, "%Y-%m-%d")
                .ok()
                .and_then(long_term_day_epoch_bounds)
                .map(|(start, end)| {
                    (
                        date.clone(),
                        start.saturating_mul(1000),
    ...
    let mut builder = QueryBuilder::<Sqlite>::new(
        "SELECT invocation_row_id, model_series_key, upstream_series_key, interval_start_ms, interval_end_ms FROM long_term_projection_interval_state WHERE ",
    );
    builder.push("(");
    for (index, (date, start_ms, end_ms)) in ranges.iter().enumerate() {
        if index > 0 {
            builder.push(" OR ");
        }
        builder
            .push("interval_start_ms < ")
            .push_bind(*end_ms)
            .push(" AND interval_end_ms > ")
            .push_bind(*start_ms)
            .push(" AND NOT EXISTS (SELECT 1 FROM long_term_projection_bucket_state hidden WHERE hidden.bucket_date = ")
            .push_bind(date)
            .push(" AND hidden.interval_baseline_ready = 0)");
    }
    builder.push(")");
    let rows = builder
        .build_query_as::<LongTermProjectionIntervalStateRow>()
        .fetch_all(pool)
        .await?;
    let mut index = HashMap::new();
    for row in rows {
        add_long_term_projection_interval_to_index(
            &mut index,
    ...
    let mut legacy_builder = QueryBuilder::<Sqlite>::new(
        "SELECT legacy.bucket_kind, legacy.bucket_key, legacy.dimension, legacy.series_key, legacy.interval_start_ms, legacy.interval_end_ms FROM long_term_projection_intervals legacy WHERE NOT EXISTS (SELECT 1 FROM long_term_projection_interval_state canonical WHERE canonical.invocation_row_id = legacy.invocation_row_id) AND NOT EXISTS (SELECT 1 FROM long_term_projection_bucket_state hidden WHERE hidden.bucket_date = legacy.bucket_date AND hidden.interval_baseline_ready = 0) AND legacy.bucket_date IN (",
    );
    {
        let mut separated = legacy_builder.separated(", ");
        for date in dates {
            separated.push_bind(date);
        }
    }
    legacy_builder.push(")");
    let legacy_rows = legacy_builder
...
fn long_term_projection_interval_key_covers_date(
    key: &LongTermProjectionIntervalKey,
    dates: &HashSet<String>,
) -> bool {
    match key.bucket_kind {
        "daily" => dates.contains(&key.bucket_key),
        "hourly" => key
            .bucket_key
            .parse::<i64>()
            .ok()
            .and_then(projection_bucket_date_for_hour)
            .is_some_and(|date| dates.contains(&date)),
        _ => false,
    }
}

async fn refresh_long_term_projection_interval_index_dates(
    pool: &Pool<Sqlite>,
    ...
    let durable_index = load_long_term_projection_interval_index(pool, dates).await?;
    index.retain(|key, _| !long_term_projection_interval_key_covers_date(key, dates));
    index.extend(durable_index);
    Ok(())
}
...
async fn insert_long_term_projection_interval_segments(
    ...
    control: LongTermProjectionWriteControl<'_>,
) -> Result<LongTermProjectionIntervalInsert> {
    let mut result = LongTermProjectionIntervalInsert::default();
    for batch in segments.chunks(LONG_TERM_PROJECTION_WRITE_BATCH_ROWS) {
        let _permit = control.begin("long_term_projection_interval_insert")?;
        let mut transaction = pool.begin().await?;
        for segment in batch {
            // Existing installations use a composite interval key. Remove the prior
            // representation first so a changed duration or date cannot leave a stale row.
            sqlx::query(
                "DELETE FROM long_term_projection_interval_state WHERE invocation_row_id = ?1",
            )
            .bind(segment.invocation_row_id)
            .execute(&mut *transaction)
            .await?;
            sqlx::query(
                "INSERT INTO long_term_projection_interval_state (invocation_row_id, model_series_key, upstream_series_key, interval_start_ms, interval_end_ms) VALUES (?1, ?2, ?3, ?4, ?5)",
            )
            .bind(segment.invocation_row_id)
            ...
            .execute(&mut *transaction)
            .await?;
            result.inserted.push(segment.clone());
        }
        transaction.commit().await?;
        drop(_permit);
        control.committed();
        result.transactions = result.transactions.saturating_add(1);
        result.largest_transaction_rows = result.largest_transaction_rows.max(batch.len());
    }
    Ok(result)
}

fn add_long_term_projection_interval_to_index(
    index: &mut HashMap<LongTermProjectionIntervalKey, LongTermProjectionIntervalUnion>,
    segment: &LongTermProjectionIntervalSegment,
    allowed_dates: Option<&HashSet<String>>,
) {
    let dimensions = [
        ("overall", "overall"),
        ("model", segment.model_series_key.as_str()),
        ("upstream", segment.upstream_series_key.as_str()),
    ];
    let mut hour_start_ms =
        segment.interval_start_ms.div_euclid(LONG_TERM_HOUR_MS) * LONG_TERM_HOUR_MS;
    while hour_start_ms < segment.interval_end_ms {
        let interval_end_ms = (hour_start_ms + LONG_TERM_HOUR_MS).min(segment.interval_end_ms);
        if let Some(bucket_date) = projection_bucket_date_for_hour(hour_start_ms / 1000)
            && allowed_dates.is_none_or(|dates| dates.contains(&bucket_date))
        {
            for (dimension, series_key) in dimensions {
                index
                    .entry(projection_interval_key(
                        "hourly",
                        (hour_start_ms / 1000).to_string(),
                        dimension.to_string(),
                        series_key.to_string(),
                    ))
                    .or_default()
                    .add(
                        hour_start_ms.max(segment.interval_start_ms),
                        interval_end_ms,
                    );
            }
        }
        hour_start_ms = hour_start_ms.saturating_add(LONG_TERM_HOUR_MS);
    }

    let Some(mut bucket_date) = Shanghai
    ...
    loop {
        let Some(day_start) = bucket_date
            .and_hms_opt(0, 0, 0)
            .and_then(|value| Shanghai.from_local_datetime(&value).single())
        else {
            return;
        ...
        if day_start_ms >= segment.interval_end_ms {
            return;
        }
        let day_end_ms = day_start
            .checked_add_signed(ChronoDuration::days(1))
            .map(|value| value.timestamp_millis())
            .unwrap_or(segment.interval_end_ms);
        let interval_start_ms = segment.interval_start_ms.max(day_start_ms);
        let interval_end_ms = segment.interval_end_ms.min(day_end_ms);
        let date_key = bucket_date.to_string();
        if interval_end_ms > interval_start_ms
            && allowed_dates.is_none_or(|dates| dates.contains(&date_key))
        {
            for (dimension, series_key) in dimensions {
                index
                    .entry(projection_interval_key(
                    ...
                    .or_default()
                    .add(interval_start_ms, interval_end_ms);
            }
        }
        let Some(next_date) = bucket_date.succ_opt() else {
            return;
        };
...
async fn apply_long_term_projection_incremental_with_runtime_and_control(
    ...
    insert_long_term_projection_interval_segments(pool, segments, control).await?;
    refresh_long_term_projection_interval_index_dates(pool, &mut interval_index, &dates).await?;
    let _permit = control.begin("long_term_projection_incremental_rollup")?;
    let mut tx = pool.begin().await?;
    ...
    sqlx::query(
        "INSERT INTO long_term_projection_state (consumer, cursor_row_id, interval_state_version, last_flush_at, last_error) VALUES (?1, ?2, ?3, datetime('now'), NULL) ON CONFLICT(consumer) DO UPDATE SET cursor_row_id = excluded.cursor_row_id, interval_state_version = MAX(long_term_projection_state.interval_state_version, excluded.interval_state_version), last_flush_at = excluded.last_flush_at, last_error = NULL, updated_at = datetime('now')",
    )
    .bind(LONG_TERM_PROJECTION_CONSUMER)
    .bind(next_cursor)
    .bind(LONG_TERM_PROJECTION_INTERVAL_BASELINE_VERSION)
...
async fn ensure_long_term_projection_schema(pool: &Pool<Sqlite>) -> Result<()> {
    ...
    let interval_state_version_migration = sqlx::query(
        "ALTER TABLE long_term_projection_state ADD COLUMN interval_state_version INTEGER NOT NULL DEFAULT 0",
    )
    .execute(pool)
    .await;
    if let Err(error) = interval_state_version_migration
    ...
    sqlx::query(
        ...
        CREATE TABLE IF NOT EXISTS long_term_projection_interval_state (
            ...
            interval_start_ms INTEGER NOT NULL,
            interval_end_ms INTEGER NOT NULL,
            PRIMARY KEY (invocation_row_id, interval_start_ms, interval_end_ms)
        )
        "#,
    )
    .execute(pool)
    .await
    .context("failed to ensure canonical long-term projection interval state table")?;
    sqlx::query(
        "CREATE INDEX IF NOT EXISTS idx_long_term_projection_interval_state_start ON long_term_projection_interval_state (interval_start_ms, interval_end_ms)",
    )
    .execute(pool)
    .await
    .context("failed to ensure canonical long-term projection interval state index")?;
    Ok(())
}
...
mod tests {
    ...
    #[tokio::test]
    async fn projection_interval_state_persists_one_canonical_row_per_invocation() {
        let pool = SqlitePoolOptions::new()
            .max_connections(1)
            .connect("sqlite::memory:")
            .await
        ...
        for id in 1..=256_i64 {
            sqlx::query(
                "INSERT INTO codex_invocations (id, invoke_id, occurred_at, status, model, payload, total_tokens, t_total_ms) VALUES (?1, ?2, '2026-07-26 10:00:00', 'success', 'gpt-5', '{\"upstreamAccountId\":42}', 100, 1000)",
            )
            .bind(id)
            .bind(format!("canonical-{id}"))
            .execute(&pool)
            .await
            .expect("source invocation");
        }
        ...
        let rebuild = build_long_term_projection_date_rebuild(&pool, "2026-07-26")
            .await
            .expect("date rebuild");
        assert_eq!(rebuild.interval_segments.len(), 256);
        commit_long_term_projection_date_rebuilds(&pool, &[rebuild], Some(256), &[], false)
            .await
            .expect("projection commit");

        let canonical_rows = sqlx::query_scalar::<_, i64>(
            "SELECT COUNT(*) FROM long_term_projection_interval_state",
        )
        .fetch_one(&pool)
        .await
        .expect("canonical state rows");
        let legacy_rows =
            sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM long_term_projection_intervals")
                .fetch_one(&pool)
                .await
                .expect("legacy state rows");
        assert_eq!(canonical_rows, 256);
        assert_eq!(legacy_rows, 0);
    }

    #[tokio::test]
    async fn projection_interval_inserts_are_bounded_to_512_rows_per_transaction() {
        let pool = SqlitePoolOptions::new()
            .max_connections(1)
            .connect("sqlite::memory:")
            .await
        ...
        let segments = (1..=1_025_i64)
            .map(|invocation_row_id| LongTermProjectionIntervalSegment {
                invocation_row_id,
                model_series_key: "model:v2:gpt-5:".to_string(),
                upstream_series_key: "account:42".to_string(),
                interval_start_ms: invocation_row_id * 1_000,
                interval_end_ms: invocation_row_id * 1_000 + 1_000,
            })
            .collect::<Vec<_>>();
        let gate = crate::db_pressure::DbPressureGate::new(1, Duration::from_secs(1));
        let inserted = insert_long_term_projection_interval_segments(
            &pool,
            &segments,
            LongTermProjectionWriteControl {
                cancel: None,
        ...
        .await
        .expect("bounded canonical insert");

        assert_eq!(inserted.inserted.len(), 1_025);
        assert_eq!(inserted.transactions, 3);
        assert_eq!(
            inserted.largest_transaction_rows,
            LONG_TERM_PROJECTION_WRITE_BATCH_ROWS
        );
        assert_eq!(
            sqlx::query_scalar::<_, i64>(
                "SELECT COUNT(*) FROM long_term_projection_interval_state",
        ...
        let replacement = LongTermProjectionIntervalSegment {
            invocation_row_id: 1,
            model_series_key: "model:v2:gpt-5.1:".to_string(),
            upstream_series_key: "account:99".to_string(),
            interval_start_ms: 9_000,
            interval_end_ms: 12_000,
        };
        ...
        .await
        .expect("replace canonical interval");
        assert_eq!(
            sqlx::query_scalar::<_, i64>(
                "SELECT COUNT(*) FROM long_term_projection_interval_state",
            )
            .fetch_one(&pool)
            .await
            .expect("canonical row count after replacement"),
            1_025
        );
        let stored = sqlx::query_as::<_, (String, String, i64, i64)>(
            "SELECT model_series_key, upstream_series_key, interval_start_ms, interval_end_ms FROM long_term_projection_interval_state WHERE invocation_row_id = 1",
        )
        .fetch_one(&pool)
        .await
        .expect("replaced canonical interval");
        assert_eq!(stored.0, replacement.model_series_key);
        assert_eq!(stored.1, replacement.upstream_series_key);
        assert_eq!(stored.2, replacement.interval_start_ms);
        assert_eq!(stored.3, replacement.interval_end_ms);
    }

    #[tokio::test]
    async fn projection_interval_state_recovers_after_reopening_durable_storage() {
        let path = std::env::temp_dir().join(format!(
            "codex-vibe-monitor-long-term-restart-{}-{}.sqlite",
            std::process::id(),
            Utc::now().timestamp_nanos_opt().unwrap_or_default(),
        ));
        ...
        let (day_start_epoch, _) = long_term_day_epoch_bounds(date).expect("day bounds");
        let segment = LongTermProjectionIntervalSegment {
            invocation_row_id: 17,
            model_series_key: "model:v2:gpt-5:".to_string(),
            upstream_series_key: "account:42".to_string(),
            interval_start_ms: (day_start_epoch + 10 * 60 * 60) * 1000,
            interval_end_ms: (day_start_epoch + 10 * 60 * 60) * 1000 + 1_000,
        };
        let gate = crate::db_pressure::DbPressureGate::new(1, Duration::from_secs(1));
        insert_long_term_projection_interval_segments(
            &pool,
        ...
        .await
        .expect("durable interval insert");
        sqlx::query(
            "INSERT INTO long_term_projection_state (consumer, cursor_row_id, interval_state_version) VALUES (?1, 17, ?2)",
        )
        .bind(LONG_TERM_PROJECTION_CONSUMER)
        .bind(LONG_TERM_PROJECTION_INTERVAL_STATE_VERSION)
        .execute(&pool)
        ...
        let reopened = SqlitePoolOptions::new()
            ...
            .connect_with(
                url.parse::<SqliteConnectOptions>()
                    .expect("reopened durable sqlite URL")
                    .create_if_missing(true),
            )
            .await
            .expect("reopened durable pool");
        let index =
            load_long_term_projection_interval_index(&reopened, &HashSet::from([date.to_string()]))
                .await
                .expect("reloaded interval index");
        let daily_overall = index
            .get(&projection_interval_key(
                "daily",
                date.to_string(),
                "overall".to_string(),
                "overall".to_string(),
            ))
            .expect("reloaded daily interval");
        assert_eq!(daily_overall.duration_ms, 1_000);
        assert_eq!(daily_overall.sample_count, 1);
        let (cursor, version) = sqlx::query_as::<_, (i64, i64)>(
            "SELECT cursor_row_id, interval_state_version FROM long_term_projection_state WHERE consumer = ?1",
        )
        .bind(LONG_TERM_PROJECTION_CONSUMER)
        .fetch_one(&reopened)
        .await
        .expect("reloaded cursor");
        assert_eq!(cursor, 17);
        assert_eq!(version, LONG_TERM_PROJECTION_INTERVAL_STATE_VERSION);
        reopened.close().await;
        std::fs::remove_file(path).expect("remove durable test database");
    }

    #[tokio::test]
    async fn projection_restart_replays_rows_after_the_durable_cursor() {
        let path = std::env::temp_dir().join(format!(
            "codex-vibe-monitor-long-term-cursor-restart-{}-{}.sqlite",
            std::process::id(),
            Utc::now().timestamp_nanos_opt().unwrap_or_default(),
        ));
        let url = format!("sqlite://{}", path.to_string_lossy());
        ...
        ensure_long_term_stats_schema(&pool)
            ...
            .expect("long-term schema");
        sqlx::query(
            "INSERT INTO codex_invocations (id, invoke_id, occurred_at, status, model, total_tokens, t_total_ms) VALUES (17, 'restart-row', '2026-07-26 10:00:00', 'success', 'gpt-5', 100, 1000)",
        )
        .execute(&pool)
        .await
        .expect("post-cursor invocation");
        sqlx::query(
            "INSERT INTO long_term_projection_state (consumer, cursor_row_id, interval_state_version) VALUES (?1, 16, ?2)",
        )
        .bind(LONG_TERM_PROJECTION_CONSUMER)
        .bind(LONG_TERM_PROJECTION_INTERVAL_STATE_VERSION)
        .execute(&pool)
        ...
        let reopened = SqlitePoolOptions::new()
            ...
            .await
            .expect("reopened durable pool");
        let rows = load_long_term_projection_terminal_rows(&reopened, 16)
            .await
            .expect("replay rows after restart");
        assert_eq!(rows.iter().map(|row| row.id).collect::<Vec<_>>(), vec![17]);
        let row = &rows[0];
        let mut hourly = HashMap::new();
        let mut daily = HashMap::new();
        let mut statistics_start = None;
        ...
        apply_long_term_projection_incremental_with_runtime(
            &reopened, &runtime, &hourly, &daily, &segments, 17, 1,
        )
        .await
        .expect("publish replayed invocation");
        assert_eq!(
            sqlx::query_scalar::<_, i64>(
                "SELECT cursor_row_id FROM long_term_projection_state WHERE consumer = ?1",
            )
            ...
            .fetch_one(&reopened)
            .await
            .expect("advanced replay cursor"),
            17
        );
        let canonical = sqlx::query_as::<_, (i64, String, String, i64, i64)>(
            "SELECT invocation_row_id, model_series_key, upstream_series_key, interval_start_ms, interval_end_ms FROM long_term_projection_interval_state WHERE invocation_row_id = 17",
        )
        .fetch_one(&reopened)
        .await
        .expect("replayed canonical interval");
        assert_eq!(canonical.0, 17);
        assert!(canonical.1.starts_with("model:v2:"));
        assert_eq!(canonical.2, "other");
        assert_eq!(canonical.4 - canonical.3, 1_000);
        let replayed_rollup = sqlx::query_as::<_, (i64, f64)>(
            "SELECT calls, wall_time_ms FROM long_term_usage_daily WHERE stats_date = '2026-07-26' AND dimension = 'overall' AND series_key = 'overall'",
        )
        .fetch_one(&reopened)
        .await
        .expect("replayed daily rollup");
        assert_eq!(replayed_rollup, (1, 1_000.0));
        reopened.close().await;
        std::fs::remove_file(path).expect("remove durable cursor database");
    }

    #[tokio::test]
    async fn projection_interval_index_refreshes_after_partial_canonical_insert() {
        let pool = SqlitePoolOptions::new()
            .max_connections(1)
            .connect("sqlite::memory:")
            .await
        ...
        ensure_long_term_projection_schema(&pool)
            .await
            .expect("projection schema");
        let date = "2026-07-26".to_string();
        let (day_start_epoch, _) =
            long_term_day_epoch_bounds(NaiveDate::parse_from_str(&date, "%Y-%m-%d").expect("date"))
                .expect("day bounds");
        let segment = LongTermProjectionIntervalSegment {
            invocation_row_id: 7,
            model_series_key: "model:v2:gpt-5:".to_string(),
        ...
        .await
        .expect("canonical interval insert");

        let mut index = HashMap::new();
        let stale_segment = LongTermProjectionIntervalSegment {
            invocation_row_id: 999,
            interval_end_ms: segment.interval_start_ms + 5_000,
            ..segment.clone()
        };
        add_long_term_projection_interval_to_index(
            &mut index,
            &stale_segment,
            Some(&HashSet::from([date.clone()])),
        );
        refresh_long_term_projection_interval_index_dates(
            &pool,
            &mut index,
            &HashSet::from([date.clone()]),
        )
        .await
        .expect("refresh durable interval index");
        ...
        let daily_overall = index
            ...
            .expect("refreshed daily overall interval");
        assert_eq!(daily_overall.duration_ms, 1_000);
        assert_eq!(daily_overall.sample_count, 1);
    }

    #[tokio::test]
    async fn incremental_projection_retries_after_cancelled_interval_commit() {
        let pool = SqlitePoolOptions::new()
            .max_connections(1)
            .connect("sqlite::memory:")
            .await
        ...
        let accumulator = LongTermAccumulator {
            calls: 1,
            token_total: 100,
            token_samples: 1,
            ..Default::default()
        ...
        let daily = HashMap::from([(
            (date.clone(), "overall".to_string(), "overall".to_string()),
            LongTermBucket {
                bucket_start_epoch: day_start_epoch,
                dimension: "overall".to_string(),
                series_key: "overall".to_string(),
                display_name: "All".to_string(),
                reasoning_effort: String::new(),
                stats_date: Some(date.clone()),
                accumulator,
            },
        )]);
        let runtime = Arc::new(Mutex::new(LongTermProjectionRuntime::default()));
        ...
        let first = apply_long_term_projection_incremental_with_runtime_and_control(
            &pool,
            &runtime,
            &HashMap::new(),
            &daily,
            ...
            LongTermProjectionWriteControl {
                cancel: Some(&cancel),
                gate: &gate,
                after_commit: Some(&cancel_after_commit),
                outer_admitted: false,
            },
        )
        .await;
        ...
        assert_eq!(
            ...
            .fetch_one(&pool)
            .await
            .expect("committed interval"),
            1
        );
        assert_eq!(
            sqlx::query_scalar::<_, i64>(
                "SELECT COALESCE(MAX(cursor_row_id), 0) FROM long_term_projection_state WHERE consumer = ?1",
            )
            .bind(LONG_TERM_PROJECTION_CONSUMER)
            .fetch_one(&pool)
            .await
            .expect("cursor after cancelled rollup"),
            0
        );
        ...
        .await
        .expect("retry incremental projection");
        assert_eq!(
            sqlx::query_scalar::<_, i64>(
                "SELECT cursor_row_id FROM long_term_projection_state WHERE consumer = ?1",
            )
            ...
            .fetch_one(&pool)
            .await
            .expect("cursor after retry"),
            7
        );
        let wall_time = sqlx::query_scalar::<_, f64>(
            "SELECT wall_time_ms FROM long_term_usage_daily WHERE stats_date = ?1 AND dimension = 'overall' AND series_key = 'overall'",
        )
        .bind(&date)
        .fetch_one(&pool)
        .await
        .expect("retried rollup");
        assert_eq!(wall_time, 1_000.0);
    }

    #[tokio::test]
    async fn legacy_projection_intervals_migrate_to_canonical_state_in_bounded_transactions() {
        let pool = SqlitePoolOptions::new()
            .max_connections(1)
            .connect("sqlite::memory:")
            .await
        ...
        let date = NaiveDate::from_ymd_opt(2026, 7, 26).expect("fixed date");
        let (day_start_epoch, _) = long_term_day_epoch_bounds(date).expect("day bounds");
        let hour_start_epoch = day_start_epoch + 10 * 60 * 60;
        let start_ms = hour_start_epoch * 1000;
        let end_ms = start_ms + 1_000;
        let legacy_keys = [
            ("hourly", hour_start_epoch.to_string(), "overall", "overall"),
            (
                "hourly",
                hour_start_epoch.to_string(),
                "model",
                "model:v2:gpt-5:",
            ),
            (
            ...
            ("daily", date.to_string(), "overall", "overall"),
            ("daily", date.to_string(), "model", "model:v2:gpt-5:"),
            ("daily", date.to_string(), "upstream", "account:42"),
        ];
        for invocation_row_id in 1..=256_i64 {
            for (bucket_kind, bucket_key, dimension, series_key) in &legacy_keys {
                sqlx::query(
                    "INSERT INTO long_term_projection_intervals (bucket_kind, bucket_date, bucket_key, dimension, series_key, invocation_row_id, interval_start_ms, interval_end_ms) VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7, ?8)",
                )
                .bind(bucket_kind)
                .bind(date.to_string())
                .bind(bucket_key)
                ...
                .execute(&pool)
                .await
                .expect("legacy interval");
            }
        }

        let gate = crate::db_pressure::DbPressureGate::new(1, Duration::from_secs(1));
        let migration = migrate_legacy_long_term_projection_intervals(
            &pool,
            LONG_TERM_PROJECTION_LEGACY_MIGRATION_INVOCATIONS_PER_FLUSH,
            LongTermProjectionWriteControl {
                cancel: None,
        ...
        assert_eq!(migration.migrated_invocations, 256);
        assert_eq!(migration.deleted.rows, 1_536);
        assert_eq!(migration.deleted.transactions, 3);
        assert_eq!(
            migration.deleted.largest_transaction_rows,
            LONG_TERM_PROJECTION_WRITE_BATCH_ROWS
        );
        assert_eq!(
            sqlx::query_scalar::<_, i64>(
        ...
        let index =
            load_long_term_projection_interval_index(&pool, &HashSet::from([date.to_string()]))
                .await
                .expect("canonical interval index");
        let daily_overall = index
            .get(&projection_interval_key(
                "daily",
                date.to_string(),
            ...
            .expect("daily overall interval");
        assert_eq!(daily_overall.duration_ms, 1_000);
        assert_eq!(daily_overall.sample_count, 256);
    }

    #[tokio::test]
    async fn legacy_projection_migration_yields_after_four_cleanup_batches() {
        let pool = SqlitePoolOptions::new()
            .max_connections(1)
            .connect("sqlite::memory:")
            .await
        ...
        ensure_long_term_projection_schema(&pool)
            ...
            .expect("projection schema");
        let legacy_keys = [
            ("hourly", "1785024000", "overall", "overall"),
            ("hourly", "1785024000", "model", "model:v2:gpt-5:"),
            ("hourly", "1785024000", "upstream", "account:42"),
            ("daily", "2026-07-26", "overall", "overall"),
            ("daily", "2026-07-26", "model", "model:v2:gpt-5:"),
            ("daily", "2026-07-26", "upstream", "account:42"),
        ];
        for invocation_row_id in 1..=512_i64 {
            for (bucket_kind, bucket_key, dimension, series_key) in &legacy_keys {
                sqlx::query(
                    "INSERT INTO long_term_projection_intervals (bucket_kind, bucket_date, bucket_key, dimension, series_key, invocation_row_id, interval_start_ms, interval_end_ms) VALUES (?1, '2026-07-26', ?2, ?3, ?4, ?5, 0, 1)",
                )
                .bind(bucket_kind)
                .bind(bucket_key)
                .bind(dimension)
        ...
        .await
        .expect("bounded legacy migration");

        assert!(!migration.complete);
        assert_eq!(migration.deleted.transactions, 4);
        assert_eq!(migration.deleted.rows, 2_048);
        assert_eq!(
            migration.deleted.largest_transaction_rows,
            LONG_TERM_PROJECTION_WRITE_BATCH_ROWS
        );
        assert_eq!(
            sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM long_term_projection_intervals",)
                .fetch_one(&pool)
                .await
                .expect("remaining legacy rows"),
            1_024
        );
    }

    #[tokio::test]
    async fn projection_interval_state_batches_are_bounded_and_stop_for_pressure_or_cancel() {
        let pool = SqlitePoolOptions::new()
            .max_connections(1)
            .connect("sqlite::memory:")
            .await
        ...
        let date = NaiveDate::from_ymd_opt(2026, 7, 26).expect("fixed date");
        let (start_epoch, end_epoch) = long_term_day_epoch_bounds(date).expect("day bounds");
        let start_ms = start_epoch * 1000;
        let end_ms = end_epoch * 1000;
        for id in 1..=1025_i64 {
            sqlx::query(
                "INSERT INTO long_term_projection_interval_state (invocation_row_id, model_series_key, upstream_series_key, interval_start_ms, interval_end_ms) VALUES (?1, 'model:v2:gpt-5:', 'other', ?2, ?3)",
            )
            .bind(id)
            .bind(start_ms)
            .bind(start_ms + 1)
        ...
        let gate = crate::db_pressure::DbPressureGate::new(1, Duration::from_secs(1));
        let control = LongTermProjectionWriteControl {
            cancel: None,
            gate: &gate,
            after_commit: None,
            outer_admitted: false,
        ...
        let batches =
            delete_long_term_projection_interval_state_for_range(&pool, start_ms, end_ms, control)
                .await
                .expect("batched delete");
        assert_eq!(batches.rows, 1025);
        assert_eq!(batches.transactions, 3);
        assert_eq!(
            batches.largest_transaction_rows,
            LONG_TERM_PROJECTION_WRITE_BATCH_ROWS
        );

        sqlx::query(
            "INSERT INTO long_term_projection_interval_state (invocation_row_id, model_series_key, upstream_series_key, interval_start_ms, interval_end_ms) VALUES (2000, 'model:v2:gpt-5:', 'other', ?1, ?2)",
        )
        .bind(start_ms)
        .bind(start_ms + 1)
        .execute(&pool)
        .await
        .expect("interval state for cancellation");
        let cancel = CancellationToken::new();
        cancel.cancel();
        let cancelled = delete_long_term_projection_interval_state_for_range(
            &pool,
            start_ms,
            end_ms,
            LongTermProjectionWriteControl {
        ...
        .await
        .expect_err("cancelled deletion");
        assert!(cancelled.to_string().contains("cancelled"));

        let held = gate
            .try_begin_background("test_projection_pressure")
            .expect("hold pressure gate");
        let pressure = delete_long_term_projection_interval_state_for_range(
            &pool,
            start_ms,
            end_ms,
            LongTermProjectionWriteControl {
        ...
        .await
        .expect_err("pressure-deferred deletion");
        assert!(pressure.to_string().contains("database pressure"));
        drop(held);
        assert_eq!(
            sqlx::query_scalar::<_, i64>(
                "SELECT COUNT(*) FROM long_term_projection_interval_state",
            ...
            .fetch_one(&pool)
            .await
            .expect("remaining interval state"),
            1
        );
    }

    #[tokio::test]
    async fn projection_interval_batches_stop_after_a_committed_batch() {
        let pool = SqlitePoolOptions::new()
            .max_connections(1)
            .connect("sqlite::memory:")
            .await
        ...
        ensure_long_term_projection_schema(&pool)
            .await
            .expect("projection schema");
        for id in 1..=1_025_i64 {
            sqlx::query(
                "INSERT INTO long_term_projection_interval_state (invocation_row_id, model_series_key, upstream_series_key, interval_start_ms, interval_end_ms) VALUES (?1, 'model:v2:gpt-5:', 'other', 0, 1)",
            )
            .bind(id)
            .execute(&pool)
            .await
        ...
        let cancelled = delete_long_term_projection_interval_state_for_range(
            ...
            LongTermProjectionWriteControl {
                cancel: Some(&cancel),
                gate: &gate,
                after_commit: Some(&cancel_after_first),
                outer_admitted: false,
            },
        )
        .await
        .expect_err("cancel after first committed batch");
        assert!(cancelled.to_string().contains("cancelled"));
        assert_eq!(
            sqlx::query_scalar::<_, i64>(
                "SELECT COUNT(*) FROM long_term_projection_interval_state",
            ...
            .fetch_one(&pool)
            .await
            .expect("remaining after cancellation"),
            513
        );

        for id in 2_000..=3_024_i64 {
            sqlx::query(
                "INSERT INTO long_term_projection_interval_state (invocation_row_id, model_series_key, upstream_series_key, interval_start_ms, interval_end_ms) VALUES (?1, 'model:v2:gpt-5:', 'other', 0, 1)",
            )
            .bind(id)
            .execute(&pool)
            .await
            .expect("pressure interval state");
        }
        let held_permit = Arc::new(std::sync::Mutex::new(
            None::<crate::db_pressure::DbBackgroundPermit>,
        ));
        let pressure_after_first = {
            let held_permit = Arc::clone(&held_permit);
            let gate_ref = &gate;
            move || {
                let permit = gate_ref
                    .try_begin_background("test_projection_mid_batch_pressure")
                    .expect("acquire pressure after first batch");
                *held_permit.lock().expect("held permit mutex") = Some(permit);
            }
        };
        let pressure = delete_long_term_projection_interval_state_for_range(
            &pool,
            ...
            LongTermProjectionWriteControl {
                cancel: None,
                gate: &gate,
                after_commit: Some(&pressure_after_first),
                outer_admitted: false,
            },
        )
        .await
        .expect_err("pressure after first committed batch");
        assert!(pressure.to_string().contains("database pressure"));
        assert_eq!(
            sqlx::query_scalar::<_, i64>(
                "SELECT COUNT(*) FROM long_term_projection_interval_state"
            ...
            .fetch_one(&pool)
            .await
            .expect("remaining after pressure"),
            1_026
        );
        held_permit.lock().expect("held permit mutex").take();
    }

    #[tokio::test]
    async fn legacy_interval_cleanup_stops_after_a_committed_batch() {
        let pool = SqlitePoolOptions::new()
            .max_connections(1)
            .connect("sqlite::memory:")
            .await
        ...
        for id in 1..=1_025_i64 {
            sqlx::query(
                "INSERT INTO long_term_projection_intervals (bucket_kind, bucket_date, bucket_key, dimension, series_key, invocation_row_id, interval_start_ms, interval_end_ms) VALUES ('daily', '2026-07-26', '2026-07-26', 'overall', 'overall', ?1, 0, 1)",
            )
            .bind(id)
            .execute(&pool)
            .await
        ...
        let invocation_ids = (1..=1_025_i64).collect::<Vec<_>>();
        let gate = crate::db_pressure::DbPressureGate::new(1, Duration::from_secs(1));
        let cancel = CancellationToken::new();
        let cancel_after_first = {
            let cancel = cancel.clone();
        ...
        let result = delete_legacy_long_term_projection_intervals_for_invocations(
            &pool,
            &invocation_ids,
            10,
            LongTermProjectionWriteControl {
        ...
        .await
        .expect_err("cancel legacy cleanup after first batch");
        assert!(result.to_string().contains("cancelled"));
        assert_eq!(
            sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM long_term_projection_intervals",)
                .fetch_one(&pool)
                .await
                .expect("remaining legacy intervals"),
            513
        );
    }

    #[tokio::test]
    async fn interrupted_date_rebuild_hides_partial_interval_state_until_retry() {
        let pool = SqlitePoolOptions::new()
            .max_connections(1)
            .connect("sqlite::memory:")
            .await
        ...
        ensure_long_term_stats_schema(&pool)
            .await
            .expect("long-term schema");
        let date = NaiveDate::from_ymd_opt(2026, 7, 26).expect("date");
        let (start_epoch, end_epoch) = long_term_day_epoch_bounds(date).expect("day bounds");
        sqlx::query(
            "INSERT INTO long_term_projection_interval_state (invocation_row_id, model_series_key, upstream_series_key, interval_start_ms, interval_end_ms) VALUES (1, 'model:v2:gpt-5:', 'account:42', ?1, ?2)",
        )
        .bind(start_epoch * 1000 + 1_000)
        .bind(start_epoch * 1000 + 2_000)
        .execute(&pool)
        .await
        .expect("canonical interval");
        sqlx::query(
            "INSERT INTO long_term_projection_bucket_state (bucket_date, interval_baseline_ready) VALUES ('2026-07-26', 1)",
        )
        .execute(&pool)
        .await
        .expect("ready bucket");
        ...
        let gate = crate::db_pressure::DbPressureGate::new(1, Duration::from_secs(1));
        let cancel = CancellationToken::new();
        let cancel_after_invalidate = {
            let cancel = cancel.clone();
            move || cancel.cancel()
        };
        let interrupted = commit_long_term_projection_date_rebuilds_with_control(
            &pool,
            ...
            LongTermProjectionWriteControl {
                cancel: Some(&cancel),
                gate: &gate,
                after_commit: Some(&cancel_after_invalidate),
                outer_admitted: false,
            },
        )
        .await
        .expect_err("interrupt rebuild");
        assert!(interrupted.to_string().contains("cancelled"));
        assert_eq!(
            sqlx::query_scalar::<_, i64>(
                "SELECT interval_baseline_ready FROM long_term_projection_bucket_state WHERE bucket_date = '2026-07-26'",
            )
            .fetch_one(&pool)
            .await
            .expect("hidden bucket state"),
            0
        );
        let hidden_index =
            load_long_term_projection_interval_index(&pool, &HashSet::from([date.to_string()]))
                .await
                .expect("hidden interval index");
        assert!(hidden_index.is_empty());
        ...
        assert_eq!(
            ...
            .fetch_one(&pool)
            .await
            .expect("ready bucket after retry"),
            1
        );
        assert_eq!(
            sqlx::query_scalar::<_, i64>(
            ...
            .fetch_one(&pool)
            .await
            .expect("cleared canonical intervals"),
            0
        );
    }

    #[tokio::test]
    async fn interrupted_date_rebuild_preserves_shared_neighbor_interval() {
        let pool = SqlitePoolOptions::new()
            .max_connections(1)
            .connect("sqlite::memory:")
            .await
        ...
        let next_date = date.succ_opt().expect("next date");
        let (start_epoch, end_epoch) = long_term_day_epoch_bounds(date).expect("day bounds");
        let (next_start_epoch, _) = long_term_day_epoch_bounds(next_date).expect("next bounds");
        let interval_start_ms = (end_epoch * 1000).saturating_sub(30 * 60 * 1000);
        let interval_end_ms = next_start_epoch * 1000 + 30 * 60 * 1000;
        sqlx::query(
            "INSERT INTO long_term_projection_interval_state (invocation_row_id, model_series_key, upstream_series_key, interval_start_ms, interval_end_ms) VALUES (1, 'model:v2:gpt-5:', 'account:42', ?1, ?2)",
        )
        .bind(interval_start_ms)
        .bind(interval_end_ms)
        .execute(&pool)
        .await
        .expect("shared canonical interval");
        sqlx::query(
            "INSERT INTO long_term_projection_bucket_state (bucket_date, interval_baseline_ready) VALUES (?1, 1), (?2, 1)",
        )
        .bind(date.to_string())
        .bind(next_date.to_string())
        .execute(&pool)
        .await
        .expect("ready buckets");
        let rebuild = LongTermProjectionDateRebuild {
            bucket_date: date.to_string(),
            start_epoch,
            end_epoch,
        ...
        let cancel_after_insert = {
            let cancel = cancel.clone();
            let commits = commits.clone();
            move || {
                if commits.fetch_add(1, std::sync::atomic::Ordering::SeqCst) == 1 {
                    cancel.cancel();
                }
            }
        };
        let interrupted = commit_long_term_projection_date_rebuilds_with_control(
            ...
            LongTermProjectionWriteControl {
                cancel: Some(&cancel),
                gate: &gate,
                after_commit: Some(&cancel_after_insert),
                outer_admitted: false,
            },
        )
        .await
        .expect_err("interrupt after replacement publication");
        assert!(interrupted.to_string().contains("cancelled"));
        assert_eq!(
            sqlx::query_scalar::<_, i64>(
            ...
            .fetch_one(&pool)
            .await
            .expect("shared interval remains durable"),
            1
        );
        let neighbor_index = load_long_term_projection_interval_index(
            &pool,
            &HashSet::from([next_date.to_string()]),
        )
        .await
        .expect("neighbor interval index");
        let neighbor_daily = neighbor_index
            .get(&projection_interval_key(
                "daily",
                next_date.to_string(),
                "overall".to_string(),
            ...
            .expect("neighbor daily interval");
        assert_eq!(neighbor_daily.duration_ms, 30 * 60 * 1000);
        assert_eq!(neighbor_daily.sample_count, 1);
    }

    #[tokio::test]
    async fn projection_date_rebuild_uses_canonical_range_seek_and_keeps_rfc3339_rows() {
        let pool = SqlitePoolOptions::new()
            .max_connections(1)
            .connect("sqlite::memory:")
            .await
        ...
        crate::schema::ensure_schema(&pool)
            .await
            .expect("full production schema");
        sqlx::query("DROP INDEX idx_codex_invocations_occurred_at_status")
            .execute(&pool)
            .await
            .expect("drop competing occurred_at index");
        sqlx::query(
            "INSERT INTO codex_invocations (id, invoke_id, occurred_at, status, total_tokens, raw_response) VALUES (1, 'canonical', '2026-07-26 12:00:00', 'success', 100, '')",
        )
        .execute(&pool)
        .await
        .expect("canonical invocation");
        sqlx::query(
            "INSERT INTO codex_invocations (id, invoke_id, occurred_at, status, total_tokens, raw_response) VALUES (2, 'rfc3339', '2026-07-25T16:30:00Z', 'success', 100, '')",
        )
        .execute(&pool)
        .await
        ...
        let query = long_term_projection_live_rows_query(
            "CASE WHEN json_valid(inv.payload) THEN CAST(json_extract(inv.payload, '$.upstreamAccountId') AS INTEGER) END",
        );
        let plan =
        ...
        assert!(plan.iter().any(|(_, _, _, detail)| {
            detail.contains("USING INDEX idx_codex_invocations_occurred_at")
        }));
    }

    #[tokio::test]
    async fn projection_date_rebuild_clears_legacy_intervals_before_migration() {
        let pool = SqlitePoolOptions::new()
            .max_connections(1)
            .connect("sqlite::memory:")
            .await
        ...
        ensure_long_term_stats_schema(&pool)
            ...
            .expect("long-term schema");
        sqlx::query(
            "INSERT INTO long_term_projection_intervals (bucket_kind, bucket_date, bucket_key, dimension, series_key, invocation_row_id, interval_start_ms, interval_end_ms) VALUES ('daily', '2026-07-26', '2026-07-26', 'overall', 'overall', 99, 0, 1)",
        )
        .execute(&pool)
        .await
        .expect("legacy interval");
        ...
        let rebuild = build_long_term_projection_date_rebuild(&pool, "2026-07-26")
            .await
            .expect("empty date rebuild");
        assert!(rebuild.interval_segments.is_empty());
        commit_long_term_projection_date_rebuilds(&pool, &[rebuild], None, &[], false)
            .await
            .expect("rebuild commit");

        assert_eq!(
            sqlx::query_scalar::<_, i64>(
                "SELECT COUNT(*) FROM long_term_projection_intervals WHERE bucket_date = '2026-07-26'",
            )
            .fetch_one(&pool)
            .await
            .expect("legacy interval count"),
            0
        );
        assert_eq!(
            sqlx::query_scalar::<_, i64>(
            ...
            .fetch_one(&pool)
            .await
            .expect("canonical interval count"),
            0
        );
    }
    ...
    #[tokio::test]
    async fn correction_after_canonical_interval_publication_queues_repair() {
        let pool = SqlitePoolOptions::new()
            .max_connections(1)
            .connect("sqlite::memory:")
            .await
        ...
        ensure_long_term_projection_correction_trigger(&pool)
            ...
            .expect("correction trigger");
        sqlx::query(
            "INSERT INTO codex_invocations (id, occurred_at, status, model, t_total_ms) VALUES (1, '2026-07-26 10:00:00', 'success', 'before', 1000)",
        )
        .execute(&pool)
        .await
        .expect("invocation");
        ...
        sqlx::query(
            "INSERT INTO long_term_projection_interval_state (invocation_row_id, model_series_key, upstream_series_key, interval_start_ms, interval_end_ms) VALUES (1, 'model:v2:before:', 'account:42', 1785024000000, 1785024001000)",
        )
        .execute(&pool)
        .await
        .expect("canonical interval");
