...
pub(crate) async fn load_pending_invocation_archive_hourly_rollup_deltas(
    ...
    let archive_rows = load_pending_invocation_archive_rows(pool, range, max_rows).await?;
    ...
    validate_pending_invocation_archive_row_budget(pool, range, max_rows, &archive_rows).await?;
    let mut pending_state = PendingInvocationArchiveOverallState::default();
    let mut scanned_rows = 0usize;

    for archive_row in archive_rows {
        scan_pending_invocation_archive(
            archive_row,
            source_scope,
            range,
            exclude_invocation_ids,
...
async fn load_pending_invocation_archive_rows(
    pool: &Pool<Sqlite>,
    range: Option<(DateTime<Utc>, DateTime<Utc>)>,
    max_rows: Option<usize>,
...
async fn validate_pending_invocation_archive_row_budget(
    pool: &Pool<Sqlite>,
    range: Option<(DateTime<Utc>, DateTime<Utc>)>,
    max_rows: Option<usize>,
    archive_rows: &[ArchiveBatchPathRow],
) -> Result<()> {
    let Some(max_rows) = max_rows else {
        return Ok(());
    };
    if range.is_some() || archive_rows.is_empty() {
        return Ok(());
    }
...
async fn scan_pending_invocation_archive(
    ...
    pending_state: &mut PendingInvocationArchiveOverallState,
) -> Result<()> {
...
pub(crate) async fn load_proxy_perf_stage_rollups_by_materialization_state(
    ...
    for archive_row in archive_rows {
        load_proxy_perf_stage_rollups_for_archive(
            archive_row,
            start,
            end,
            exclude_invocation_ids,
...
async fn load_proxy_perf_stage_rollups_for_archive(
    archive_row: ArchiveBatchPathRow,
    start: DateTime<Utc>,
    end: DateTime<Utc>,
    exclude_invocation_ids: Option<&HashSet<i64>>,
    pending_state: &mut PendingProxyPerfArchiveState,
) -> Result<()> {
    let Some((archive_pool, temp_cleanup)) =
    ...
    loop {
        ...
        for row in rows {
            if exclude_invocation_ids.is_some_and(|excluded_ids| excluded_ids.contains(&row.id))
                || !invocation_hourly_source_record_matches_range(&row, Some((start, end)))
            {
                continue;
            }
...
pub(crate) async fn query_unmaterialized_proxy_perf_stage_rollups_from_archives(
    ...
    let by_stage = merge_unmaterialized_proxy_perf_deltas(
        pending_state.unmaterialized,
        &materialized_by_bucket_stage,
...
fn merge_unmaterialized_proxy_perf_deltas(
    pending_deltas: BTreeMap<(i64, String), ProxyPerfStageHourlyDelta>,
    materialized_by_bucket_stage: &BTreeMap<(i64, String), ProxyPerfStageHourlyRollupRecord>,
    known_materialized_archive_perf_by_bucket_stage: &BTreeMap<
        (i64, String),
        ProxyPerfStageHourlyDelta,
    >,
    materialized_bucket_sources: &HashSet<(i64, String)>,
    unreadable_materialized_bucket_start_epochs: &HashSet<i64>,
) -> Result<BTreeMap<String, ProxyPerfStageHourlyDelta>> {
    let mut by_stage = BTreeMap::new();
    for ((bucket_start_epoch, stage), archive_delta) in pending_deltas {
