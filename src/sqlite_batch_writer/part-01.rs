fn merge_terminal_invocation(
    mut terminal: BatchedTerminalInvocationWrite,
    existing: &BatchedTerminalInvocationWrite,
...
struct PendingBatchAccounting<'a> {
    estimated_bytes: &'a mut usize,
    coalesced_rows: &'a mut usize,
}

fn replace_map_entry<K, V, M, I, F>(
    map: &mut M,
    key: K,
    value: V,
    write_bytes: usize,
    accounting: PendingBatchAccounting<'_>,
    insert: I,
    estimate: F,
) -> usize
where
    I: FnOnce(&mut M, K, V) -> Option<V>,
    F: Fn(&V) -> usize,
{
    if let Some(old) = insert(map, key, value) {
        let old_bytes = estimate(&old);
        *accounting.estimated_bytes = accounting
            .estimated_bytes
            .saturating_sub(old_bytes)
            .saturating_add(write_bytes);
        *accounting.coalesced_rows += 1;
        old_bytes
    } else {
        *accounting.estimated_bytes = accounting.estimated_bytes.saturating_add(write_bytes);
        0
    }
}
...
impl PendingBatch {
    ...
    fn push(&mut self, write: SqliteBatchWrite) -> usize {
        ...
        match write {
            SqliteBatchWrite::TerminalInvocation(terminal) => {
                self.push_terminal(terminal, write_bytes)
            }
            SqliteBatchWrite::AttemptProgress(progress) => {
                self.push_attempt_progress(progress, write_bytes)
            }
            SqliteBatchWrite::InvocationDerived(derived) => {
                self.push_invocation_derived(derived, write_bytes)
            }
            SqliteBatchWrite::AccountSelectedTouch(touch) => {
                self.push_account_selected_touch(touch, write_bytes)
            }
            SqliteBatchWrite::SystemTaskFinish(finish) => {
                self.push_system_task_finish(finish, write_bytes)
            }
            #[cfg(test)]
            SqliteBatchWrite::StartupBackfillWake(task) => {
                self.push_startup_backfill_wake(task, write_bytes)
            }
        }
    }
    ...
    fn push_terminal(
        ...
        let estimate_change = match self.terminal_invocations.entry(key) {
            ...
            std::collections::btree_map::Entry::Occupied(mut entry) => {
                ...
                let mut terminal = merge_terminal_invocation(terminal, entry.get());
                terminal.dashboard_terminal_sequence = terminal
    ...
    fn push_attempt_progress(
        &mut self,
        progress: BatchedAttemptProgress,
        write_bytes: usize,
    ) -> usize {
        replace_map_entry(
            &mut self.attempt_progress,
            progress.attempt_id,
            progress,
            write_bytes,
            PendingBatchAccounting {
                estimated_bytes: &mut self.estimated_bytes,
                coalesced_rows: &mut self.coalesced_rows,
            },
            std::collections::HashMap::insert,
            estimated_attempt_progress_memory_bytes,
        )
    }

    fn push_invocation_derived(
        &mut self,
        derived: BatchedInvocationDerivedWrites,
        write_bytes: usize,
    ) -> usize {
        replace_map_entry(
            &mut self.invocation_derived,
            derived.invocation_id,
            derived,
            write_bytes,
            PendingBatchAccounting {
                estimated_bytes: &mut self.estimated_bytes,
            ...
            estimated_invocation_derived_memory_bytes,
        )
    }

    fn push_account_selected_touch(
        &mut self,
    ...
    fn push_system_task_finish(
        &mut self,
        finish: BatchedSystemTaskFinish,
        write_bytes: usize,
    ) -> usize {
        replace_map_entry(
            &mut self.system_task_finishes,
            finish.run_id,
            finish,
            write_bytes,
            PendingBatchAccounting {
            ...
            std::collections::HashMap::insert,
            estimated_system_task_finish_memory_bytes,
        )
    }

    #[cfg(test)]
    fn push_startup_backfill_wake(
        &mut self,
