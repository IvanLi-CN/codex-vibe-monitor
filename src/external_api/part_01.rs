...
async fn commit_external_api_key_rotation(
    pool: &Pool<Sqlite>,
    existing: &ExternalApiKeyRow,
    now_iso: &str,
    secret_hash: &str,
    secret_prefix: &str,
) -> Result<ExternalApiKeyRow, (StatusCode, String)> {
    let mut tx = pool
        .begin_with("BEGIN IMMEDIATE")
        .await
        .map_err(internal_error_tuple_local)?;
    ...
    ensure_external_api_key_name_available(tx.as_mut(), &existing.name, Some(existing.id)).await?;
    sqlx::query(
        r#"
...
pub(crate) async fn rotate_external_api_key_inner(
    ...
    let row = commit_external_api_key_rotation(
        &state.pool,
        &existing,
        &now_iso,
        &secret_hash,
