...
type LegacyRejectedCooldownRow = (
    i64,
    Option<String>,
    Option<String>,
    Option<String>,
...
async fn reconcile_legacy_upstream_rejected_cooldowns(
    ...
    let rows = load_legacy_rejected_cooldown_rows(pool, limit).await?;
...
async fn load_legacy_rejected_cooldown_rows(
    pool: &Pool<Sqlite>,
    limit: i64,
) -> Result<Vec<LegacyRejectedCooldownRow>> {
    sqlx::query_as::<_, LegacyRejectedCooldownRow>(
        r#"
        SELECT
            id,
            last_action_source,
