...
pub(crate) const UPSTREAM_ACCOUNT_ROW_SELECT_COLUMNS: &str = r#"
    ...
    standalone_search_capability, standalone_search_capability_observed_at,
    standalone_search_capability_reason, policy_standalone_search_capability_override,
...
pub(crate) async fn ensure_protected_system_tag(
    ...
    sqlx::query(
        ...
        VALUES (?1, ?2, 1, 1, 1, 'normal', 'keep_original', 0, 0, 0, ?3, ?3)
        "#,
    )
    .bind(name)
    .bind(system_key)
...
pub(crate) async fn load_tag_summaries(
    ...
    query.push(
        " GROUP BY tag.id, tag.name, tag.system_key, tag.protected, tag.allow_cut_out, tag.allow_cut_in, tag.priority_tier, tag.fast_mode_rewrite_mode, tag.concurrency_limit, tag.upstream_429_retry_enabled, tag.upstream_429_max_retries, tag.available_models_json, tag.updated_at",
    );
...
pub(crate) async fn persist_tag_update(
    ...
    let affected = sqlx::query(
        ...
        SET name = ?2,
            ...
            allow_cut_in = ?4,
            priority_tier = ?5,
            fast_mode_rewrite_mode = ?6,
            concurrency_limit = ?7,
            upstream_429_retry_enabled = ?8,
...
pub(crate) async fn load_upstream_account_groups(
    ...
    let rows = sqlx::query_as::<_, UpstreamAccountGroupListRow>(
        ...
        SELECT
            ...
            notes.policy_codex_imagegen_rewrite_mode,
...
pub(crate) fn build_summary_from_row(
    ...
    UpstreamAccountSummary {
        ...
        bound_proxy_keys: row.bound_proxy_keys(),
        ...
        standalone_search_capability: build_capability_state(
            row.standalone_search_capability.as_deref(),
            row.standalone_search_capability_observed_at.as_ref(),
            row.standalone_search_capability_reason.as_ref(),
...
pub(crate) fn group_routing_rule_from_group_metadata(
    ...
    GroupAccountRoutingRule {
        ...
        codex_imagegen_rewrite_mode: Some(CodexImagegenRewriteMode::KeepOriginal),
...
pub(crate) async fn enrich_current_forward_proxy_for_summaries(
    ...
    let (binding_display_names, shared_group_current_bindings, account_current_bindings) = {
        ...
        let shared_group_current_bindings = groups
            ...
            .collect::<HashMap<_, _>>();
        let account_current_bindings = items
            .iter()
            .filter(|item| !item.bound_proxy_keys.is_empty())
            .filter_map(|item| {
                manager
                    .current_bound_scope_binding_key(
                        &format!("account:{}", item.id),
                        &item.bound_proxy_keys,
                    )
                    .map(|binding_key| (item.id, binding_key))
            })
            .collect::<HashMap<_, _>>();
        (
            binding_display_names,
            shared_group_current_bindings,
            account_current_bindings,
        )
    };
    let relevant_binding_keys = shared_group_current_bindings
        .values()
        .cloned()
        .chain(account_current_bindings.values().cloned())
    ...
    for item in items {
        ...
        if let Some(binding_key) = account_current_bindings.get(&item.id).cloned() {
            assign_current_forward_proxy(
                item,
...
pub(crate) async fn enrich_current_forward_proxy_for_non_node_shunt_detail(
    ...
    if !item.bound_proxy_keys.is_empty() {
        let binding_key = {
            let manager = state.forward_proxy.lock().await;
            manager.current_bound_scope_binding_key(
                &format!("account:{}", item.id),
                &item.bound_proxy_keys,
            )
        };
...
pub(crate) async fn enrich_window_actual_usage_for_summaries_from_storage_at(
    ...
    let Some((plans, query_start, _query_end)) = collect_account_window_usage_plans(items, now)
    else {
        return Ok((AccountWindowUsageBuildOutcome::Ready, telemetry));
    };
    ...
    let minute_rows = if has_minute_stats_rollups {
        let full_minute_ranges = collect_account_window_full_minute_ranges(&plans);
        if !full_minute_ranges.is_empty() {
            load_window_actual_usage_minute_rows_from_pool(pool, &full_minute_ranges).await?
        } else {
            Vec::new()
        }
    } else {
    ...
    if has_live_invocations {
        ...
        let archived_partial_minute_keys = partial_minute_keys
            ...
            .copied()
            .collect::<HashSet<_>>();
        let archived_partial_hour_keys = partial_hour_keys
            .iter()
        ...
        let live_partial_minute_keys = partial_minute_keys
            ...
            .copied()
            .collect::<HashSet<_>>();
        let live_partial_hour_keys = partial_hour_keys
            .iter()
            .filter(|(_, bucket_epoch)| *bucket_epoch >= retention_cutoff_epoch)
            .copied()
            .collect::<HashSet<_>>();
        let live_missing_full_hour_keys = missing_full_hour_keys
            .iter()
            .filter(|(_, bucket_epoch)| *bucket_epoch >= retention_cutoff_epoch)
            .copied()
            .collect::<HashSet<_>>();
        let mut exact_fallback_row_ids = HashSet::new();
        if !live_partial_minute_keys.is_empty() {
            let live_rows = load_window_actual_usage_rows_for_account_bucket_keys_from_pool(
                pool,
                &live_partial_minute_keys,
                60,
                None,
                None,
            )
        ...
        if live_rollup_cursor > 0 {
            let live_tail_rows = load_window_actual_usage_rows_for_plans_from_pool(
                pool,
                &plans,
            ...
            let live_tail_rows = live_tail_rows
                .into_iter()
                .filter(|row| !exact_fallback_row_ids.contains(&row.id))
                .collect::<Vec<_>>();
