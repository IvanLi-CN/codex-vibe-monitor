...
#[path = "sync_account_imports_tags/duplicate_identity_clusters.rs"]
mod duplicate_identity_clusters;
...
pub(crate) async fn load_duplicate_info_map(
    ...
    let (by_account_id, by_user_id) = duplicate_identity_clusters::build(&rows);

    Ok(duplicate_identity_clusters::duplicate_info(
        rows,
        by_account_id,
        by_user_id,
    ))
