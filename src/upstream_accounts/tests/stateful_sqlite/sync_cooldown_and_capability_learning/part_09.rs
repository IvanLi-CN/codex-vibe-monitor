...
pub(crate) async fn resolver_uses_reset_time_candidate_after_single_account_rotation_429() {
    ...
    let (exhausted, closer_reset, farther_reset) =
        seed_single_account_rotation_reset_accounts(&state, crypto_key, group_name).await;
    ...
    assert_reset_time_candidate(&state, sticky_key, closer_reset).await;
}

async fn assert_reset_time_candidate(state: &AppState, sticky_key: &str, expected_id: i64) {
    let resolution =
...
async fn seed_single_account_rotation_reset_accounts(
    state: &AppState,
    crypto_key: &[u8; 32],
    group_name: &str,
) -> (i64, i64, i64) {
