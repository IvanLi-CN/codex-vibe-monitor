...
pub(crate) async fn model_routing_timeline_queries_use_epoch_and_latest_event_indexes() {
    model_routing_timeline_queries_use_epoch_and_latest_event_indexes_impl().await;
}

async fn model_routing_timeline_queries_use_epoch_and_latest_event_indexes_impl() {
    let state = test_app_state_with_usage_base("http://127.0.0.1:9").await;
    let (account_id, model, attempt_id, recent_utc, expired_utc) =
        seed_timeline_index_fixture(&state).await;

    let cutoff_epoch_ms = (Utc::now() - ChronoDuration::minutes(15)).timestamp_millis();
    assert_timeline_query_plans(&state.pool, cutoff_epoch_ms).await;
    assert_timeline_api(
        &state,
        account_id,
        &model,
...
async fn seed_timeline_index_fixture(state: &Arc<AppState>) -> (i64, String, i64, String, String) {
    ...
    .await;
    let model = "gpt-timeline-index".to_string();
    let now = Utc::now();
    ...
    let attempt_id: i64 = sqlx::query_scalar(
        "INSERT INTO pool_upstream_request_attempts (invoke_id, occurred_at, endpoint, route_mode, request_model, upstream_account_id, upstream_route_key, attempt_index, distinct_account_index, same_account_retry_index, status) VALUES (?1, ?2, '/v1/responses', 'pool', ?3, ?4, 'timeline-index', 1, 1, 0, 'success') RETURNING id",
    )
    ...
    sqlx::query(
        "INSERT INTO pool_upstream_account_events (account_id, occurred_at, action, source, attempt_id, model, model_route_state_before, model_route_state_after, created_at) VALUES (?1, ?2, 'model_route_state_changed', 'call', ?3, ?4, 'available', 'degraded', ?2), (?1, ?5, 'model_route_state_changed', 'call', ?3, ?4, 'degraded', 'available', ?5), (?1, ?6, 'model_route_state_changed', 'call', NULL, ?4, 'available', 'degraded', ?6), (?1, ?7, 'model_route_state_changed', 'call', NULL, ?4, 'available', 'degraded', ?7)",
    )
    .bind(account_id)
    .bind(&recent_local)
    .bind(attempt_id)
    ...
    (account_id, model, attempt_id, recent_utc, expired_utc)
}

async fn assert_timeline_query_plans(pool: &SqlitePool, cutoff_epoch_ms: i64) {
    ...
    .collect::<Vec<_>>();
    assert!(attempt_plan.iter().any(|detail| detail.contains(
    ...
    .map(|(_, _, _, detail)| detail)
    .collect::<Vec<_>>();
    assert!(event_plan.iter().any(|detail| detail.contains(
...
pub(crate) async fn model_routing_live_query_stays_bounded_for_dense_recent_timeline() {
    ...
    seed_dense_timeline_rows(
        &mut transaction,
        DenseTimelineFixture {
            account_id,
            model,
            now,
...
async fn seed_dense_timeline_rows(
    transaction: &mut sqlx::Transaction<'_, sqlx::Sqlite>,
    fixture: DenseTimelineFixture<'_>,
) {
    let DenseTimelineFixture {
        account_id,
        model,
        now,
        timeline_rows,
        insert_batch_size,
        attempt_id_base,
        linked_event_id_base,
        standalone_event_id_base,
    } = fixture;
...
async fn seed_model_routing_live_fixture(
    state: &Arc<AppState>,
) -> (i64, String, i64, i64, String, String) {
    let display_name = "Routing live API account".to_string();
    let account_id = insert_test_pool_api_key_account_with_options(
        state,
        &display_name,
    ...
    let model = "gpt-routing-live-api".to_string();
...
async fn assert_model_routing_live_initial(
    state: Arc<AppState>,
    account_id: i64,
    model: &str,
    display_name: &str,
    cache_usage_missing_since: &str,
) {
    let Json(live) = get_model_routing_live(
        State(state.clone()),
        Query(ModelRoutingLiveQuery {
    ...
    assert!(
        live.records
            .iter()
            .all(|record| record.kind == "attempt" && record.model == model)
    );
    assert!(
        live.records
            .iter()
...
pub(crate) async fn model_routing_live_api_lists_api_key_attempts_and_pages_account_history() {
    model_routing_live_api_lists_api_key_attempts_and_pages_account_history_impl().await;
}

async fn model_routing_live_api_lists_api_key_attempts_and_pages_account_history_impl() {
    let state = test_app_state_with_usage_base("http://127.0.0.1:9").await;
    let (account_id, model, first_attempt, second_attempt, cache_usage_missing_since, display_name) =
        seed_model_routing_live_fixture(&state).await;

    assert_model_routing_live_initial(
        state.clone(),
        account_id,
        &model,
        &display_name,
    ...
    let deleted_account_id = seed_live_api_extra_accounts(&state, &model).await;
...
async fn seed_live_api_extra_accounts(state: &Arc<AppState>, model: &str) -> i64 {
...
async fn assert_live_api_history(
    state: &Arc<AppState>,
    account_id: i64,
    model: &str,
    first_attempt: i64,
...
pub(crate) async fn enable_cache_hit_protection(state: &AppState) {
    sqlx::query(
...
pub(crate) async fn cache_hit_protection_observation_respects_sample_boundary_and_threshold() {
    ...
    assert_cache_observation_progression(&state, account_id, model).await;
}

async fn assert_cache_observation_progression(state: &AppState, account_id: i64, model: &str) {
