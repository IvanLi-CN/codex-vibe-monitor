...
async fn external_api_keys_support_rotate_disable_and_bearer_auth() {
    ...
    let rotated_value = serde_json::to_value(&rotated_response).expect("serialize rotated api key");
    ...
    let rotate_again_err = rotate_external_api_key(
        State(state.clone()),
        external_api_key_settings_headers(),
        AxumPath(key_id),
    )
    ...
    let Json(disabled_response) = disable_external_api_key(
        State(state.clone()),
        external_api_key_settings_headers(),
        AxumPath(rotated_id),
    )
...
async fn external_oauth_upsert_preserves_manual_email_while_refreshing_verified_email() {
    ...
    assert_eq!(
        detail.summary.email.as_deref(),
        Some("operator-picked@example.com")
    );
    assert_eq!(
...
async fn external_oauth_patch_waits_for_inflight_maintenance() {
    ...
    assert_eq!(updated.note.as_deref(), Some("patched after maintenance"));
