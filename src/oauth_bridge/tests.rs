...
fn extract_completed_response_from_sse_returns_completed_response() {
    ...
    let value = extract_completed_response_from_sse(payload).expect("extract completed response");
...
async fn oauth_responses_buffered_json_success_passthroughs_non_sse_payload() {
    ...
    let payload: Value = serde_json::from_slice(&body).expect("decode oauth json response payload");
