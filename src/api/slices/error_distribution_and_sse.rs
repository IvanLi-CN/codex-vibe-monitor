...
async fn load_live_request_streaming_perf_rows(
    pool: &sqlx::SqlitePool,
    range: &RangeWindow,
    invocation_max_days: u64,
) -> Result<Vec<LiveRequestStreamingPerfRow>, ApiError> {
...
struct LiveRequestStreamingCohortAccumulator {
    ...
    fallback_reason_counts: std::collections::BTreeMap<String, i64>,
...
async fn query_live_request_streaming_perf(
    ...
    let rows = load_live_request_streaming_perf_rows(pool, range, invocation_max_days).await?;
    ...
    for row in rows {
        ...
        if cohort == "treatment" && transport_mode == "buffered" {
            let reason = payload
                .get("liveFirstReason")
                .and_then(serde_json::Value::as_str)
    ...
    let cohorts = by_cohort
        ...
        .map(|(key, mut entry)| {
            ...
            LiveRequestStreamingCohortStats {
                ...
                fallback_reason_counts: entry.fallback_reason_counts,
...
#[derive(Default)]
struct LiveRequestStreamingMetricSamples {
    control: BTreeMap<String, Vec<f64>>,
    live_first: BTreeMap<String, Vec<f64>>,
}

#[derive(Default)]
struct LiveRequestStreamingRiskStratum {
    control_events: i64,
    control_total: i64,
    treatment_events: i64,
    treatment_total: i64,
}

#[derive(Default)]
struct LiveRequestStreamingRiskCounts {
    by_group: BTreeMap<String, LiveRequestStreamingRiskStratum>,
}

impl LiveRequestStreamingRiskCounts {
    fn observe(&mut self, group: &str, variant: &str, event: bool) {
        if group == "unknown" {
            return;
        }
        let stratum = self.by_group.entry(group.to_string()).or_default();
        match variant {
            "control" => {
                stratum.control_total += 1;
                stratum.control_events += i64::from(event);
            }
            "treatment" => {
                stratum.treatment_total += 1;
                stratum.treatment_events += i64::from(event);
            }
            _ => {}
        }
    }
    ...
    fn into_delta(&self) -> Option<LiveRequestStreamingRiskDelta> {
        let (control_events, control_total, treatment_events, treatment_total) = self
            .by_group
            .values()
            .filter(|stratum| stratum.control_total > 0 && stratum.treatment_total > 0)
            .fold((0_i64, 0_i64, 0_i64, 0_i64), |acc, stratum| {
                (
                    acc.0 + stratum.control_events,
                    acc.1 + stratum.control_total,
                    acc.2 + stratum.treatment_events,
                    acc.3 + stratum.treatment_total,
                )
            });
        if control_total <= 0 || treatment_total <= 0 {
            return None;
        }
        let control_rate = control_events as f64 / control_total as f64;
        let treatment_rate = treatment_events as f64 / treatment_total as f64;
        let (control_lower, _) = wilson_interval(control_events, control_total);
        let (_, treatment_upper) = wilson_interval(treatment_events, treatment_total);
        Some(LiveRequestStreamingRiskDelta {
            difference: treatment_rate - control_rate,
            upper_bound: (treatment_upper - control_lower).max(0.0),
        })
    }
}

fn evaluation_group(payload: &serde_json::Value) -> String {
    for key in ["liveFirstAccountGroup", "upstreamAccountGroup"] {
...
fn push_evaluation_metric(
    samples: &mut LiveRequestStreamingMetricSamples,
    variant: &str,
    transport_mode: &str,
    group: &str,
    value: Option<f64>,
    ...
    let target = match (variant, transport_mode) {
        ("control", "buffered") => &mut samples.control,
        ("treatment", "live_first") => &mut samples.live_first,
        _ => return,
    };
    target.entry(group.to_string()).or_default().push(value);
}

fn metric_confidence_interval(
    samples: &LiveRequestStreamingMetricSamples,
    higher_is_better: bool,
) -> Option<LiveRequestStreamingConfidenceInterval> {
    let groups = samples
        .control
        .iter()
        .filter_map(|(group, control)| {
            if group == "unknown" {
                return None;
            }
            let treatment = samples.live_first.get(group)?;
            (!control.is_empty() && !treatment.is_empty()).then_some((control, treatment))
        })
        .collect::<Vec<_>>();
    if groups.is_empty() {
        return None;
    ...
    for (control, treatment) in &groups {
        control_values.extend(control.iter().copied());
        treatment_values.extend(treatment.iter().copied());
    }
    if control_values.len() < LIVE_REQUEST_STREAMING_MIN_SUCCESS_SAMPLES as usize
        || treatment_values.len() < LIVE_REQUEST_STREAMING_MIN_SUCCESS_SAMPLES as usize
    {
        return None;
    }

    let mut control_sorted = control_values.clone();
    let mut treatment_sorted = treatment_values.clone();
    control_sorted.sort_by(f64::total_cmp);
    treatment_sorted.sort_by(f64::total_cmp);
    let control_p50 = percentile_sorted_f64(&control_sorted, 0.50);
    let treatment_p50 = percentile_sorted_f64(&treatment_sorted, 0.50);
    let difference = if higher_is_better {
        treatment_p50 - control_p50
    } else {
        control_p50 - treatment_p50
    };

    let mut state = 0x9e37_79b9_7f4a_7c15_u64;
    let mut differences = Vec::with_capacity(LIVE_REQUEST_STREAMING_BOOTSTRAP_RESAMPLES);
    for _ in 0..LIVE_REQUEST_STREAMING_BOOTSTRAP_RESAMPLES {
        let mut control_resampled = Vec::with_capacity(control_values.len());
        let mut treatment_resampled = Vec::with_capacity(treatment_values.len());
        for (control, treatment) in &groups {
            for _ in 0..control.len() {
                let index = next_bootstrap_index(&mut state, control.len());
                control_resampled.push(control[index]);
            }
            for _ in 0..treatment.len() {
                let index = next_bootstrap_index(&mut state, treatment.len());
                treatment_resampled.push(treatment[index]);
            }
        }
        control_resampled.sort_by(f64::total_cmp);
        treatment_resampled.sort_by(f64::total_cmp);
        let control_p50 = percentile_sorted_f64(&control_resampled, 0.50);
        let treatment_p50 = percentile_sorted_f64(&treatment_resampled, 0.50);
        differences.push(if higher_is_better {
            treatment_p50 - control_p50
        } else {
            control_p50 - treatment_p50
        });
    ...
    Some(LiveRequestStreamingConfidenceInterval {
        p50_difference_ms: difference,
        lower_ms: percentile_sorted_f64(&differences, 0.025),
        upper_ms: percentile_sorted_f64(&differences, 0.975),
    })
}

fn next_bootstrap_index(state: &mut u64, length: usize) -> usize {
    *state = state
        .wrapping_mul(2_862_933_555_777_941_757)
        .wrapping_add(3_039_700_499_889_820_449);
    (*state as usize) % length
}

fn wilson_interval(events: i64, total: i64) -> (f64, f64) {
    if total <= 0 {
        return (0.0, 1.0);
    }
    let n = total as f64;
    let p = (events.max(0).min(total) as f64) / n;
    let z = 1.96_f64;
...
fn evaluate_live_request_streaming_rows(
    rows: &[LiveRequestStreamingPerfRow],
    range: &RangeWindow,
) -> LiveRequestStreamingEvaluationResponse {
    let mut treatment_assignment_count = 0_i64;
    let mut treatment_eligible_count = 0_i64;
    let mut actual_live_first_count = 0_i64;
    let mut treatment_buffered_fallback_count = 0_i64;
    let mut route_finalization = LiveRequestStreamingRouteFinalizationAccumulator::default();
    let mut first_response = LiveRequestStreamingMetricSamples::default();
    let mut first_token = LiveRequestStreamingMetricSamples::default();
    let mut overlap = LiveRequestStreamingMetricSamples::default();
    let mut first_attempt_failure = LiveRequestStreamingRiskCounts::default();
    let mut fallback_or_retry = LiveRequestStreamingRiskCounts::default();
    let mut capture_failure = LiveRequestStreamingRiskCounts::default();
    let mut ambiguous_delivery = LiveRequestStreamingRiskCounts::default();
    ...
    for row in rows {
        ...
        if payload.get("endpoint").and_then(serde_json::Value::as_str) != Some("/v1/responses")
            || payload
                .get("liveFirstRevision")
                .and_then(serde_json::Value::as_str)
                != Some(LIVE_REQUEST_STREAMING_REVISION)
        {
            continue;
        }
        let variant = payload
            ...
            .and_then(serde_json::Value::as_str)
            .unwrap_or("unknown");
        if !matches!(variant, "control" | "treatment") {
            continue;
        }
        let transport_mode = payload
            .get("requestBodyTransportMode")
            .and_then(serde_json::Value::as_str)
            .unwrap_or("unknown");
        let group = evaluation_group(&payload);
        if variant == "treatment" {
            treatment_assignment_count += 1;
            treatment_eligible_count += i64::from(payload_bool(&payload, "liveFirstEligibility"));
            actual_live_first_count += i64::from(transport_mode == "live_first");
            treatment_buffered_fallback_count += i64::from(transport_mode == "buffered");
        }
        first_attempt_failure.observe(
            &group,
            variant,
            payload_bool(&payload, "liveFirstAttemptFailed"),
        );
        fallback_or_retry.observe(
            &group,
            variant,
            payload_bool(&payload, "liveFirstFallbackOrRetry"),
        );
        capture_failure.observe(
            &group,
            variant,
            payload_bool(&payload, "liveFirstCaptureFailed"),
        );
        ambiguous_delivery.observe(
            &group,
            variant,
            payload_bool(&payload, "ambiguousUpstreamDelivery"),
        );
        route_finalization.observe(&payload);
        if row.status.as_deref() != Some("success") || row.failure_kind.is_some() {
            continue;
        }
        push_evaluation_metric(
            &mut first_response,
            variant,
            transport_mode,
            &group,
            payload
                .get("firstResponseByteTotalMs")
        ...
        push_evaluation_metric(
            ...
            &group,
            payload
                .get("requestUpstreamOverlapMs")
                .and_then(serde_json::Value::as_f64),
        );
    }

    let actual_live_first_rate =
        live_request_streaming_rate(actual_live_first_count, treatment_assignment_count);
    let metrics = LiveRequestStreamingEvaluationMetrics {
        first_response: metric_confidence_interval(&first_response, false),
        first_token: metric_confidence_interval(&first_token, false),
    ...
    let status = if treatment_assignment_count < LIVE_REQUEST_STREAMING_MIN_TREATMENT_ASSIGNMENTS {
        reason_codes.push("treatment_assignments_below_minimum".to_string());
        "insufficient_data"
        ...
        || metrics.overlap.is_none()
    {
        ...
        let risks = [
            ...
            risk.capture_failure.as_ref(),
            risk.ambiguous_delivery.as_ref(),
        ];
        let risk_safe = risks.iter().all(|risk| {
            risk.is_some_and(|risk| risk.upper_bound <= LIVE_REQUEST_STREAMING_MAX_RISK_INCREASE)
        });
        if response.lower_ms >= LIVE_REQUEST_STREAMING_MIN_LATENCY_BENEFIT_MS
            && token.lower_ms >= LIVE_REQUEST_STREAMING_MIN_LATENCY_BENEFIT_MS
            && overlap.lower_ms > 0.0
            && risk_safe
        {
            reason_codes.push("latency_and_risk_thresholds_met".to_string());
            "recommend_keep"
        } else if response.upper_ms <= 0.0 && token.upper_ms <= 0.0 {
            reason_codes.push("latency_benefit_not_detected".to_string());
            "recommend_remove"
        } else {
            reason_codes.push("latency_or_risk_evidence_inconclusive".to_string());
            if !risk_safe {
                reason_codes.push("risk_increase_above_threshold".to_string());
            }
            "review_required"
        }
    };
    ...
    for row in rows {
        ...
        let transport_mode = payload
            ...
            .unwrap_or("unknown");
        let entry = by_cohort
            .entry(format!("{cohort}:{transport_mode}"))
            .or_default();
        entry.invocation_count += 1;
        if cohort == "treatment" && transport_mode == "buffered" {
            let reason = payload
        ...
        entry.ambiguous_upstream_delivery_count +=
            i64::from(payload_bool(&payload, "ambiguousUpstreamDelivery"));
        if row.status.as_deref() == Some("success") && row.failure_kind.is_none() {
            entry.success_sample_count += 1;
            push_payload_ms(
                &mut entry.first_response_byte_total_ms,
                &payload,
    ...
    let cohorts = by_cohort
        .into_iter()
        .map(|(key, mut entry)| {
            let (cohort, transport_mode) = key.split_once(':').unwrap_or(("unknown", "unknown"));
            let first_response_byte_sample_count = entry.first_response_byte_total_ms.len() as i64;
            let first_token_sample_count = entry.first_token_ms.len() as i64;
            let request_upstream_overlap_sample_count =
                entry.request_upstream_overlap_ms.len() as i64;
            LiveRequestStreamingCohortStats {
    ...
    LiveRequestStreamingEvaluationResponse {
        revision: LIVE_REQUEST_STREAMING_REVISION.to_string(),
        endpoint: "/v1/responses".to_string(),
        range_start: format_utc_iso(range.start),
        range_end: format_utc_iso(range.display_end),
        treatment_assignment_count,
        treatment_eligible_count,
        actual_live_first_count,
        treatment_buffered_fallback_count,
        actual_live_first_rate,
        cohorts,
        route_finalization: route_finalization.into_response(),
        metrics,
        risk,
        decision: LiveRequestStreamingEvaluationDecision {
            status: status.to_string(),
            reason_codes,
            min_treatment_assignments: LIVE_REQUEST_STREAMING_MIN_TREATMENT_ASSIGNMENTS,
            min_actual_live_first_rate: LIVE_REQUEST_STREAMING_MIN_ACTUAL_LIVE_RATE,
            min_metric_samples: LIVE_REQUEST_STREAMING_MIN_SUCCESS_SAMPLES,
            min_latency_benefit_ms: LIVE_REQUEST_STREAMING_MIN_LATENCY_BENEFIT_MS,
            max_risk_increase: LIVE_REQUEST_STREAMING_MAX_RISK_INCREASE,
            bootstrap_resamples: LIVE_REQUEST_STREAMING_BOOTSTRAP_RESAMPLES,
        },
    }
}

pub(crate) async fn fetch_live_request_streaming_evaluation(
    State(state): State<Arc<AppState>>,
) -> Result<Json<LiveRequestStreamingEvaluationResponse>, ApiError> {
    let range_end = Utc::now();
    let range = RangeWindow {
        start: range_end - ChronoDuration::hours(24 * 7),
        end: range_end,
        display_end: range_end,
        duration: ChronoDuration::hours(24 * 7),
    };
    let rows = load_live_request_streaming_perf_rows(
        &state.pool,
        &range,
        state.config.invocation_max_days,
    )
    .await?;
    Ok(Json(evaluate_live_request_streaming_rows(&rows, &range)))
}
...
struct LiveRequestStreamingRouteFinalizationAccumulator {
    ...
    outcome_counts: std::collections::BTreeMap<String, i64>,
...
impl LiveRequestStreamingRouteFinalizationAccumulator {
    fn observe(&mut self, payload: &serde_json::Value) {
        ...
        *self.outcome_counts.entry(outcome.to_string()).or_default() += 1;
...
mod tests {
    ...
    fn evaluation_range_for_test() -> RangeWindow {
        let end = Utc::now();
        let duration = ChronoDuration::days(7);
        RangeWindow {
            start: end - duration,
            end,
            display_end: end,
        ...
        LiveRequestStreamingPerfRow {
            ...
            payload: Some(
                serde_json::json!({
                    ...
                    "liveFirstExperimentVariant": variant,
                    "requestBodyTransportMode": transport_mode,
                    "liveFirstEligibility": true,
                    "upstreamAccountGroup": "test",
                    "firstResponseByteTotalMs": first_response_ms,
                    "firstTokenTotalMs": first_token_ms,
                    "requestUpstreamOverlapMs": overlap_ms,
                })
                .to_string(),
            ),
        }
    ...
    fn evaluation_rows(
        control: (f64, f64, f64),
        treatment: (f64, f64, f64),
        treatment_transport: &str,
        treatment_count: usize,
    ) -> Vec<LiveRequestStreamingPerfRow> {
        let mut rows = Vec::with_capacity(treatment_count * 2);
        for index in 0..treatment_count {
            rows.push(evaluation_row(
                index as i64,
                "control",
                "buffered",
                control.0,
    ...
    #[test]
    fn live_request_streaming_evaluation_locks_decision_boundaries_and_seed() {
        let range = evaluation_range_for_test();

        let insufficient = evaluate_live_request_streaming_rows(
            &evaluation_rows(
                (300.0, 400.0, 0.0),
                (100.0, 100.0, 100.0),
                "live_first",
        ...
        assert_eq!(insufficient.decision.status, "insufficient_data");
        assert_eq!(
            insufficient.decision.reason_codes,
            vec!["treatment_assignments_below_minimum"]
        );

        let remove = evaluate_live_request_streaming_rows(
            &evaluation_rows((300.0, 400.0, 0.0), (100.0, 100.0, 0.0), "buffered", 1_000),
            &range,
        );
        assert_eq!(remove.decision.status, "recommend_remove");
        assert_eq!(remove.actual_live_first_count, 0);
        assert_eq!(
            remove.decision.reason_codes,
            vec!["actual_live_first_rate_below_minimum"]
        );

        let keep_rows = evaluation_rows(
            (300.0, 400.0, 0.0),
            (100.0, 100.0, 100.0),
            "live_first",
            1_000,
        );
        let keep = evaluate_live_request_streaming_rows(&keep_rows, &range);
        let keep_again = evaluate_live_request_streaming_rows(&keep_rows, &range);
        assert_eq!(keep.decision.status, "recommend_keep");
        assert_eq!(keep.decision.bootstrap_resamples, 2_000);
        assert_eq!(
            serde_json::to_value(&keep.metrics).expect("serialize metrics"),
            serde_json::to_value(&keep_again.metrics).expect("serialize repeated metrics")
        );

        let review = evaluate_live_request_streaming_rows(
            &evaluation_rows(
                (150.0, 100.0, 0.0),
                (100.0, 100.0, 0.0),
                "live_first",
        ...
        assert_eq!(review.decision.status, "review_required");
        assert_eq!(
            review.decision.reason_codes,
            vec!["latency_or_risk_evidence_inconclusive"]
        );
    }

    #[test]
    fn live_request_streaming_evaluation_only_compares_matched_account_groups() {
        let mut risk = LiveRequestStreamingRiskCounts::default();
        risk.observe("unknown", "control", true);
