...
pub(crate) const INVOCATION_RESOLVED_FAILURE_CLASS_SQL: &str = concat!(
    ...
    "      WHEN LOWER(TRIM(COALESCE(CASE WHEN json_valid(payload) THEN CAST(json_extract(payload, '$.failureKind') AS TEXT) END, failure_kind, ''))) IN ('request_body_stream_error_client_closed', 'request_body_invalid_json', 'invalid_api_key', 'api_key_not_found', 'api_key_missing') ",
    "        OR LOWER(TRIM(COALESCE(error_message, ''))) LIKE '[request_body_stream_error_client_closed]%' ",
    "        OR LOWER(TRIM(COALESCE(error_message, ''))) LIKE '[request_body_invalid_json]%' ",
...
impl SummaryProjection {
    fn response_for_query(
        ...
        let is_full_rollup_bucket = |record: &SummaryProjectionRecord| {
...
async fn load_summary_projection_archive_account_ids(
    ...
    // Keep the input itself bounded.  A DISTINCT query can still scan an entire low-cardinality
    // archive before it knows that no new account id exists; the projection must fail closed
    // after a bounded number of rows instead of allowing that scan to consume its deadline.
...
async fn build_summary_projection(
    ...
    cached_archive_account_id_count = archive_account_ids_by_file
        .values()
        .map(HashSet::len)
        .sum::<usize>();
    if cached_archive_account_id_count > SUMMARY_PROJECTION_MAX_EXACT_RECORDS {
    ...
    for archive in &archives {
        ...
        {
            return Err(anyhow!(
                "summary projection cannot prove account coverage for a large materialized archive without a bounded discovery cache"
            ));
        }
    ...
    if all_time_was_fully_rebuilt {
        ...
        let global_response =
            load_summary_response_from_query(state, &global_params, SummaryBuildRoute::Background)
                .await
                .map_err(|error| {
        ...
        let live_tail_ids = crate::stats::load_live_invocation_ids_after_id(
            &state.pool,
            InvocationSourceScope::All,
            rollup_live_cursor,
        )
    ...
    for upstream_account_id in std::iter::once(None).chain(account_ids.iter().copied().map(Some)) {
        ...
        let in_progress = match upstream_account_id {
            ...
            Some(account_id) if active_in_progress_accounts.contains(&account_id) => {
                ...
                _ = cadence.tick() => {
                    state
                        ...
                        .is_none_or(|projection| {
                            projection.refreshed_at.is_none_or(|refreshed_at| {
...
async fn load_dashboard_activity_account_build_result(
    ...
    let mut build_telemetry = DashboardActivityBuildTelemetry {
        account_aggregation: account_aggregation_telemetry,
...
pub(crate) async fn fetch_dashboard_activity(
    ...
    if elapsed_ms >= 250 {
        tracing::warn!(
            ...
            aggregation_mode = if build_telemetry.account_aggregation.fallback_hour_count == 0 { "rollup_plus_boundary" } else { "partial_rollup_with_exact_fallback" },
    ...
    async fn summary_projection_fails_closed_when_unmaterialized_archive_exceeds_budget() {
        ...
        sqlx::query(
            "WITH RECURSIVE rows(value) AS (SELECT 1 UNION ALL SELECT value + 1 FROM rows WHERE value < 50001) \
             ...
             SELECT value, 'archive-summary-' || value, datetime('now', '+8 hours', '-1 minute'), 'proxy', \
        ...
        let mut budget = 0;
        let error = merge_summary_projection_archive_records(
            &pool,
            &HashSet::new(),
            &HashMap::new(),
            &HashMap::new(),
