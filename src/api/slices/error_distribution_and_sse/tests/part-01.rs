...
fn runtime_projection_merges_restored_network_bucket_without_double_counting() {
    ...
    assert_merged_network_projection_is_stable(&hub);
}

fn assert_merged_network_projection_is_stable(hub: &RuntimeProjectionHub) {
    let first = hub
    ...
    for slice in [&first.slice, &second.slice] {
        let global = slice
            .network_live_bucket
            .as_ref()
