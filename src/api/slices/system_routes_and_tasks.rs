...
async fn refresh_system_raw_payload_metrics_inventory_inner(state: &AppState) -> Result<u64> {
    ...
    let update_result = sqlx::query(
        ...
        SET inventory_state = ?1,
            ...
            raw_overflow_spool_bytes = ?10,
            updated_at = datetime('now')
        WHERE singleton = 1
          AND inventory_state = ?11
        "#,
    )
    .bind(state_name)
    .bind(next_cursor)
...
pub(crate) fn spawn_system_raw_payload_metrics_inventory(
    ...
    tokio::spawn(async move {
        loop {
            ...
            } else if state.raw_capture_circuit.snapshot().inventory_state == "ready" {
