...
async fn test_pool() -> Pool<Sqlite> {
    ...
    create_invocation_table(&pool).await;
    create_rollup_tables(&pool).await;
    create_archive_tables(&pool).await;
    pool
}

async fn create_invocation_table(pool: &Pool<Sqlite>) {
    sqlx::query(
        r#"
...
async fn create_rollup_tables(pool: &Pool<Sqlite>) {
    sqlx::query(
        r#"
...
async fn create_archive_tables(pool: &Pool<Sqlite>) {
    sqlx::query(
        r#"
...
async fn create_account_activity_v2_test_tables(pool: &Pool<Sqlite>) {
    sqlx::query(
        r#"
...
async fn seed_account_activity_v2_archive(pool: &Pool<Sqlite>) {
    sqlx::query(
        r#"
...
async fn account_activity_v2_generation_requeues_materialized_invocation_archives() {
    ...
    create_account_activity_v2_test_tables(&pool).await;
    seed_account_activity_v2_archive(&pool).await;
...
async fn assert_archived_breakdown_state(pool: &Pool<Sqlite>, bucket_start_epoch: i64) {
...
async fn retention_archived_bucket_clears_breakdown_rollup_without_marking_it_materialized() {
    ...
    mark_retention_archived_hourly_rollup_targets_tx(tx.as_mut(), "codex_invocations", &[row], &[])
        .await
        .expect("mark retention archived hourly rollup targets");
    tx.commit().await.expect("commit transaction");

    assert_archived_breakdown_state(&pool, bucket_start_epoch).await;
}

fn archived_breakdown_source_row(id: i64, occurred_at: String) -> InvocationHourlySourceRecord {
...
async fn seed_retention_live_invocation(pool: &Pool<Sqlite>, occurred_at: &str) {
    sqlx::query(
            r#"
...
async fn seed_retention_partial_breakdown_rows(pool: &Pool<Sqlite>, bucket_start_epoch: i64) {
    for (
        upstream_account_key,
        upstream_account_id,
        normalized_model,
...
async fn assert_retention_partial_hour_state(pool: &Pool<Sqlite>, bucket_start_epoch: i64) {
    ...
    assert_eq!(retained, (1, 1, 70, 0.13_f64, 0.0_f64));
...
async fn retention_partial_hour_removes_archived_breakdown_without_dropping_retained_live_rows() {
    ...
    let archived_row = archived_breakdown_source_row(1, archived_occurred_at);
    let bucket_start_epoch = invocation_bucket_start_epoch(&archived_row.occurred_at)
        .expect("derive bucket start epoch");

    seed_retention_live_invocation(&pool, &live_occurred_at).await;

    seed_retention_partial_breakdown_rows(&pool, bucket_start_epoch).await;
    ...
    assert_retention_partial_hour_state(&pool, bucket_start_epoch).await;
}

async fn assert_replayed_breakdown_state(pool: &Pool<Sqlite>, bucket_start_epoch: i64) {
...
async fn retention_partial_hour_preserves_previously_replayed_archive_breakdown_rows() {
    ...
    assert_replayed_breakdown_state(&pool, bucket_start_epoch).await;
}

fn breakdown_filter_row(
    id: i64,
    occurred_at: String,
    status: &'static str,
    metrics: (i64, i64, i64, f64),
    payload: String,
) -> InvocationHourlySourceRecord {
    let failed = status == "failed";
    let (input_tokens, output_tokens, total_tokens, cost) = metrics;
    InvocationHourlySourceRecord {
        id,
        occurred_at,
        source: SOURCE_PROXY.to_string(),
        ...
        cost_output: None,
        cost_reasoning: None,
        error_message: failed.then(|| "upstream stream error".to_string()),
        failure_kind: failed.then(|| "upstream_response_failed".to_string()),
        failure_class: Some(if failed { "service_failure" } else { "none" }.to_string()),
        is_actionable: Some(i64::from(failed)),
        payload: Some(payload),
        t_total_ms: Some(100.0 + (id - 10) as f64),
        t_req_read_ms: Some(0.0),
        t_req_parse_ms: Some(0.0),
        t_upstream_connect_ms: Some(0.0),
        t_upstream_ttfb_ms: Some(10.0 + (id - 10) as f64),
        first_token_ms: None,
        t_upstream_stream_ms: Some(20.0 + (id - 10) as f64),
        t_resp_parse_ms: Some(0.0),
        t_persist_ms: Some(0.0),
    }
}
...
async fn breakdown_rollup_excludes_running_and_pending_rows() {
    ...
    let rows = [
        ...
        breakdown_filter_row(13, occurred_at, "pending", (13, 23, 36, 0.50), payload),
    ];
...
const REPAIR_FIRST_FILE_PATH: &str = "/tmp/usage-breakdown-overlap-first.sqlite.gz";
const REPAIR_SECOND_FILE_PATH: &str = "/tmp/usage-breakdown-overlap-second.sqlite.gz";

async fn seed_repair_archive_batch(
    pool: &Pool<Sqlite>,
    file_path: &str,
...
async fn seed_repair_state(pool: &Pool<Sqlite>, bucket_start_epoch: i64) {
    seed_repair_archive_batch(
        pool,
        REPAIR_FIRST_FILE_PATH,
        "2026-07-01 15:05:00",
        "2026-07-01 15:15:00",
        false,
        101,
...
async fn assert_repair_state(pool: &Pool<Sqlite>, bucket_start_epoch: i64) {
    ...
    for file_path in [REPAIR_FIRST_FILE_PATH, REPAIR_SECOND_FILE_PATH] {
        let replay_marker_count: i64 = sqlx::query_scalar(
                "SELECT COUNT(*) FROM hourly_rollup_archive_replay WHERE target = ?1 AND dataset = ?2 AND file_path = ?3",
            )
            .bind(HOURLY_ROLLUP_TARGET_UPSTREAM_ACCOUNT_USAGE_BREAKDOWN)
...
async fn repair_materialized_breakdown_reopens_overlapping_replayed_batches() {
    ...
    seed_repair_state(&pool, bucket_start_epoch).await;
    ...
    assert_repair_state(&pool, bucket_start_epoch).await;
}
