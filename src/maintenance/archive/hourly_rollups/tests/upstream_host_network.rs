...
async fn test_pool() -> Pool<Sqlite> {
    ...
    create_attempt_and_rollup_tables(&pool).await;
    pool
}
...
async fn create_attempt_and_rollup_tables(pool: &Pool<Sqlite>) {
    sqlx::query(
        r#"
...
async fn upstream_host_network_direct_rollup_seeds_cursor_without_backfill() {
    ...
    let row_count: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM upstream_host_network_minute")
        .fetch_one(&pool)
        .await
...
async fn assert_live_repair_cursor_state(pool: &Pool<Sqlite>) {
    ...
    assert_eq!(repaired, (Some(42), "gpt-5".to_string(), 1, 0.11_f64));
...
async fn usage_breakdown_live_repair_backfills_rows_behind_shared_cursor() {
    ...
    assert_live_repair_cursor_state(&pool).await;
}
