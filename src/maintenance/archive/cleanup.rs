    ...
    F: FnOnce(&str) -> io::Result<()>,
{
    ...
    let Some(source_safe_start) = load_staged_archive_deletion_source_safe_start(
        tx.as_mut(),
        archive_batch_id,
        dataset,
        file_path,
    ...
    if !verify_and_remove_archive_file(dataset, file_path, expected_sha256, remove_file)? {
        tx.rollback().await?;
        return Ok(false);
    }
...
fn verify_and_remove_archive_file<F>(
    dataset: &str,
    file_path: &str,
    expected_sha256: &str,
    remove_file: F,
...
async fn load_staged_archive_deletion_source_safe_start(
    tx: &mut SqliteConnection,
    archive_batch_id: i64,
    dataset: &str,
    file_path: &str,
    expected_sha256: &str,
) -> Result<Option<Option<NaiveDate>>> {
    ...
    if dataset == HOURLY_ROLLUP_DATASET_INVOCATIONS
        && !summary_archive_snapshot_deletion_proof_is_valid(
            &mut *tx,
            archive_batch_id,
            expected_sha256,
        )
    ...
    let Some(value) = staged_source_safe_start else {
        return Ok(Some(None));
    };
...
async fn summary_archive_snapshot_deletion_proof_is_valid(
    tx: &mut SqliteConnection,
    archive_batch_id: i64,
    expected_sha256: &str,
) -> Result<bool> {
    ...
    summary_archive_snapshot_has_proof_tx(&mut *tx, archive_batch_id, expected_sha256).await
}

include!("cleanup/part-01.rs");
include!("cleanup/part-02.rs");
include!("cleanup/part-03.rs");
include!("cleanup/part-04.rs");
include!("cleanup/part-05.rs");
#[cfg(test)]
mod tests {
    include!("cleanup/tests/part-01.rs");
}
