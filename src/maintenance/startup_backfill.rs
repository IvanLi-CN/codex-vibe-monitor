...
pub(crate) fn startup_backfill_next_delay(
    ...
    if run.force_idle {
        return Duration::from_secs(STARTUP_BACKFILL_IDLE_INTERVAL_SECS);
    }
    if run.retry_soon {
        return Duration::from_secs(15);
    }
    if run.updated > 0 {
        return Duration::from_secs(STARTUP_BACKFILL_ACTIVE_INTERVAL_SECS);
    }
    if run.hit_scan_limit {
        let seconds = if run.scanned == 0 {
            STARTUP_BACKFILL_ACTIVE_INTERVAL_SECS
        } else {
            match zero_update_streak {
                0 | 1 => 15,
                2 => 60,
                3 => 5 * 60,
                _ => 15 * 60,
    ...
    if run.scanned == 0 || zero_update_streak > 0 {
        return Duration::from_secs(STARTUP_BACKFILL_IDLE_INTERVAL_SECS);
    }
...
include!("startup_backfill/part-01.rs");
include!("startup_backfill/part-02.rs");
include!("startup_backfill/part-03.rs");
include!("startup_backfill/part-04.rs");
include!("startup_backfill/part-05.rs");
include!("startup_backfill/part-06.rs");
