...
pub(crate) fn record_pool_upstream_binding_window_stats(
    ...
    proxy_key: &str,
    row: &PoolUpstreamBindingWindowStatsRow,
) {
    let stats = grouped.entry(proxy_key.to_string()).or_default();
    stats.attempts += row.attempts;
    stats.success_count += row.success_count;
    *latency_totals.entry(proxy_key.to_string()).or_insert(0.0) +=
        row.latency_sum_ms.unwrap_or(0.0);
    *latency_samples.entry(proxy_key.to_string()).or_insert(0) += row.latency_sample_count;
}
