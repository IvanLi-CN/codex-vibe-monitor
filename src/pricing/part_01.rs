...
const REQUIRED_PRICING_MODELS: &[&str] = &[
    "gpt-5.4",
    "gpt-5.4-pro",
    "gpt-5.4-mini",
    "gpt-5.5",
...
pub(crate) async fn ensure_pricing_models_present(pool: &Pool<Sqlite>) -> Result<()> {
    let mut models = default_pricing_catalog().models;
    for model in REQUIRED_PRICING_MODELS {
        let pricing = models
            .remove(*model)
            .with_context(|| format!("missing required default pricing model: {model}"))?;
        ensure_pricing_model_present(pool, model, pricing).await?;
    }
    Ok(())
}
...
async fn refresh_official_pricing_row_if_unchanged(
    pool: &Pool<Sqlite>,
    model: &str,
    ...
    sqlx::query(
        ...
        SET input_per_1m = ?1,
            ...
            updated_at = datetime('now')
        WHERE model = ?5
          AND lower(trim(source)) = 'official'
          AND input_per_1m = ?6
          AND output_per_1m = ?7
    ...
    .execute(pool)
    .await
    .with_context(|| format!("failed to refresh unchanged official pricing row: {model}"))?;

    Ok(())
}

pub(crate) async fn refresh_repriced_gpt_5_6_defaults(pool: &Pool<Sqlite>) -> Result<()> {
    refresh_official_pricing_row_if_unchanged(
        pool,
        "gpt-5.6-terra",
        CachedPricingRate {
            input_per_1m: 2.5,
