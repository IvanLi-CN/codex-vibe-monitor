...
pub(crate) async fn persist_pool_failover_terminal_invocation(
    ...
    let live_request_streaming_measurement = live_request_streaming_decision.map(|_| {
        let risk = live_request_streaming_risk_flags(
            ...
            runtime_snapshot_context
                .is_some_and(|context| context.live_first_request_body_first_byte_at.is_some()),
                ...
                match select_pool_account_forward_proxy_client(state.as_ref(), &account).await {
                    ...
                    Err(message) => {
                        if let Err(route_err) = reservation_guard
                            .fence_failure(
                                record_pool_route_transport_failure_for_model_and_broadcast(
                                    state.as_ref(),
                                    account.account_id,
                                    sticky_key,
                                    &message,
                ...
                PoolResolvedAuth::ApiKey { authorization } => {
                    ...
                    .await
                    {
                        ...
                        Ok(Err(err)) => {
                            ...
                            if let Err(route_err) = reservation_guard.fence_failure(
        ...
        codex_imagegen_rewrite: attempted_codex_imagegen_rewrite.clone(),
                ...
                )
            {
                ...
                let route_failure_result = if codex_imagegen_upstream_incompatible {
                    ...
                    reservation_guard.fence_failure(
