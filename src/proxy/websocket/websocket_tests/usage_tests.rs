...
#[tokio::test]
async fn websocket_terminal_usage_retains_unsupported_actual_tier_from_intermediate_event() {
    ...
    tracker
        .observe_upstream_text(
            &state,
            r#"{"type":"response.in_progress","response":{"id":"resp_unsupported_tier","model":"gpt-6-sol","status":"in_progress","service_tier":"batch","usage":{"input_tokens":1000,"output_tokens":100,"total_tokens":1100}}}"#,
        )
        .await;
    tracker
        .observe_upstream_text(
            &state,
            r#"{"type":"response.completed","response":{"id":"resp_unsupported_tier","model":"gpt-6-sol","status":"completed","usage":{"input_tokens":1000,"output_tokens":100,"total_tokens":1100}}}"#,
        )
        .await;
    state
        .sqlite_batch_writer
    ...
    let payload: Value = serde_json::from_str(&persisted.1).expect("decode invocation payload");
