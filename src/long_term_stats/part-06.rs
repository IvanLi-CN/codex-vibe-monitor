async fn commit_long_term_projection_date_rebuild_chunk_with_control(
    ...
    let repaired_start_date = long_term_projection_repaired_start_date(rebuilds, &publication);
    let repaired_nonempty = repaired_start_date.is_some();

    let mut rebuild_tokens = Vec::with_capacity(rebuilds.len());
    for rebuild in rebuilds {
        let token =
            prepare_long_term_projection_rebuild(pool, rebuild, &publication, control).await?;
        rebuild_tokens.push(token.clone());
        persist_long_term_projection_rebuild_intervals(pool, rebuild, &token, control).await?;

        suppress_long_term_projection_rebuild_intervals(pool, rebuild, &token, control).await?;
    }

    finalize_long_term_projection_rebuild_rows(pool, rebuilds, &rebuild_tokens, control).await?;
    ...
    ensure_long_term_projection_can_publish(&mut transaction, publication.mark_ready).await?;
    ...
    clear_long_term_projection_backup_claims(
        &mut transaction,
        rebuilds,
        &rebuild_tokens,
        publication.publication_token.is_none(),
    )
    .await?;
...
fn long_term_projection_repaired_start_date(
    rebuilds: &[LongTermProjectionDateRebuild],
    publication: &LongTermProjectionRebuildPublication<'_>,
) -> Option<String> {
    publication
...
async fn finalize_long_term_projection_rebuild_rows(
    pool: &Pool<Sqlite>,
    rebuilds: &[LongTermProjectionDateRebuild],
    rebuild_tokens: &[String],
    control: &LongTermProjectionWriteControl<'_>,
) -> Result<()> {
    for rebuild in rebuilds {
...
async fn ensure_long_term_projection_can_publish(
    ...
    let marker = sqlx::query_scalar::<_, Option<String>>(
    ...
    .await?
    .flatten();
    if marker.as_deref() == Some(LONG_TERM_INITIAL_MATERIALIZATION_PENDING_ERROR) {
        bail!(
...
async fn clear_long_term_projection_backup_claims(
    transaction: &mut sqlx::Transaction<'_, Sqlite>,
    rebuilds: &[LongTermProjectionDateRebuild],
    rebuild_tokens: &[String],
    cleanup_needed: bool,
) -> Result<()> {
    if !cleanup_needed || rebuild_tokens.is_empty() {
        return Ok(());
    }
    ...
    let mut claims =
        QueryBuilder::<Sqlite>::new("DELETE FROM long_term_projection_daily_backup_claims WHERE ");
    ...
    state.build().execute(&mut **transaction).await?;
    claims.build().execute(&mut **transaction).await?;
    Ok(())
}

async fn prepare_long_term_projection_rebuild(
    pool: &Pool<Sqlite>,
...
async fn suppress_long_term_projection_rebuild_intervals(
    pool: &Pool<Sqlite>,
    rebuild: &LongTermProjectionDateRebuild,
    token: &str,
    control: &LongTermProjectionWriteControl<'_>,
    ...
    loop {
        ...
        let suppressed = sqlx::query(
            "INSERT OR IGNORE INTO long_term_projection_interval_suppressions (invocation_row_id, bucket_date) SELECT candidate.invocation_row_id, ?1 FROM (SELECT state.invocation_row_id FROM long_term_projection_interval_state state WHERE state.interval_start_ms < ?2 AND state.interval_end_ms > ?3 UNION SELECT legacy.invocation_row_id FROM long_term_projection_intervals legacy WHERE legacy.bucket_date = ?1) candidate WHERE NOT EXISTS (SELECT 1 FROM long_term_projection_rebuild_members member WHERE member.rebuild_token = ?4 AND member.invocation_row_id = candidate.invocation_row_id) AND NOT EXISTS (SELECT 1 FROM long_term_projection_interval_suppressions suppressed WHERE suppressed.invocation_row_id = candidate.invocation_row_id AND suppressed.bucket_date = ?1) LIMIT ?5",
        ).bind(&rebuild.bucket_date).bind(rebuild.end_epoch * 1_000).bind(rebuild.start_epoch * 1_000)
            .bind(token).bind(LONG_TERM_PROJECTION_WRITE_BATCH_ROWS as i64)
...
async fn persist_long_term_projection_rebuild_intervals(
    pool: &Pool<Sqlite>,
    rebuild: &LongTermProjectionDateRebuild,
    token: &str,
    control: &LongTermProjectionWriteControl<'_>,
    ...
    if rebuild.interval_segments.is_empty() {
        return mark_long_term_projection_interval_baseline_pending(
            pool,
            &rebuild.bucket_date,
            control,
        )
        ...
        .enumerate()
    {
        let (mut transaction, permit) = control.begin(pool).await?;
        if batch_index == 0 {
            insert_long_term_projection_interval_baseline_pending(
                &mut transaction,
                &rebuild.bucket_date,
            )
            .await?;
        ...
        for segment in batch {
            sqlx::query("INSERT INTO long_term_projection_interval_state (invocation_row_id, model_series_key, upstream_series_key, interval_start_ms, interval_end_ms) VALUES (?1, ?2, ?3, ?4, ?5) ON CONFLICT(invocation_row_id) DO UPDATE SET model_series_key = excluded.model_series_key, upstream_series_key = excluded.upstream_series_key, interval_start_ms = excluded.interval_start_ms, interval_end_ms = excluded.interval_end_ms")
                .bind(segment.invocation_row_id).bind(&segment.model_series_key).bind(&segment.upstream_series_key)
                .bind(segment.interval_start_ms).bind(segment.interval_end_ms).execute(&mut *transaction).await?;
            sqlx::query("INSERT OR IGNORE INTO long_term_projection_rebuild_members (rebuild_token, invocation_row_id) VALUES (?1, ?2)")
                .bind(token).bind(segment.invocation_row_id).execute(&mut *transaction).await?;
            sqlx::query("DELETE FROM long_term_projection_interval_suppressions WHERE invocation_row_id = ?1 AND bucket_date = ?2")
                .bind(segment.invocation_row_id).bind(&rebuild.bucket_date).execute(&mut *transaction).await?;
        }
        control.commit(transaction, permit).await?;
    }
    Ok(())
...
async fn mark_long_term_projection_interval_baseline_pending(
    pool: &Pool<Sqlite>,
    bucket_date: &str,
    control: &LongTermProjectionWriteControl<'_>,
) -> Result<()> {
    let (mut transaction, permit) = control.begin(pool).await?;
    insert_long_term_projection_interval_baseline_pending(&mut transaction, bucket_date).await?;
    control.commit(transaction, permit).await
}

async fn insert_long_term_projection_interval_baseline_pending(
    transaction: &mut sqlx::Transaction<'_, Sqlite>,
    bucket_date: &str,
) -> Result<()> {
    sqlx::query("INSERT INTO long_term_projection_bucket_state (bucket_date, interval_baseline_ready) VALUES (?1, 0) ON CONFLICT(bucket_date) DO UPDATE SET interval_baseline_ready = 0, updated_at = datetime('now')")
        .bind(bucket_date).execute(&mut **transaction).await?;
    Ok(())
}
