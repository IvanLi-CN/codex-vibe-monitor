async fn load_long_term_projection_rows_for_date(
    ...
    let (mut rows, mut row_positions) =
        load_long_term_projection_live_rows_for_date(pool, start, end).await?;
    ...
    for archive_path in archive_paths {
        merge_long_term_projection_archive_rows_for_date(
            pool,
            LongTermProjectionArchiveMerge {
                date,
                start,
                end,
                control,
                attempt_accounts: &attempt_accounts,
                archive_path: &archive_path,
                rows: &mut rows,
                row_positions: &mut row_positions,
            },
        )
        .await?;
    }
...
async fn load_long_term_projection_live_rows_for_date(
    pool: &Pool<Sqlite>,
    start: chrono::DateTime<chrono_tz::Tz>,
    end: chrono::DateTime<chrono_tz::Tz>,
) -> Result<(Vec<LongTermInvocationRow>, HashMap<i64, usize>)> {
    ...
    let row_positions = rows
        ...
        .collect::<HashMap<_, _>>();
    Ok((rows, row_positions))
}

async fn merge_long_term_projection_archive_rows_for_date(
    pool: &Pool<Sqlite>,
    input: LongTermProjectionArchiveMerge<'_, '_>,
) -> Result<()> {
    let LongTermProjectionArchiveMerge {
        date,
        start,
        end,
        control,
    ...
    let overlaps = long_term_projection_archive_path_overlaps(archive_path, date);
    if !overlaps {
        return Ok(());
    }
    let archive_rows = load_long_term_projection_archive_rows_for_date(
        pool,
        date,
        start,
        end,
    ...
    for mut row in archive_rows {
        hydrate_long_term_archive_attempt_account(&mut row, attempt_accounts);
...
struct LongTermProjectionArchiveMerge<'a, 'control> {
    date: NaiveDate,
    start: chrono::DateTime<chrono_tz::Tz>,
    end: chrono::DateTime<chrono_tz::Tz>,
    control: &'a LongTermProjectionWriteControl<'control>,
    attempt_accounts: &'a HashMap<(String, String), i64>,
    archive_path: &'a ArchiveBatchPathRow,
    rows: &'a mut Vec<LongTermInvocationRow>,
    row_positions: &'a mut HashMap<i64, usize>,
}

async fn load_long_term_projection_archive_rows_for_date(
    pool: &Pool<Sqlite>,
    date: NaiveDate,
    start: chrono::DateTime<chrono_tz::Tz>,
    end: chrono::DateTime<chrono_tz::Tz>,
    ...
    let Some((archive_pool, cleanup)) =
        open_invocation_archive_batch_pool(archive_path, "long-term-projection-targeted-repair")
            .await?
    else {
        anyhow::bail!(
    ...
    let archive_rows = async {
        let archive_fingerprint = long_term_archive_pool_fingerprint(&archive_pool).await?;
...
fn long_term_projection_archive_path_overlaps(
    archive_path: &ArchiveBatchPathRow,
    date: NaiveDate,
) -> bool {
    let attempt_start = date.pred_opt().unwrap_or(date);
...
async fn ensure_long_term_projection_daily_backup_for_date(
    ...
    copy_long_term_projection_daily_backup_rows(pool, bucket_date, rebuild_token, control).await?;
...
async fn copy_long_term_projection_daily_backup_rows(
    pool: &Pool<Sqlite>,
    bucket_date: &str,
    rebuild_token: &str,
    control: &LongTermProjectionWriteControl<'_>,
...
async fn replace_long_term_projection_date_rollups(
    ...
    mark_long_term_projection_interval_baseline_ready(pool, &rebuild.bucket_date, control).await
}

async fn mark_long_term_projection_interval_baseline_ready(
    pool: &Pool<Sqlite>,
    bucket_date: &str,
    control: &LongTermProjectionWriteControl<'_>,
) -> Result<()> {
