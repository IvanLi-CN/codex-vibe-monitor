...
async fn load_long_term_archive_attempt_accounts(
    ...
    let paths = load_long_term_attempt_archive_paths(pool).await?;
    let mut accounts = HashMap::new();
    let mut consumed_archives = HashSet::new();
    for archive_path in paths
        .into_iter()
        .filter(|path| long_term_attempt_archive_overlaps(path, date_range))
    {
        let rows = load_long_term_attempt_archive_rows(pool, &archive_path, date_range).await?;
        for row in rows {
...
async fn load_long_term_attempt_archive_paths(
    pool: &Pool<Sqlite>,
) -> Result<Vec<LongTermAttemptArchivePath>> {
    match sqlx::query_as::<_, LongTermAttemptArchivePath>(
...
fn long_term_attempt_archive_overlaps(
    path: &LongTermAttemptArchivePath,
    date_range: Option<(NaiveDate, NaiveDate)>,
) -> bool {
...
async fn load_long_term_attempt_archive_rows(
    pool: &Pool<Sqlite>,
    archive_path: &LongTermAttemptArchivePath,
    date_range: Option<(NaiveDate, NaiveDate)>,
) -> Result<Vec<LongTermArchiveAttemptRow>> {
