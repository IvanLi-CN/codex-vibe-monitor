struct LongTermBucketIdentity<'a> {
    dimension: &'a str,
    series_key: &'a str,
    display_name: &'a str,
...
fn add_long_term_row(
    ...
    identity: &LongTermBucketIdentity<'_>,
    start_ms: i64,
    ...
    let key = (
        hour_start_ms / 1000,
        identity.dimension.to_string(),
        identity.series_key.to_string(),
    );
    let bucket = buckets.entry(key).or_insert_with(|| LongTermBucket {
        ...
        series_key: identity.series_key.to_string(),
        display_name: identity.display_name.to_string(),
        reasoning_effort: identity.reasoning_effort.to_string(),
        stats_date: None,
