fn long_term_refresh_live_tail_start(today: NaiveDate) -> Option<String> {
...
struct LongTermRefreshPublicationState {
    status: &'static str,
    last_error: Option<String>,
}

struct LongTermRefreshStatusInput<'a> {
    completed_integrity_repairs: &'a HashSet<NaiveDate>,
    reconstructable_start: NaiveDate,
    initial_materialization: bool,
    source_rows_empty: bool,
    daily_rows_empty: bool,
    has_persisted_daily_rows: bool,
    clears_pending_empty_initial_materialization: bool,
    archive_read_failed: bool,
    terminal_proof_reconciliation_incomplete: bool,
}
...
async fn apply_long_term_refresh_rollups_with_control(
    ...
    let (has_persisted_daily_rows, clears_pending_empty_initial_materialization, refresh_backups) =
        prepare_long_term_refresh_replacement(
            pool,
            recomputed_dates,
            initial_materialization,
            source_rows_empty,
    ...
    let daily_rows_empty = daily.is_empty();
    persist_long_term_refresh_candidates(
        pool,
        hourly,
        daily,
        retention_start,
    ...
    .await?;
    let publication_state = determine_long_term_refresh_publication_state(
        pool,
        LongTermRefreshStatusInput {
            completed_integrity_repairs,
            reconstructable_start,
            initial_materialization,
            source_rows_empty,
            daily_rows_empty,
            has_persisted_daily_rows,
            clears_pending_empty_initial_materialization,
            archive_read_failed,
            terminal_proof_reconciliation_incomplete,
        },
    )
    .await?;
    publish_long_term_refresh_replacement(
        pool,
        &refresh_backups,
        completed_integrity_repairs,
        publication_state,
    ...
    persist_long_term_refresh_archive_results(
        pool,
...
async fn publish_long_term_refresh_replacement(
    pool: &Pool<Sqlite>,
    refresh_backups: &[(String, String)],
    completed_integrity_repairs: &HashSet<NaiveDate>,
    publication_state: LongTermRefreshPublicationState,
    statistics_start_date: Option<&str>,
    processed_rows_count: i64,
    control: &LongTermProjectionWriteControl<'_>,
) -> Result<()> {
    let publication_token =
        stage_long_term_refresh_publication(pool, refresh_backups, control).await?;
    commit_long_term_refresh_publication_state(
        pool,
        completed_integrity_repairs,
        publication_state,
        statistics_start_date,
        processed_rows_count,
        publication_token.as_deref(),
        control,
    )
    .await?;
    release_long_term_refresh_publication(
        pool,
        refresh_backups,
        publication_token.as_deref(),
        control,
...
async fn commit_long_term_refresh_publication_state(
    pool: &Pool<Sqlite>,
    completed_integrity_repairs: &HashSet<NaiveDate>,
    publication_state: LongTermRefreshPublicationState,
    statistics_start_date: Option<&str>,
    ...
    if let Some(publication_token) = publication_token {
        sqlx::query("INSERT INTO long_term_projection_date_publications (publication_token, published) VALUES (?1, 1) ON CONFLICT(publication_token) DO UPDATE SET published = 1, updated_at = datetime('now')")
            .bind(publication_token).execute(&mut *transaction).await?;
    }
    ...
    sqlx::query("UPDATE long_term_stats_state SET status = ?1, statistics_start_date = ?2, processed_rows = ?3, total_rows = ?3, last_error = ?4, updated_at = datetime('now') WHERE id = ?5")
        .bind(publication_state.status).bind(statistics_start_date).bind(processed_rows_count)
        .bind(publication_state.last_error).bind(LONG_TERM_STATE_ID).execute(&mut *transaction).await?;
    control.commit(transaction, permit).await?;
    for date in completed_integrity_repairs {
...
async fn release_long_term_refresh_publication(
    pool: &Pool<Sqlite>,
    refresh_backups: &[(String, String)],
    publication_token: Option<&str>,
    control: &LongTermProjectionWriteControl<'_>,
...
async fn persist_long_term_refresh_archive_results(
    pool: &Pool<Sqlite>,
    markers: LongTermRefreshArchiveMarkers<'_>,
    control: &LongTermProjectionWriteControl<'_>,
) -> Result<()> {
    persist_long_term_refresh_archive_markers_with_control(pool, markers, control).await
}

async fn stage_long_term_refresh_publication(
    pool: &Pool<Sqlite>,
    refresh_backups: &[(String, String)],
    control: &LongTermProjectionWriteControl<'_>,
    ...
    Ok(publication_token)
}

async fn persist_long_term_refresh_candidates(
    pool: &Pool<Sqlite>,
    ...
    let retention_start_epoch = retention_start
        ...
        .unwrap_or(i64::MIN);
    prune_long_term_refresh_hourly(pool, retention_start_epoch, control).await?;
    let hourly = hourly
        .values()
        .filter(|bucket| {
            Shanghai
...
async fn prune_long_term_refresh_hourly(
    pool: &Pool<Sqlite>,
...
async fn determine_long_term_refresh_publication_state(
    pool: &Pool<Sqlite>,
    input: LongTermRefreshStatusInput<'_>,
) -> Result<LongTermRefreshPublicationState> {
    let LongTermRefreshStatusInput {
        completed_integrity_repairs,
        reconstructable_start,
        initial_materialization,
        source_rows_empty,
    ...
    let completed_repair_dates = completed_integrity_repairs
        .iter()
        .filter(|date| **date >= reconstructable_start)
        .map(ToString::to_string)
        .collect::<Vec<_>>();
    let pending_integrity_repairs = count_pending_long_term_integrity_repairs(
        pool,
        reconstructable_start,
        &completed_repair_dates,
    )
    .await?;
    ...
    let last_error = long_term_refresh_publication_error(
        status,
        initial_materialization,
        terminal_proof_reconciliation_incomplete,
        pending_integrity_repairs,
        archive_read_failed,
    );
    Ok(LongTermRefreshPublicationState { status, last_error })
}

async fn count_pending_long_term_integrity_repairs(
    pool: &Pool<Sqlite>,
    reconstructable_start: NaiveDate,
    completed_repair_dates: &[String],
) -> Result<i64> {
    let mut query = QueryBuilder::<Sqlite>::new(
        "SELECT COUNT(*) FROM long_term_stats_repair_queue WHERE stats_date >= ",
    );
    query.push_bind(reconstructable_start.to_string());
    if !completed_repair_dates.is_empty() {
        query.push(" AND stats_date NOT IN (");
        let mut dates = query.separated(", ");
        for date in completed_repair_dates {
...
fn long_term_refresh_publication_error(
    status: &str,
    initial_materialization: bool,
    terminal_proof_reconciliation_incomplete: bool,
    pending_integrity_repairs: i64,
    archive_read_failed: bool,
) -> Option<String> {
    if status == LONG_TERM_STATUS_ERROR && initial_materialization {
        ...
        archive_read_failed
            .then(|| "one or more invocation archives could not be materialized".to_string())
    }
}

async fn prepare_long_term_refresh_replacement(
    pool: &Pool<Sqlite>,
    ...
    if recomputed_dates.is_empty()
        && (!has_persisted_daily_rows || clears_pending_empty_initial_materialization)
    {
        clear_long_term_refresh_rollups(pool, control).await?;
    } else {
        for date in recomputed_dates {
...
async fn clear_long_term_refresh_rollups(
    pool: &Pool<Sqlite>,
    control: &LongTermProjectionWriteControl<'_>,
) -> Result<()> {
...
async fn persist_long_term_refresh_archive_marker_with_control(
    ...
    let source_identity_after =
        verify_long_term_refresh_archive_source(pool, dataset, file_path, archive_sha256, control)
            .await?;
    let (mut transaction, permit) = control.begin(pool).await?;
    let result = sqlx::query(
        r#"
...
async fn verify_long_term_refresh_archive_source(
    pool: &Pool<Sqlite>,
    dataset: &str,
    file_path: &str,
    archive_sha256: &str,
    ...
    let source_identity_before =
        long_term_refresh_archive_identity_or_cleanup(pool, dataset, file_path, control).await?;
    if let Err(error) =
    ...
    if source_identity_before == source_identity_after {
        return Ok(source_identity_after);
    }
    delete_long_term_refresh_archive_marker_with_control(pool, dataset, file_path, control).await?;
    bail!(
        "long-term archive source changed before its replay marker could be persisted: {file_path}"
    )
}

async fn long_term_refresh_archive_identity_or_cleanup(
    pool: &Pool<Sqlite>,
    dataset: &str,
    file_path: &str,
    control: &LongTermProjectionWriteControl<'_>,
) -> Result<String> {
    match long_term_archive_file_identity(file_path) {
        Ok(source_identity) => Ok(source_identity),
        Err(error) => {
            delete_long_term_refresh_archive_marker_with_control(pool, dataset, file_path, control)
                .await?;
...
fn accumulate_long_term_invocation(
    ...
    for (dimension, series_key, display_name, reasoning_effort) in dimensions {
        let identity = LongTermBucketIdentity {
            dimension,
        ...
        add_long_term_row(hourly, &identity, start_ms, row, interval);
        add_long_term_daily_row(daily, &identity, &date_string, row, interval);
    }
}
