...
async fn refresh_long_term_stats_once(
    ...
    let state_snapshot = load_long_term_refresh_state(pool).await?;
    let was_ready =
        state_snapshot
            .as_ref()
    ...
    .await;
    record_long_term_refresh_error(pool, control, &result).await;
    result
}

async fn load_long_term_refresh_state(
    pool: &Pool<Sqlite>,
) -> Result<
    Option<(
        Option<String>,
...
async fn record_long_term_refresh_error(
    pool: &Pool<Sqlite>,
    control: &LongTermProjectionWriteControl<'_>,
    result: &Result<()>,
) {
    ...
    let Ok((mut transaction, permit)) = control.begin(pool).await else {
        return;
    };
    let _ = sqlx::query(
        "UPDATE long_term_stats_state SET status = ?1, last_error = ?2, updated_at = datetime('now') WHERE id = ?3",
