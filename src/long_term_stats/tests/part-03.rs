...
async fn interrupted_date_rebuild_keeps_publication_and_cursor_uncommitted() {
    let (pool, date_text, rebuild) = setup_interrupted_date_rebuild().await;
    run_interrupted_date_rebuild(&pool, &date_text, &rebuild).await;
}

async fn setup_interrupted_date_rebuild() -> (Pool<Sqlite>, String, LongTermProjectionDateRebuild) {
    ...
    let daily = (0..=LONG_TERM_PROJECTION_WRITE_BATCH_ROWS)
        .map(|index| {
    ...
    (pool, date_text, rebuild)
}

async fn run_interrupted_date_rebuild(
    pool: &Pool<Sqlite>,
    date_text: &str,
    ...
    commit_long_term_projection_date_rebuilds_with_control(
        ...
        &[LongTermProjectionDirtyBucket {
            bucket_date: date_text.to_string(),
            generation: 1,
        }],
        false,
        &control,
    ...
    assert_interrupted_date_rebuild_state(pool, date_text, &shutdown, &committed_batches).await;
    ...
    assert_published_date_rebuild_state(pool, date_text).await;
}

async fn assert_interrupted_date_rebuild_state(
    pool: &Pool<Sqlite>,
    date_text: &str,
...
async fn assert_published_date_rebuild_state(pool: &Pool<Sqlite>, date_text: &str) {
    assert_eq!(
        load_long_term_projection_cursor(pool)
            .await
...
async fn chunked_date_rebuild_keeps_empty_state_until_final_publication() {
    let (pool, first_date_text, rebuilds, dirty) = setup_chunked_date_rebuild().await;
    run_chunked_date_rebuild(&pool, &first_date_text, &rebuilds, &dirty).await;
}

async fn setup_chunked_date_rebuild() -> (
    Pool<Sqlite>,
    String,
    Vec<LongTermProjectionDateRebuild>,
    Vec<LongTermProjectionDirtyBucket>,
) {
    let pool = long_term_stats_memory_pool().await;
    ...
    (pool, first_date_text, rebuilds, dirty)
}

async fn run_chunked_date_rebuild(
    pool: &Pool<Sqlite>,
    first_date_text: &str,
    ...
    assert_chunked_rebuild_unpublished(pool, first_date_text).await;
    let unrestricted = LongTermProjectionWriteControl::unrestricted();
    commit_long_term_projection_date_rebuilds_with_control(
        pool,
        rebuilds,
    ...
    assert_chunked_rebuild_published(pool, first_date_text).await;
}

async fn assert_chunked_rebuild_unpublished(pool: &Pool<Sqlite>, first_date_text: &str) {
    let status =
...
async fn assert_chunked_rebuild_published(pool: &Pool<Sqlite>, first_date_text: &str) {
    let status =
        sqlx::query_scalar::<_, String>("SELECT status FROM long_term_stats_state WHERE id = ?1")
            .bind(LONG_TERM_STATE_ID)
            .fetch_one(pool)
...
async fn projection_date_rebuild_filters_rfc3339_rows_by_shanghai_epoch_bounds() {
    let pool = setup_projection_rfc3339_rows().await;
...
async fn setup_projection_rfc3339_rows() -> Pool<Sqlite> {
    let pool = SqlitePoolOptions::new()
        .max_connections(1)
        .connect("sqlite::memory:")
        .await
    ...
    ensure_long_term_stats_schema(&pool)
        .await
        .expect("long-term schema");
    for (id, invoke_id, occurred_at, total_ms) in [
        (1, "utc-boundary", "2026-07-25T16:30:00Z", None),
        (2, "local-boundary", "2026-07-26 00:30:00", None),
        (
            3,
            "rfc3339-crossing",
        ...
        (4, "legacy-crossing", "2026-07-25 23:59:59", Some(2_000.0)),
        (5, "rfc3339-outside", "2026-07-25T15:59:57Z", Some(1_000.0)),
        (6, "rfc3339-day-start", "2026-07-25T16:00:00Z", None),
        (7, "rfc3339-next-day-start", "2026-07-26T16:00:00Z", None),
        (
            8,
            "rfc3339-before-day-start",
        ...
        (
            9,
            "rfc3339-before-next-day-start",
            "2026-07-26T15:59:59.9999Z",
            None,
        ),
        (
            10,
            "rfc3339-submicro-before-day-start",
            "2026-07-25T15:59:59.9999999Z",
            None,
        ),
        (
            11,
            "rfc3339-submicro-before-next-day-start",
            "2026-07-26T15:59:59.9999999Z",
            None,
        ),
        (
            12,
        ...
        (
            13,
            "rfc3339-high-precision-before-start",
            "2026-07-25T15:59:59.99999999999999999999Z",
            None,
        ),
    ] {
        insert_projection_rfc3339_row(&pool, id, invoke_id, occurred_at, total_ms).await;
    }
    pool
}

async fn insert_projection_rfc3339_row(
    pool: &Pool<Sqlite>,
    id: i64,
    invoke_id: &str,
    occurred_at: &str,
    ...
    sqlx::query(
        "INSERT INTO codex_invocations (id, invoke_id, occurred_at, status, total_tokens, t_total_ms) VALUES (?1, ?2, ?3, 'success', 100, ?4)",
    )
    .bind(id)
    .bind(invoke_id)
    .bind(occurred_at)
    ...
    .execute(pool)
    .await
    .expect("projection RFC3339 invocation");
}
