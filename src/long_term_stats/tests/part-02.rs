...
async fn projection_interval_retention_batches_defer_cancel_and_resume() {
    ...
    assert_retention_pressure_defers(&pool).await;
    run_interrupted_retention_batches(&pool).await;
    resume_retention_and_assert_clean(&pool).await;
}

async fn seed_retention_fixture(pool: &Pool<Sqlite>) {
    let retention_start = long_term_projection_hourly_retention_start_date(366);
...
async fn assert_retention_pressure_defers(pool: &Pool<Sqlite>) {
...
async fn run_interrupted_retention_batches(pool: &Pool<Sqlite>) {
...
async fn resume_retention_and_assert_clean(pool: &Pool<Sqlite>) {
    let unrestricted = LongTermProjectionWriteControl::unrestricted();
...
async fn incremental_projection_defers_before_interval_and_rollup_publication() {
    ...
    assert!(long_term_projection_write_is_deferred(&deferred));
    assert_incremental_projection_unpublished(&pool).await;
...
async fn legacy_projection_interval_migration_compresses_expansion_in_one_cancellable_batch() {
    ...
    ensure_long_term_projection_schema(&pool)
        .await
        .expect("projection schema");
    seed_legacy_expanded_intervals(&pool).await;
    run_legacy_interval_migration_batches(&pool).await;
}

async fn seed_legacy_expanded_intervals(pool: &Pool<Sqlite>) {
...
async fn run_legacy_interval_migration_batches(pool: &Pool<Sqlite>) {
    let shutdown = CancellationToken::new();
    let gate = crate::db_pressure::DbPressureGate::new(1, Duration::ZERO);
    let committed_batches = AtomicUsize::new(0);
    let interrupted =
