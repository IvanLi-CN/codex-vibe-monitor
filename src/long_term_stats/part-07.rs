...
async fn audit_long_term_integrity(
    ...
    let (untrusted_dates, expected_rows) =
        load_integrity_inputs(pool, start_epoch, end_epoch).await?;
    let materialized =
        load_materialized_integrity_inputs(pool, start_date, end_date, start_epoch, end_epoch)
            .await?;

    let index = build_integrity_index(
        expected_rows,
        materialized.actual_hourly_rows,
        materialized.actual_daily_rows,
        materialized.daily_dates,
        materialized.hourly_buckets,
        materialized.nonempty_daily_dates,
        materialized.nonempty_hourly_buckets,
    );

    Ok(collect_integrity_mismatches(
        &untrusted_dates,
        &index.expected_by_date,
        &index.actual_hourly_by_date,
        &index.actual_daily_by_date,
        &index.materialized_dates,
        &index.materialized_nonempty_dates,
    ))
}

struct LongTermIntegrityIndex {
    expected_by_date: HashMap<NaiveDate, HashMap<i64, LongTermIntegrityTotals>>,
    actual_hourly_by_date: HashMap<NaiveDate, HashMap<i64, LongTermIntegrityTotals>>,
    actual_daily_by_date: HashMap<NaiveDate, LongTermIntegrityTotals>,
    materialized_dates: HashSet<NaiveDate>,
    materialized_nonempty_dates: HashSet<NaiveDate>,
}

async fn load_integrity_inputs(
    pool: &Pool<Sqlite>,
    start_epoch: i64,
    end_epoch: i64,
) -> Result<(HashSet<NaiveDate>, Vec<LongTermIntegrityHourRow>)> {
    let untrusted_dates = sqlx::query_scalar::<_, i64>(
        "SELECT DISTINCT bucket_start_epoch FROM invocation_rollup_hourly WHERE bucket_start_epoch >= ?1 AND bucket_start_epoch < ?2 AND terminal_proof_complete <> 1",
    ).bind(start_epoch).bind(end_epoch).fetch_all(pool).await?
        .into_iter().filter_map(long_term_bucket_date).collect();
    let expected_rows = sqlx::query_as::<_, LongTermIntegrityHourRow>(
        "SELECT bucket_start_epoch, COALESCE(SUM(terminal_count), 0) AS calls, COALESCE(SUM(terminal_tokens), 0) AS token_total, COALESCE(SUM(terminal_cost), 0.0) AS cost_total FROM invocation_rollup_hourly WHERE bucket_start_epoch >= ?1 AND bucket_start_epoch < ?2 AND terminal_proof_complete = 1 GROUP BY bucket_start_epoch",
    ).bind(start_epoch).bind(end_epoch).fetch_all(pool).await?;
    Ok((untrusted_dates, expected_rows))
}

struct LongTermMaterializedIntegrityInputs {
    daily_dates: Vec<String>,
    hourly_buckets: Vec<i64>,
    nonempty_daily_dates: Vec<String>,
    nonempty_hourly_buckets: Vec<i64>,
    actual_hourly_rows: Vec<LongTermIntegrityHourRow>,
    actual_daily_rows: Vec<(String, i64, i64, f64)>,
}

async fn load_materialized_integrity_inputs(
    pool: &Pool<Sqlite>,
    start_date: NaiveDate,
    end_date: NaiveDate,
    start_epoch: i64,
    end_epoch: i64,
) -> Result<LongTermMaterializedIntegrityInputs> {
    let daily_dates = sqlx::query_scalar::<_, String>(
        "SELECT DISTINCT stats_date FROM long_term_usage_daily WHERE stats_date >= ?1 AND stats_date <= ?2",
    ).bind(start_date.to_string()).bind(end_date.to_string()).fetch_all(pool).await?;
    let hourly_buckets = sqlx::query_scalar::<_, i64>(
        "SELECT DISTINCT bucket_start_epoch FROM long_term_usage_hourly WHERE bucket_start_epoch >= ?1 AND bucket_start_epoch < ?2",
    ).bind(start_epoch).bind(end_epoch).fetch_all(pool).await?;
    let nonempty_daily_dates = sqlx::query_scalar::<_, String>(
        "SELECT DISTINCT stats_date FROM long_term_usage_daily WHERE stats_date >= ?1 AND stats_date <= ?2 AND (calls <> 0 OR token_total <> 0 OR ABS(cost_total) > 1e-12)",
    ).bind(start_date.to_string()).bind(end_date.to_string()).fetch_all(pool).await?;
    let nonempty_hourly_buckets = sqlx::query_scalar::<_, i64>(
        "SELECT DISTINCT bucket_start_epoch FROM long_term_usage_hourly WHERE bucket_start_epoch >= ?1 AND bucket_start_epoch < ?2 AND (calls <> 0 OR token_total <> 0 OR ABS(cost_total) > 1e-12)",
    ).bind(start_epoch).bind(end_epoch).fetch_all(pool).await?;
    let actual_hourly_rows = sqlx::query_as::<_, LongTermIntegrityHourRow>(
        "SELECT bucket_start_epoch, COALESCE(SUM(calls), 0) AS calls, COALESCE(SUM(token_total), 0) AS token_total, COALESCE(SUM(cost_total), 0.0) AS cost_total FROM long_term_usage_hourly WHERE dimension = 'overall' AND bucket_start_epoch >= ?1 AND bucket_start_epoch < ?2 GROUP BY bucket_start_epoch",
    ).bind(start_epoch).bind(end_epoch).fetch_all(pool).await?;
    let actual_daily_rows = sqlx::query_as::<_, (String, i64, i64, f64)>(
        "SELECT stats_date, COALESCE(SUM(calls), 0), COALESCE(SUM(token_total), 0), COALESCE(SUM(cost_total), 0.0) FROM long_term_usage_daily WHERE dimension = 'overall' AND stats_date >= ?1 AND stats_date <= ?2 GROUP BY stats_date",
    ).bind(start_date.to_string()).bind(end_date.to_string()).fetch_all(pool).await?;
    Ok(LongTermMaterializedIntegrityInputs {
        daily_dates,
        hourly_buckets,
        nonempty_daily_dates,
        nonempty_hourly_buckets,
        actual_hourly_rows,
        actual_daily_rows,
    })
}
...
fn build_integrity_index(
    expected_rows: Vec<LongTermIntegrityHourRow>,
    actual_hourly_rows: Vec<LongTermIntegrityHourRow>,
    actual_daily_rows: Vec<(String, i64, i64, f64)>,
    materialized_daily_dates: Vec<String>,
    materialized_hourly_buckets: Vec<i64>,
    materialized_nonempty_daily_dates: Vec<String>,
    materialized_nonempty_hourly_buckets: Vec<i64>,
) -> LongTermIntegrityIndex {
    LongTermIntegrityIndex {
        expected_by_date: long_term_integrity_hourly_index(expected_rows),
        actual_hourly_by_date: long_term_integrity_hourly_index(actual_hourly_rows),
        actual_daily_by_date: actual_daily_rows
            .into_iter()
            ...
            .collect(),
        materialized_dates: long_term_integrity_dates(
            materialized_daily_dates,
            materialized_hourly_buckets,
        ),
        materialized_nonempty_dates: long_term_integrity_dates(
            materialized_nonempty_daily_dates,
            materialized_nonempty_hourly_buckets,
        ),
    }
}

fn long_term_integrity_hourly_index(
    rows: Vec<LongTermIntegrityHourRow>,
) -> HashMap<NaiveDate, HashMap<i64, LongTermIntegrityTotals>> {
    let mut result: HashMap<NaiveDate, HashMap<i64, LongTermIntegrityTotals>> = HashMap::new();
    for row in rows {
        if let Some(date) = long_term_bucket_date(row.bucket_start_epoch) {
...
fn long_term_integrity_dates(
    daily_dates: Vec<String>,
    hourly_buckets: Vec<i64>,
) -> HashSet<NaiveDate> {
    let mut dates = daily_dates
        .into_iter()
        .filter_map(|date| NaiveDate::parse_from_str(&date, "%Y-%m-%d").ok())
        .collect::<HashSet<_>>();
    dates.extend(hourly_buckets.into_iter().filter_map(long_term_bucket_date));
    dates
}

fn collect_integrity_mismatches(
    untrusted_dates: &HashSet<NaiveDate>,
    expected_by_date: &HashMap<NaiveDate, HashMap<i64, LongTermIntegrityTotals>>,
    actual_hourly_by_date: &HashMap<NaiveDate, HashMap<i64, LongTermIntegrityTotals>>,
    actual_daily_by_date: &HashMap<NaiveDate, LongTermIntegrityTotals>,
    materialized_dates: &HashSet<NaiveDate>,
    materialized_nonempty_dates: &HashSet<NaiveDate>,
) -> Vec<LongTermIntegrityMismatch> {
    let mut dates = expected_by_date
        .keys()
    ...
    for date in dates {
        ...
        let expected_daily = expected_hourly.values().fold(
            ...
            |mut total, value| {
                ...
                total.token_total += value.token_total;
                total.cost_total += value.cost_total;
                total
            },
        );
        ...
        {
            mismatches.push(LongTermIntegrityMismatch {
                date, expected: LongTermIntegrityTotals::default(),
