...
pub(super) async fn ensure_schema_prompt_cache_and_attempts(pool: &Pool<Sqlite>) -> Result<()> {
    ensure_pricing_and_oauth_settings(pool).await?;
    ensure_forward_proxy_metadata(pool).await?;
    ensure_proxy_attempt_tables(pool).await?;
    ensure_prompt_cache_bindings(pool).await?;
    ensure_prompt_cache_operation_events(pool).await?;
    ensure_pool_attempt_columns(pool).await?;
    ensure_forward_proxy_weight_tables(pool).await?;
    ensure_forward_proxy_indexes(pool).await?;
    Ok(())
}

async fn ensure_pricing_and_oauth_settings(pool: &Pool<Sqlite>) -> Result<()> {
...
async fn ensure_forward_proxy_metadata(pool: &Pool<Sqlite>) -> Result<()> {
    sqlx::query(
        r#"
...
async fn ensure_proxy_attempt_tables(pool: &Pool<Sqlite>) -> Result<()> {
    sqlx::query(
        r#"
...
async fn ensure_prompt_cache_bindings(pool: &Pool<Sqlite>) -> Result<()> {
    sqlx::query(
        r#"
...
async fn ensure_prompt_cache_operation_events(pool: &Pool<Sqlite>) -> Result<()> {
...
async fn ensure_pool_attempt_columns(pool: &Pool<Sqlite>) -> Result<()> {
...
async fn ensure_forward_proxy_weight_tables(pool: &Pool<Sqlite>) -> Result<()> {
    sqlx::query(
        r#"
...
async fn ensure_forward_proxy_indexes(pool: &Pool<Sqlite>) -> Result<()> {
    ensure_forward_proxy_attempt_indexes(pool).await?;
    ensure_pool_upstream_request_identity_indexes(pool).await?;
    ensure_pool_upstream_request_timeline_indexes(pool).await?;
    ensure_forward_proxy_weight_indexes(pool).await?;
    Ok(())
}

async fn ensure_forward_proxy_attempt_indexes(pool: &Pool<Sqlite>) -> Result<()> {
    sqlx::query(
        r#"
...
async fn ensure_pool_upstream_request_identity_indexes(pool: &Pool<Sqlite>) -> Result<()> {
    sqlx::query(
        r#"
...
async fn ensure_pool_upstream_request_timeline_indexes(pool: &Pool<Sqlite>) -> Result<()> {
    sqlx::query(
        r#"
...
async fn ensure_forward_proxy_weight_indexes(pool: &Pool<Sqlite>) -> Result<()> {
    sqlx::query(
        r#"
