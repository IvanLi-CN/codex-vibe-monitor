...
pub(super) async fn ensure_schema_rollups(pool: &Pool<Sqlite>) -> Result<(bool, bool)> {
    let (has_existing_rows, added_columns) = ensure_invocation_rollup_hourly(pool).await?;
    ensure_failure_and_proxy_rollups(pool).await?;
    ensure_parallel_work_rollups(pool).await?;
    ensure_timeseries_projection_rollups(pool).await?;
    Ok((has_existing_rows, added_columns))
}

async fn ensure_invocation_rollup_hourly(pool: &Pool<Sqlite>) -> Result<(bool, bool)> {
...
async fn ensure_failure_and_proxy_rollups(pool: &Pool<Sqlite>) -> Result<()> {
    ensure_failure_rollup(pool).await?;
    ensure_proxy_perf_rollup(pool).await?;
    ensure_prompt_cache_rollups(pool).await?;
    Ok(())
}

async fn ensure_failure_rollup(pool: &Pool<Sqlite>) -> Result<()> {
    sqlx::query(
        r#"
...
async fn ensure_proxy_perf_rollup(pool: &Pool<Sqlite>) -> Result<()> {
    sqlx::query(
        r#"
...
async fn ensure_prompt_cache_rollups(pool: &Pool<Sqlite>) -> Result<()> {
    sqlx::query(
        r#"
...
async fn ensure_parallel_work_rollups(pool: &Pool<Sqlite>) -> Result<()> {
    ensure_parallel_work_minute_key_rollups(pool).await?;
    ensure_parallel_work_hourly_rollups(pool).await?;
    ensure_parallel_work_coverage(pool).await?;
    Ok(())
}

async fn ensure_parallel_work_minute_key_rollups(pool: &Pool<Sqlite>) -> Result<()> {
...
async fn ensure_parallel_work_hourly_rollups(pool: &Pool<Sqlite>) -> Result<()> {
    sqlx::query(
        r#"
...
async fn ensure_parallel_work_coverage(pool: &Pool<Sqlite>) -> Result<()> {
    sqlx::query(
        r#"
...
async fn ensure_timeseries_projection_rollups(pool: &Pool<Sqlite>) -> Result<()> {
    sqlx::query(
        r#"
