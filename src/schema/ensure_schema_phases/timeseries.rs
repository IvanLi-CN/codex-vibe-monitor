...
pub(super) async fn ensure_schema_timeseries(pool: &Pool<Sqlite>) -> Result<()> {
    ensure_timeseries_recovery_table(pool).await?;
    ensure_non_proxy_terminal_trigger(pool).await?;
    ensure_timeseries_rollup_tables(pool).await?;
    Ok(())
}

async fn ensure_timeseries_recovery_table(pool: &Pool<Sqlite>) -> Result<()> {
...
async fn ensure_non_proxy_terminal_trigger(pool: &Pool<Sqlite>) -> Result<()> {
...
async fn ensure_timeseries_rollup_tables(pool: &Pool<Sqlite>) -> Result<()> {
    sqlx::query(
        r#"
