...
pub(super) async fn ensure_schema_defaults_and_maintenance(pool: &Pool<Sqlite>) -> Result<()> {
    ensure_default_forward_proxy_and_startup_progress(pool).await?;
    ensure_system_task_runs_table(pool).await?;
    normalize_system_task_runs_and_indexes(pool).await?;
    ensure_raw_payload_metrics(pool).await?;
    seed_dependent_schemas(pool).await?;
    Ok(())
}

async fn ensure_default_forward_proxy_and_startup_progress(pool: &Pool<Sqlite>) -> Result<()> {
...
async fn ensure_system_task_runs_table(pool: &Pool<Sqlite>) -> Result<()> {
    sqlx::query(
        r#"
...
async fn normalize_system_task_runs_and_indexes(pool: &Pool<Sqlite>) -> Result<()> {
    normalize_system_task_runs(pool).await?;
    ensure_system_task_run_indexes(pool).await?;
    Ok(())
}

async fn normalize_system_task_runs(pool: &Pool<Sqlite>) -> Result<()> {
    sqlx::query(
        r#"
...
async fn ensure_system_task_run_indexes(pool: &Pool<Sqlite>) -> Result<()> {
    sqlx::query(
        r#"
...
async fn ensure_raw_payload_metrics(pool: &Pool<Sqlite>) -> Result<()> {
    sqlx::query(
        r#"
...
async fn seed_dependent_schemas(pool: &Pool<Sqlite>) -> Result<()> {
