...
mod projection_checkpoints;

pub(super) async fn ensure_schema_account_stats_minute(
    pool: &Pool<Sqlite>,
    state: &mut SchemaAccountStatsState,
) -> Result<()> {
    ensure_minute_columns_and_count(pool, state).await?;
    ensure_network_host_minute(pool).await?;
    ensure_socket_and_sticky_tables(pool).await?;
    ensure_forward_proxy_and_node_archive(pool).await?;
    ensure_node_hourly_archive(pool).await?;
    ensure_archive_replay_table(pool).await?;
    ensure_archive_guard_triggers(pool).await?;
    ensure_rollup_progress_tables(pool).await?;
    ensure_summary_source_change_tables(pool).await?;
    ensure_summary_compaction_backfill(pool).await?;
    ensure_summary_source_cursor_and_checkpoint(pool).await?;
    ensure_snapshot_table(pool).await?;
    ensure_snapshot_backfill_tables(pool).await?;
    ensure_snapshot_proof_and_obligations(pool).await?;
    migrate_terminal_summary_outcomes(pool).await?;
    ensure_snapshot_outcome_columns(pool).await?;
    projection_checkpoints::ensure_summary_projection_checkpoint(pool).await?;
    projection_checkpoints::ensure_account_projection_checkpoint(pool).await?;
    projection_checkpoints::ensure_usage_projection_checkpoint(pool).await?;
    Ok(())
}

async fn ensure_minute_columns_and_count(
    pool: &Pool<Sqlite>,
    state: &mut SchemaAccountStatsState,
) -> Result<()> {
...
async fn ensure_network_host_minute(pool: &Pool<Sqlite>) -> Result<()> {
    sqlx::query(
        r#"
...
async fn ensure_socket_and_sticky_tables(pool: &Pool<Sqlite>) -> Result<()> {
    sqlx::query(
        r#"
...
async fn ensure_forward_proxy_and_node_archive(pool: &Pool<Sqlite>) -> Result<()> {
    sqlx::query(
        r#"
...
async fn ensure_node_hourly_archive(pool: &Pool<Sqlite>) -> Result<()> {
...
async fn ensure_archive_replay_table(pool: &Pool<Sqlite>) -> Result<()> {
    sqlx::query(
        r#"
...
async fn ensure_archive_guard_triggers(pool: &Pool<Sqlite>) -> Result<()> {
    let mut archive_guard_trigger_tx = pool
        .begin_with("BEGIN IMMEDIATE")
        .await
        .context("failed to begin authoritative invocation archive guard refresh")?;
    drop_archive_guard_triggers(archive_guard_trigger_tx.as_mut()).await?;
    create_archive_guard_triggers(archive_guard_trigger_tx.as_mut()).await?;
...
async fn drop_archive_guard_triggers(tx: &mut sqlx::SqliteConnection) -> Result<()> {
    sqlx::query(
...
async fn create_archive_guard_triggers(tx: &mut sqlx::SqliteConnection) -> Result<()> {
    sqlx::query(
        r#"
...
async fn ensure_rollup_progress_tables(pool: &Pool<Sqlite>) -> Result<()> {
    sqlx::query(
        r#"
...
async fn ensure_summary_source_change_tables(pool: &Pool<Sqlite>) -> Result<()> {
    sqlx::query(
        r#"
...
async fn ensure_summary_compaction_backfill(pool: &Pool<Sqlite>) -> Result<()> {
...
async fn ensure_summary_source_cursor_and_checkpoint(pool: &Pool<Sqlite>) -> Result<()> {
    sqlx::query(
        r#"
...
async fn ensure_snapshot_table(pool: &Pool<Sqlite>) -> Result<()> {
    sqlx::query(
        r#"
...
async fn ensure_snapshot_backfill_tables(pool: &Pool<Sqlite>) -> Result<()> {
    sqlx::query(
        r#"
...
async fn ensure_snapshot_proof_and_obligations(pool: &Pool<Sqlite>) -> Result<()> {
...
async fn migrate_terminal_summary_outcomes(pool: &Pool<Sqlite>) -> Result<()> {
...
async fn ensure_snapshot_outcome_columns(pool: &Pool<Sqlite>) -> Result<()> {
    ensure_column_with_definition(
        pool,
