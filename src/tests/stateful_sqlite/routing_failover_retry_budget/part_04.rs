...
async fn resolve_pool_account_for_request_with_wait_wakes_when_model_reservation_is_released() {
    ...
    let started = Instant::now();
    let resolution = resolve_after_model_reservation_release(&state, model)
        .await
...
async fn resolve_after_model_reservation_release(
    state: &AppState,
    model: &str,
...
fn pool_route_existing_sticky_owner_preserves_last_failure_when_cutout_target_is_unusable() {
    ...
    .await;
        ...
        assert_sticky_owner_preserved_failure(&state, &attempts, response, primary_id).await;
...
async fn assert_sticky_owner_preserved_failure(
    state: &Arc<AppState>,
