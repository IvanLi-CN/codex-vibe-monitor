...
async fn assert_persisted_capture_attempts(pool: &SqlitePool, primary_id: i64) {
    ...
    >(
        "SELECT upstream_account_id, attempt_index, distinct_account_index,
                same_account_retry_index, status, http_status, failure_kind, stream_latency_ms
    ...
    assert_eq!(rows[0].0, Some(primary_id));
    assert_eq!(rows[0].1, 1);
    assert_eq!(rows[0].2, 1);
    assert_eq!(rows[0].3, 1);
    assert_eq!(rows[0].4, POOL_UPSTREAM_REQUEST_ATTEMPT_STATUS_HTTP_FAILURE);
    assert_eq!(
        rows[0].6.as_deref(),
    ...
    assert_eq!(rows[2].4, POOL_UPSTREAM_REQUEST_ATTEMPT_STATUS_SUCCESS);
    assert_eq!(rows[2].5, Some(200));
    assert!(rows[2].7.unwrap_or_default() >= 0.0);
...
async fn pool_route_existing_sticky_owner_preserves_last_failure_after_distinct_budget_exhausts() {
    ...
    assert_distinct_budget_failure_preserved(
        &state,
        &attempts,
        response,
        primary_id,
...
async fn assert_distinct_budget_failure_preserved(
    state: &Arc<AppState>,
    ...
    let attempt_rows = sqlx::query_as::<_, AttemptStatusRow>(
        "SELECT status, failure_kind FROM pool_upstream_request_attempts ORDER BY attempt_index ASC",
    )
    .fetch_all(&state.pool)
    .await
...
#[tokio::test]
async fn pool_route_uses_ungrouped_transit_when_grouped_alternate_exists() {
...
#[tokio::test]
async fn pool_route_uses_ungrouped_transit_when_it_is_only_candidate() {
    let (upstream_base, attempts, upstream_handle) = spawn_pool_retry_upstream(&[]).await;
    let state =
        test_state_with_openai_base(Url::parse(&upstream_base).expect("valid upstream base url"))
            .await;
...
#[tokio::test]
async fn pool_route_keeps_ungrouped_sticky_transit_when_cut_out_is_allowed() {
    let (upstream_base, attempts, upstream_handle) = spawn_pool_retry_upstream(&[]).await;
    let state =
        test_state_with_openai_base(Url::parse(&upstream_base).expect("valid upstream base url"))
            .await;
...
#[tokio::test]
async fn pool_route_keeps_ungrouped_sticky_transit_when_cut_out_is_forbidden() {
    let (upstream_base, attempts, upstream_handle) = spawn_pool_retry_upstream(&[]).await;
    let state =
        test_state_with_openai_base(Url::parse(&upstream_base).expect("valid upstream base url"))
            .await;
...
fn priority_handoff_does_not_retry_upstream_413() {
    let _priority_handoff_guard = crate::upstream_accounts::priority_handoff_test_guard_blocking();
