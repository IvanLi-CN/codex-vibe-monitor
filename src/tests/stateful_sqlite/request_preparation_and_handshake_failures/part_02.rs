...
async fn seed_pending_streaming_attempt(
    ...
    insert_pool_upstream_request_attempt_with_scope(
        ...
        PoolAttemptInsertRequest {
            ...
            phase: Some(POOL_UPSTREAM_REQUEST_ATTEMPT_PHASE_STREAMING_RESPONSE),
            connect_latency_ms: Some(10.0),
            first_byte_latency_ms: Some(180.0),
...
async fn seed_failed_predecessor_attempt(
    ...
    insert_pool_upstream_request_attempt_with_scope(
        ...
        PoolAttemptInsertRequest {
            ...
            phase: Some(POOL_UPSTREAM_REQUEST_ATTEMPT_PHASE_FAILED),
            http_status: Some(StatusCode::BAD_GATEWAY),
            failure_kind: Some("upstream_failure"),
            error_message: Some("first attempt failed"),
            ..PoolAttemptInsertRequest::default()
        },
    )
    .await
