...
async fn ensure_schema_keeps_legacy_fast_mode_rewrite_mode_column_inert() {
    ...
    assert!(
        columns
            .iter()
            .any(|column| column == "openai_proxy_websocket_enabled"),
        "websocket downstream setting column should exist",
    );
    assert!(
        columns
            .iter()
            .any(|column| column == "openai_proxy_upstream_websocket_default_enabled"),
        "websocket upstream setting column should exist",
    );
    assert!(
        columns
            .iter()
            .any(|column| column == "request_body_logging_enabled"),
        "request body logging setting column should exist",
    );
    assert!(
        columns
            .iter()
