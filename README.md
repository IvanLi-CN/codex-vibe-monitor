...
- 工作流：
  ...
  - `.github/workflows/ci-main.yml`：`push main` 后运行主线校验；每个 merged commit 独立成组，running run 非抢占，也不会因为共享 pending 队列丢掉较早 commit。
  - `.github/workflows/release.yml`：由 `CI Main` 首次成功（`run_attempt == 1`）后经 `workflow_run` 触发发布；也支持 `workflow_dispatch(commit_sha)` 手动 backfill，但只接受已经成功跑过 `CI Main` 的 `main` commit。
...
- 并发约定：
  ...
  - `workflow_dispatch(commit_sha)` 会校验目标 commit 已成功通过 `CI Main`；RC 版本后缀与版本基线都按 `target_sha` 解析，避免把更旧 commit 误打成更新版本。
- 版本与 tag 规则：
