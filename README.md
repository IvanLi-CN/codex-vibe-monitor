...
- 工作流：
  ...
  - `.github/workflows/release.yml`：由任一成功完成的 `CI Main` `workflow_run` 触发发布；也支持 `workflow_dispatch(commit_sha)` 手动 backfill。手动 backfill 接受两类 `main` commit：已经成功跑过 `CI Main` 的 commit，或仅在 `Release Snapshot` 这一步失败、其余 `CI Main` 校验均已通过的历史 commit；手动入口会使用当前主干脚本补齐 immutable snapshot。
...
- 并发约定：
  ...
  - `workflow_dispatch(commit_sha)` 会校验目标 commit 要么已成功通过 `CI Main`，要么仅因 `Release Snapshot` 失败而阻塞；对于后者，release 会先用当前主干脚本补齐 immutable snapshot。若该历史 commit 没有 frozen intent artifact，则只在这个手动 backfill 路径下允许回退到 merged PR 的当前 labels。整个流程仍只消费 immutable snapshot，不会在自动路径重新读取当前 PR labels，也不会重新计算版本。
