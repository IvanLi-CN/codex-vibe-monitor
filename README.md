...
安装分为 repo root、`web`、`docs-site` 三项 `bun install --frozen-lockfile` 和一项 `cargo fetch --locked`，每项有独立的 Git-metadata digest 与存在性检查。资源同步与自动 setup 都使用 per-worktree 非阻塞 advisory lock；持锁进程退出后锁自动释放，手动 setup 会串行等待。自动 `post-checkout` 对同一 failed digest 只记录一次并不阻断 checkout；手动 bootstrap 会重试并在存在失败时返回非零。若需只执行依赖安装，或强制恢复四项，可使用：
