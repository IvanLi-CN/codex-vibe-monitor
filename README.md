...
- 工作流：
  ...
  - `.github/workflows/release.yml`：由任一成功完成的 `CI Main` `workflow_run` 触发发布；也支持 `workflow_dispatch(commit_sha)` 手动 backfill，但只接受已经成功跑过 `CI Main` 且存在 immutable snapshot 的 `main` commit。
