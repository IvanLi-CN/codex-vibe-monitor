...
- 并发约定：
  ...
  - `workflow_dispatch(commit_sha)` 会校验目标 commit 要么已成功通过 `CI Main`，要么仅因 `Release Snapshot` 失败而阻塞；对于后者，release 会先用当前主干脚本补齐 immutable snapshot，再继续发布。整个流程仍只消费 immutable snapshot，不会重新读取当前 PR labels，也不会重新计算版本。
