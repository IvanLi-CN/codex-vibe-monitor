...
[![CI PR](https://github.com/IvanLi-CN/codex-vibe-monitor/actions/workflows/ci-pr.yml/badge.svg?branch=main)](https://github.com/IvanLi-CN/codex-vibe-monitor/actions/workflows/ci-pr.yml)
...
- 工作流：
  ...
  - `.github/workflows/ci-main.yml`：`push main` 后运行主线校验；running run 非抢占，不会被后续 push 打断。
  - `.github/workflows/release.yml`：由 `CI Main` 成功后经 `workflow_run` 触发发布；也支持 `workflow_dispatch(commit_sha)` 手动 backfill。
