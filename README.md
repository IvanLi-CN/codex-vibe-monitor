...
- 工作流：
  ...
  - `.github/workflows/ci-main.yml`：`push main` 后运行主线校验，并只为当前 merged commit 写入不可变 release snapshot；默认只消费 PR 阶段 trusted workflow 预冻结的 `release-intent` artifact，历史补发才允许走受限 legacy label 回退；每个 commit 独立成组，running run 非抢占，也不会因为共享 pending 队列丢掉较早 commit。
  - `.github/workflows/release.yml`：由任一成功完成的 `CI Main` `workflow_run` 触发发布；也支持 `workflow_dispatch(commit_sha)` 手动 backfill。手动 backfill 接受两类 `main` commit：已经成功跑过 `CI Main` 的 commit，或仅在 `Release Snapshot` 这一步失败、其余 `CI Main` 校验均已通过的历史 commit；手动入口会使用当前主干脚本只为目标 commit 补齐 immutable snapshot。
- PR 发版意图（labels，必须且各 1 个）：
  ...
  - `Label Gate` 在 trusted base 上校验通过后会上传 `release-intent-pr-<pr_number>-<pr_head_sha>` artifact，冻结 PR 的发布意图给后续 `CI Main` / `Release` 使用。
- 并发约定：
  ...
  - `workflow_run` 会接受成功完成的 `CI Main` rerun；发布去重依赖按 `target_sha` 的并发分组，以及 snapshot/tag/release 的幂等保护，而不是依赖 `run_attempt == 1`。
  - `workflow_dispatch(commit_sha)` 会校验目标 commit 要么已成功通过 `CI Main`，要么仅因 `Release Snapshot` 失败而阻塞；对于后者，release 会先用当前主干脚本只补齐目标 commit 的 immutable snapshot。若该历史 commit 没有 frozen intent artifact，则只在这个手动 backfill 路径下允许回退到 merged PR 的当前 labels。整个流程仍只消费 immutable snapshot；自动 `CI Main` 只处理当前 commit，不会因为更早历史缺少 snapshot 而阻塞新的 merged commit。
  - legacy label 回退会回放 merge 时刻 labels，并且只在目标 commit 的前一个 mainline parent 还处于“未部署 artifact-capable Label Gate”状态时才允许；一旦前一个 mainline parent 已经具备 artifact 冻结能力，缺少预冻结 artifact 的新 PR 会在 `CI Main` fail closed。
- 版本与 tag 规则：
  - stable：`vX.Y.Z`
