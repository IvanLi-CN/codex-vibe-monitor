...
CODEX_INVOCATION_ARCHIVE_LAYOUT=segment_v1       # (`segment_v1`，codex_invocations 走不可变日分片)
CODEX_INVOCATION_ARCHIVE_SEGMENT_GRANULARITY=day # (`day`，当前仅支持按上海自然日分片)
INVOCATION_ARCHIVE_CODEC=gzip                    # (`gzip`，segment 文件压缩格式)
...
- `codex_invocations` 的成功记录超过 30 个上海自然日后，会先把完整行写入对应上海自然日的不可变 archive segment，再把主库内的原始 payload / raw response / raw file 引用精简为 `structured_only`，但保留结构化统计字段用于在线排障。
- 任意调用记录超过 90 个上海自然日后，会先归档到 `ARCHIVE_DIR/codex_invocations/YYYY/MM/DD/part-<seq>.sqlite.gz`，写入 `archive_batches` manifest 后再从主库删除；其他 dataset 仍沿用现有月归档布局。
...
cargo run -- maintenance verify-archive-storage --dry-run
cargo run -- maintenance verify-archive-storage
cargo run -- maintenance prune-archive-batches --dry-run
...
GHCR 发布镜像默认提供多架构 manifest（`linux/amd64` + `linux/arm64`）。`rc` 永远不会更新 `${image}:latest`；`stable` 只有在不存在更高的已发布 stable 时才会更新 `${image}:latest`，未发布的 pending stable 不会提前压掉它。
...
- 工作流：
  ...
  - `.github/workflows/ci-pr.yml`：PR / merge_group 检查；`main` 只接受 PR 合并，且 PR 必须通过全部 7 个 required checks（`Validate PR labels`、`Lint & Format Check`、`Front-end Tests`、`Records Overlay E2E`、`Backend Tests`、`Build Artifacts`、`Review Policy Gate`）后才可合并。旧 PR run 可被新提交抢占取消。
...
- GitHub `main` ruleset：
  - 必须开启 `Require a pull request before merging`
  - 必须禁止 bypass（包括管理员 / owner 直推）
  - 必须要求上述 7 个 required checks 全绿
  - 必须保持 `Require signed commits`、禁止 force push、禁止删除分支
...
- 版本与 tag 规则：
  ...
  - stable snapshot 的 immutable `tags_csv` 只冻结版本 tag；是否额外发布 `${image}:latest` 会在 release 阶段按“最新已发布 stable”动态重算。
- 镜像：推送至 GHCR `ghcr.io/ivanli-cn/codex-vibe-monitor`
  - stable：始终发布 `${image}:vX.Y.Z`；仅当不存在更高的已发布 stable 时，同时发布 `${image}:latest`
