...
- 工作流：
  ...
  - `.github/workflows/ci-main.yml`：`push main` 后运行主线校验，并只为当前 merged commit 写入不可变 release snapshot；发布意图直接来自该 commit 关联 PR 的当前 labels。整个 workflow 使用固定并发组 `ci-main-main` 串行执行，运行中的主线 run 不会被新 merge 取消，也不会并发推断同一个下一个版本号。
  - `.github/workflows/release.yml`：由任一成功完成的 `CI Main` `workflow_run` 触发发布；也支持 `workflow_dispatch(commit_sha)` 手动 backfill。手动 backfill 接受已经成功跑过 `CI Main` 的 commit，或仅在 `Release Snapshot` 这一步失败、其余 `CI Main` 校验均已通过的历史 commit；手动入口会使用当前主干脚本只为目标 commit 补齐 immutable snapshot。整个 workflow 使用固定并发组 `release-main` 串行执行。
...
- 并发约定：
  ...
  - `CI Main` / `Release`：`cancel-in-progress: false`，并分别使用固定并发组 `ci-main-main` / `release-main` 做全局串行；新的 merge 不会取消较早的 main/release run。
