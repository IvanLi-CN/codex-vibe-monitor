...
- 并发约定：
  ...
  - legacy label 回退会回放 merge 时刻 labels，只对显式 allowlist（`PR #126` / `PR #130` / `PR #131` / `PR #133`）以及 rollout 过渡窗口内、在 `2026-03-16T00:00:00Z` 前 merged 的 PR 开放；其它缺少预冻结 artifact 的新 PR 会在 `CI Main` fail closed。
- 版本与 tag 规则：
  - stable：`vX.Y.Z`
