...
- 工作流：
  ...
  - `.github/workflows/ci-main.yml`：`push main` 后运行主线校验，并为每个 merged commit 写入不可变 release snapshot；默认只消费 PR 阶段 trusted workflow 预冻结的 `release-intent` artifact，历史补发才允许走受限 legacy label 回退；每个 commit 独立成组，running run 非抢占，也不会因为共享 pending 队列丢掉较早 commit。
...
- 并发约定：
  ...
  - `workflow_dispatch(commit_sha)` 会校验目标 commit 要么已成功通过 `CI Main`，要么仅因 `Release Snapshot` 失败而阻塞；对于后者，release 会先用当前主干脚本只补齐目标 commit 的 immutable snapshot。若该历史 commit 没有 frozen intent artifact，则只在这个手动 backfill 路径下允许回退到 merged PR 的当前 labels。整个流程仍只消费 immutable snapshot，不会在自动路径重新读取当前 PR labels，也不会因为更早历史缺少 snapshot 而阻塞这次 backfill。
